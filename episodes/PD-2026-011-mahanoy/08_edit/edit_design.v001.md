...
| **③ 本編** | act1〜act4（SPN-0003〜0021,0026〜0028 を物語順に） | shotlist 各ショット | 約9.5〜10分 |
| **④ エンディング** | 結末＋次回予告＋CTA/フォロー誘導（SPN-0022〜0025） | shotlist ending | 約80〜90秒 |

> **尺の目標＝約12分（11.5〜12.5分）**。台本（ナレ本文）は変えない。本編ナレ＋**フック(約30秒)＋オープニング＋エンディング(約80〜90秒)＋幕間の“ひと呼吸”**（山場SPN-0014の判決リビール後の余韻、act転換の短い間）で約12分に寄せる。間延びさせず密度で稼ぐ（カット切替は§2基準=約4.5秒を維持）。
...
- **字幕（ナレ全文・forced alignment）**: 画面**下部の安全帯**。**ナレと語単位でぴったり同期（ズレ≤約120ms）**・一字一句一致。**投稿の罵倒語は字幕にも出さない**（描写文のみ）。
- **見やすさ（VIDEO_RULES §13・数値）**: 文字 ≈48〜60px・本文太字・**白文字＋濃い縁取り/影＋半透明黒帯(不透明度~55〜70%)**・最大2行・中央寄せ・1〜2行ずつ送り読み切れる表示時間（高速点滅切替しない）。
...
### 美しくダイナミックにする加飾レイヤ（VIDEO_RULES §12）＝**DL済みファクトリ素材を活かす**
意味あるアニメ（上表）は**コード演出が主役**。その上に、`assets/asset_manifest.v001.json`（商用OK・DL済み）から**トーンの合うものだけ**を選び（`./.venv/Scripts/python.exe scripts/select_factory_assets.py` で type/サブタイプ抽出 → `remotion/public/mahanoy/factory/` へコピー）、`MahanoyPremium` に次の3層で重ねる：
- **背景プレート（下地）**：`backgrounds`/`loops` の暗め・抽象・機関系をシーン背後に薄く（ベタ塗り回避＝奥行き）。被写体はくっきり分離。
- **オーバーレイ（上）**：`light_assets`/`vfx_overlays`/`particle_assets` を **screen/add 合成**で光・きらめき・埃感。**リビール（SPN-0006のスクショ固定、SPN-0014の8–1）に光アクセント**。
- **質感**：`texture_assets`（紙/グレイン）を `overlay` で書類・年表・カードの下地に（SPN-0002 条文、0009 年表、0010 基準カード）。
合成は控えめに（主役を食わない）。テーマに合わない背景は使わない。コード演出（`Particles`/`LightSweep`）と二重がけしすぎない（過剰回避）。
- **映画的カメラ**：全カットに `MovingStage`/`CameraRig` で寄り/引き/パララックス＋イージング（spring/ease、リニア禁止）。山場は**ため→開放**でSFX同期。トランジションは上品・速め（光ワイプ/クロスディゾルブ）。
