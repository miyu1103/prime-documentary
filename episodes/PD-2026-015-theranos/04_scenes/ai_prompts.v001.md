# AI画像 生成リスト（設計書） — PD-2026-015-theranos

題材: Theranos, Elizabeth Holmes, and the line between a bold promise and criminal fraud

**保存先フォルダ（必ずここに保存）**: `H:\pd-media\assets\ai\theranos\`
各画像を **指定のファイル名** で上のフォルダに保存すること。保存後 `import_to_remotion.py 15 --write` を回すと、各場面へ自動で入る（仮の写真と差しかわる）。
