# AI画像 生成リスト（設計書） — PD-2026-012-arbitration

題材: Mandatory arbitration and class-action waivers — AT&T Mobility v. Concepcion (2011) and Epic Systems v. Lewis (2018)

**保存先フォルダ（必ずここに保存）**: `H:\pd-media\assets\ai\arbitration\`
各画像を **指定のファイル名** で上のフォルダに保存すること。保存後 `import_to_remotion.py 12 --write` を回すと、各場面へ自動で入る（仮の写真と差しかわる）。
