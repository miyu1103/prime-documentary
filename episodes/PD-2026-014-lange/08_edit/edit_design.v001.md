...
入力: `04_scenes/shotlist.v001.json`（23ショット・約619.6秒≒10.3分・本編実尺）, `03_script/script.en.v001.md`（`[VO:]`=ナレ本文・**変更禁止**・**ナレは ElevenLabs で生成OK＝課金承認待ち不要・止まらず進める**／詳細＝`06_audio/audio_cue_sheet.v001.md`）, `04_scenes/asset_map.v001.md`, AI画像 `H:\pd-media\assets\ai\lange\SPN-XXXX*.png`（生成済み前提）, 実写stock `H:\pd-media\episodes\PD-2026-014-lange\stock\*`（DL済み・asset_map参照）, サムネ `H:\pd-media\assets\ai\thumbs\lange\THUMB-01..06.png`（生成済み前提）。
...
§4品質ゲートの**最終確認版**（書き出し直前に通しで1回）。1つでも未達なら書き出さない。

- [ ] **4部構成**＝フック(約30秒)→オープニング(BrandOpening)→本編(act1–4)→エンディング(SPN-0019–0021)の順で並んでいる（§1）。
- [ ] **`coded/cards = 0`**＝全23ショットが実写 / AI画像 / 図解で埋まり、空カード・プレースホルダなし（§4）。
- [ ] **全カットが動く**＝静止画ゼロ・Ken Burns一辺倒でない。全カットに映画的カメラ（`MovingStage`/`CameraRig` の寄り引き/パララックス＋イージング・リニア禁止）（§2/§6）。
- [ ] **字幕がぴったり同期＆非重複**＝forced alignment で語単位・一字一句一致・ズレ≤約120ms。文字48〜60px・白＋濃い縁取り/影＋半透明黒帯(55〜70%)・最大2行・下部安全帯。字幕／テロップ(上・中央)／出典(右下・金ライン)が**位置で分離**し一度も被らない（§3）。
- [ ] **音4層＋ダッキング −14 LUFS**＝VO/BGM/SFX/ambience の4層。VOサイドチェインでBGM・ambienceが下がりナレ常時明瞭。integrated −14 LUFS／true peak ≤ −1 dBTP。山場SPN-0013と各テロップ出現にSFX同期、SPN-0001/0005「扉の下の足＝立入」に決定的ビート（`06_audio/audio_cue_sheet.v001.md`）。
- [ ] **9–0表記が正確**＝本編・テロップ・サムネで「**9–0＝判決（破棄差戻し）の全員一致**であり、**全員一致の“意見”ではない**（Roberts＋Alitoは結論同意・より広いルールを主張）」を取り違えない。`Vote`流用時は9マス全点灯＋"9–0"＋"in judgment"（§5/§6）。
- [ ] **中立・肖像なし**＝住居プライバシー vs 警察の安全に公平・Californiaを悪役化しない・争点は「立入の可否」でDUIの有罪無罪でない。実在人物（Arthur Lange・各判事）の肖像なし＝象徴のみ・ディープフェイク不可（§5）。
- [ ] **ファクトリ三層加飾**＝背景プレート(bg・薄く下地)＋light/particle/vfxオーバーレイ(screen/add)＋texture(overlay)。**実在subtypeのみ**・license=allowedのみ・出典/sha256記録。意味あるアニメ（コード演出）が主役・ファクトリは加飾（1カット1〜2レイヤ・主役を食わない）（§7）。
- [ ] **約12分**＝目安 11.5〜12.5分（本編ナレ＋フック＋オープニング＋エンディング＋幕間の“ひと呼吸”）。間延びさせず密度で稼ぐ（カット切替約4.5秒維持）（§1）。
- [ ] **ナレ確認待ちで止まっていない**＝ナレは ElevenLabs で生成OK（課金承認待ち不要）。中間ゲートで停止せず最終レンダー＋パッケージまで進める（停止点はYouTubeアップロード直前のみ）。
- [ ] **台本/claims/shotlist 不改変**＝`[VO:]`一字一句一致・SPNの内容を変えていない（Read専用）。
