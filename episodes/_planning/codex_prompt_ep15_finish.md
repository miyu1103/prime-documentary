...
現行 proxy v003 は **本編612.4s＋オープニング3.5s＋エンドカード9s＝624.9s（10.42分）** で、フックと幕間が無いため窓に届かない。§1の表どおり**不足分の約80秒を既存素材のみ**で足して **690〜750秒（狙い≈706秒＝11.8分）** に入れる。**フックは約7秒の短いtease（オーナー指定・長尺化しない）**なので、尺は**主に幕間の“ひと呼吸”と山場の余韻**で稼ぐ：
- **① フック（約7秒・短くpunchy）**＝本編の山場ハイライト **約3〜4カットの短いtease**＋riser SFX。**新規制作しない**（既存本編映像＋既存VO断片の流用）。候補から強い2〜3点だけ＝`SPN-0001`(一滴の血＋$9B→$0)/`0013`(2022・4件有罪の評決ボード)/`0016`(量刑~11y3m) 等。決め所VO断片は0〜1個だけ。**長く引き伸ばさない。R3：フックで断定有罪の含意や患者=有罪の誤読を作らない。** 最後は無音ぎみ→`BrandOpening` へブリッジ(約1.5秒)。
- **② 幕間の“ひと呼吸”×4（各約10秒・計40秒）**＝act1→2 / 2→3 / 3→4 / 4→ending の転換に**ナレなしの held ビジュアル＋音楽スウェル**（尺の主役はここ）。
- **③ 山場の余韻（約10秒）**＝評決ボード確定後（`SPN-0013`→`0014`）の“ため→開放”。
- **④ 主要グラフの間（計約15秒）**＝評価額 $9B→$0 崩落の着地／4件 count-up／境界の等式の組み上がりに小さな held。
- **⑤ CTA hold（約8秒）**＝`SPN-0022` Subscribe を少し長く持つ。
