...
## v002 追記（尺調整・2026-07-05）
- v001 ナレ音声実測 **8.42分**＝標準バンド(11.5–12.5)に**約3分不足**（EP14と同じ失敗モード）。→ 台本を **v002** に拡張（+7スパン/計2,087語）。
- 追加ビート（全て検証済み・水増しでなく驚き/学びを付加）：SPN-0028 スマホ＝最も親密な証人／SPN-0032 頻度で自分事化／SPN-0029 Payneの"指を選ばせたら別"留保／SPN-0033 割れた実例の人間の顔／**SPN-0021 国境・空港例外(FR-BORDER)**／SPN-0031 沈黙への再フック／**SPN-0030 再起動でパスコード強制(FR-BFU 端末仕様の事実)**。
- 3パス再確認：**Pass1** 追加ビートに MYTHS 混入なし（国境=「未確定/ほぼ拘束せず」、BFU=「端末仕様の事実・法的助言でない」）／**Pass2** 各追加が講義でなく場面・比喩・再フックで機能／**Pass3** 再フック密度と息継ぎ字幕OK。**全PASS**。
- v002 ナレ音声実測 **11.56分**（OP/ED込み最終≈11.8分）＝**バンド内**。追加7スパンのみ生成（26スキップ・idempotent）。
- 画像：国境ビート用に **IMG-41/42（空港・検査台）** を追加（元40枚に無い絵）。ショットリスト/READYパックを v002 反映。
