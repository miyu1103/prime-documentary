...
## 補助素材：新規取り込みの CC0/PD 音源棚（supplementary・SFXのみ）
- 台帳 **`H:\pd-media\assets\archive\_ledger\freesound.jsonl`**（**125件**・実体は `D:\pd-archive\sfx_environment\`）。実測内訳：**`license_decision` は全件 `cc0`**、`usage_tag` は全件 `sfx`、`theme` は全件 `sfx_environment`。キーワード分布は rain / thunder / water / wind / fire / room-tone 系が中心。
- **EP56 に効く候補**（すべて CC0・雰囲気レイヤ用）：`158691` / `158692` "Distant Thunder and Rain from Half Open Window"（**半開きの窓の雨＝本作の店内ビートに最適**）、`778366`–`778368` "Heavy Urban Rain With Distant Thunder"（夜の高街）、`407204` / `407205` / `407207` "room tone small with door and window open … distant traffic"（狭い部屋の room tone＝カウンター裏・取調べ）。
- **使用ルール（拘束）**：① これは **L4（SFX／薄い追いレイヤ）専用**。**L3 の正典 ambience ベッド集合（`AMBIENCE_BEDS`）は置き換えない**（ビルダーが固定・distinctness 採点の対象）。② 出荷ミックスに入れる前に **`SOUND_LIBRARY_RIGHTS.v001.json` へ登録**（id / license_decision / source_url / sha256 / commercial_ok）— 権利台帳収載が公開ゲート。③ **wind 主体のファイルは採らない**（`amb_light_wind` の「飛行機の音」前例）。④ 雷は**遠雷のみ**（近接の落雷は本作の抑制トーンに合わない）。⑤ CC0 のため attribution 不要だが、provenance（source_url・fetched_at・sha256）は台帳に残したまま運用する。
