...
| `comparebars` | 3 | robbery-first の弾劾戦術／多数意見 vs 反対意見の争点／type B vs type O |
...
| **合計** | **30** | variety = 8種（`quote/timeline/stat/comparebars/votetally/numberticker/pindropmap/acttitle`）≥3 ✓ |

`graphics[]`（kinetic typography）3枠: 幕タイトルの語同期切れ上がり等。→ variety に `kinetic_typography` が加わり 9種。
...
3. **同じ kind を連続させない。**
4. 1枠 4.0–8.0秒。
5. 幕3の説明区間に `comparebars`＋`quote`＋`timeline` を分散し 20秒超の平坦区間をゼロに。
