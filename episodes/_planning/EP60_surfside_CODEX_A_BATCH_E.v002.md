# EP60 surfside — Codex 画像生成 **バッチE：実写で埋まらなかった4領域** v002（100枚・1プロンプト1枚）

> ## ⚠ v002 で変えたのは **露光の床**です。プロンプト100行は1文字も変えていません。
> 納品済み179枚を実測したところ、**38枚（21.2%）が平均輝度45未満**でした。受入ゲート `check_image_cut_luma` の上限は **12%** なので、このままでは**レンダー後に確実に落ちます**（S102=14.0、S042=15.3）。
> 原因は Codex ではなく、v001 のスタイル指定にあった `near-black falloff at the edges` です。冷たさは残し、**黒つぶれだけをやめます**。
...
| **Q8** | **画面全体が暗すぎる**（スマホで見て何が写っているか分からない・影の中が真っ黒） |
...
[E-10] 全100枚の平均輝度を測る（グレースケール0-255）
       ・45未満が **0枚**
       ・100枚の中央値が **55以上**
       ・連続する暗いカットは S番号順で **3連続まで**
       （基準の出典: check_image_cut_luma.py DARK_YAVG=45 / DARK_FRAC_MAX=0.12 / DARK_RUN_MAX=4）
[E-9] BATCH_E_QC_v001.json を A/B/C/D と同じ形式で出力
      （schema は pd.surfside.batch_e_qc.v001。deliverable_count / generation_attempts /
        ...
        checks に all_279_present、all_279_sha256_duplicates_zero、luma_min_ge_45、luma_median_ge_55 を入れる）
```
