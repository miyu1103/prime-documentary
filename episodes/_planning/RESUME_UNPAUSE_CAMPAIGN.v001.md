...
## ⚠ 配信カレンダーの担当は1つに寄せる（2026-08-09 オーナー指示「ここでやって」）

2026-08-09 に**2つのセッションが同じカレンダーを同時に書き換えた**。結果は偶然一致したが、
両方が `reslot_short_schedule.py --apply` を打つと後から打った方が勝ち、ローカル台帳は
どちらとも合わなくなる（実際この日 30 本ずれた）。

- **公開カレンダーの操作（アップロード・予約・再配置）は1セッションだけが行う。**
- 日々の作業は `bash scripts/daily_shorts_push.sh [reserve]` 1本に集約した。
  索引キャッシュ削除 → 実チャンネル再取得 → 台帳同期 → クォータ内でアップロード →
  再取得して衝突を報告、まで含む。**自己申告で終わらない。**
- 長尺のアップロードが同日にある時は `daily_shorts_push.sh 1650`（1本あたり1650ユニット）。
- **12:00 JST は長尺専用。ショートは 6/9/18/21。**

---

## 2026-08-09 18:05 JST 最終チェックポイント — **キャンペーン完了（API実測）**

| 話 | 新video | 状態 |
|---|---|---|
| norfolk | `H8j_K1x9Dog` | private・publishAt **8/10 12:00 JST** |
...
- **norfolk の最終状態は下の16:00記述と異なる**: 即時公開後（0 views・処理中のうち）に
  ...
  （1回目のverifyでpublishAt=Noneに見えた）。
- 過去日付ガードは別スレ(initiate側)と本スレ(main側+verify 3回リトライ)の二重装備になった。
  両方とも実証済み。重複だが害はない。
- **ショート30本を再配置済み**（8/10〜8/17を4本/日 6/9/18/21 JSTへ、12:00完全回避、
  `reslot_short_schedule.py --apply` 実測30 OK）。
- 持ち越し: probe受領書の自動発行 / check_shipped_frames のリビジョン選択罠の恒久修正
  （willinghamはv002隔離で回避済み）/ ショート関連リンク54本（Studio UIのみ・ツール未整備）。

---

## 2026-08-09 16:00 JST 実測チェックポイント（別スレ記録・norfolkの結末は上書き済み）
