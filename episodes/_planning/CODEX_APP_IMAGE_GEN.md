...
【生成する画像】
各話の ai_prompts.v001.md にある各行（「`ファイル名` ← 内容, アングル」）＝それぞれ1枚。
記載のファイル名（連番 _02, _03 含む）でそのまま保存する。長い場面は約4.5秒ごとに切替えるため複数枚ある。
合計およそ **577枚**（必要なら増えてOK。枚数より「最高画質・美しさ」を優先）。

【スタイル（全画像共通・厳守）】
cinematic documentary still, ドラマチックで重厚な照明, 配色は「黒・濃紺・エレクトリックブルー・金」,
写実的, 浅い被写界深度, 16:9, 美しく映画的。
**最高画質：4K相当・超精細・シャープなフォーカス・質感まで緻密に。書き出しは長辺2048px以上（できれば3840px）。**
