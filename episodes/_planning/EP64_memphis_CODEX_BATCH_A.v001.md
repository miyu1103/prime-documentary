...
台本の修理でHOOKが作り直された。**§4のHOOK表（`M001` / `M002` / `M004`）は古い。**
プレートは1枚も増減していないので生成する枚数は **218枚のまま** だが、
**HOOKに実際に出るのは以下の3枚**である。

| HOOKカット | プレート | 内容 |
|---|---|---|
| 1 | `M003` | 一軍の家に届いた2通の請求書 |
| 2 | `M048` | 最終通知（状態4／状態8で回収） |
| 3 | `M005` | メアリー・クラフトの電話（ACT_5で回収） |

`M001`（壁の2つのメーター）・`M002`・`M004` は **HOOKから外れ、ACT_1 と ACT_5 の通常カットに戻る**。
