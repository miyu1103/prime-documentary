...
（最新標準＝`VIDEO_RULES.md §13` 準拠。長尺48〜60pxを縦1080×1920基準にスケールした数値で明示する。）

- ナレ音声＋台本VOから**語単位タイムスタンプ**を作る（`scripts/gen_captions_forced.py` 系を流用、出力を `ShortData.captions` に格納）。
- **ぴったり同期（最重要）**：`CaptionLayer` はナレと**語単位でぴったり同期**し、表示はナレと**一字一句一致**。**音と字幕のズレ ≤ 約120ms**を目標（早出し/遅出しを作らない）。
- **送り方**：現在時刻に応じて**1〜2行ずつ**表示（語ハイライトでも可）。読み切れる表示時間を確保し、**1語ごとの高速点滅切替はしない**（1〜2行単位で送る）。
- **見やすさ数値（縦1080×1920基準）**：
  - 本文＝**白文字・太字**（Trebuchet系）、**濃い縁取り/影**＋背後に**半透明の黒帯（不透明度 ~55〜70%）**。文字サイズ ≈ **56〜68px**（既存§4の本文サイズに一致）。
  - **最大2行**・中央寄せ・1行は折返し前提で短めに。
  - 位置＝**下ゾーン y1280〜1560 を厳守**（下端から約360px空ける＝UI回避）。**上ゾーンのテロップと縦に完全分離（一度も被らせない）**。出典は左上（`CitationTopLeft`）に逃がす。
