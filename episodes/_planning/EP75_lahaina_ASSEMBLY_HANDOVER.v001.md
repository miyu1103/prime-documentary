...
**⑤ `check_script_length` は「使ってよい。ただし帯を秒で渡すこと」**（2026-08-22 訂正）。
以前ここには「使わないでください」と書いていました。**その理由（HTMLコメントを語数に数える）は解消済み**で、
108行目が `<!--…-->` を除去します。実測 5,320語（ナレ索引の実数 5,338 と 0.3% 差）。
**残っている問題は別で、このツールは `episode_spec` を一切読みません**（`--slug` も無い）。
既定は 11.5–12.5 分にハードコードされており、`--lo/--hi` の単位は**秒**です。

```
check_script_length.py <script>                     → FAIL（11.5-12.5分の帯で判定）
check_script_length.py <script> --lo 1740 --hi 1920 → PASS（5,320語 / 3,973-5,480）
```
