...
| thumb_prompts | ✅ 4案 | `G220/G221/G222` ＋ 新規 `G226` 案 |
...
H:\pd-media\assets\ai\greene\G001.png … G226.png            ← 生成物（原本）
...
### 3-4. ループは `G206` で閉じる。`G209` ではない

台本 ENDING のコールバックは `G206`（紙が平らに貼られた同フレーム）。`G209`（テープごと消えた無地）は ENDING の別カット。**最後の画は `G226`**（褪せていない四角い跡）。§7 追加3 に明記。
