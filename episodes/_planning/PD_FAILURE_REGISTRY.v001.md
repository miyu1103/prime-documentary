...
| **F-19** | **本編カットに「光/パーティクル」素材が使われ、途中で4.7秒の黒が出る** | ①`AF-LIGHT`/`AF-PART`/`AF-VFX` は**画の上にスクリーン合成する板**でありb-rollではない。EP54は factory 218本が**全部これ**で、映像の68%が抽象エフェクトだった ②輝度検査が**中央1点サンプル**だった。AF-LIGHT-0498は中央31.8で合格したが両端は8.2→2.2でフェード黒 | ステージ時・マニフェスト時・プリゲート時の**3層**で: 名前による overlay-class 除外 ＋ **head/mid/tail の最小輝度**で判定 | 除外/overlayへ降格・exit 1 | **本日 EP54（ポストゲートが検出）** |
| **F-20** | **本数は足りているのに中身が要件を満たさない** | 「factory 218本」は健全に見えたが、b-rollは0本だった。**カウントはコンテンツの証拠にならない** | 素材プールは種別（実写背景 `AF-BG` か否か）まで見る。出荷前にコンタクトシートで目視 | — | **本日 EP54** |
