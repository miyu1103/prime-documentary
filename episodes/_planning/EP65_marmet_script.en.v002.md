# EP65 · MARMET HEALTH CARE CENTER AND BROWN — SCRIPT v002

**Second pass · facts `EP65_marmet_FACTS_LEDGER.v001.md` · remand `measurements/EP65_brown_remand_RAW.md` · design `EP65_marmet_FILM_BIBLE.v001.md` §19 · contract `episodes/PD-2026-065-marmet/episode_spec.v001.json`**

**Locks:** 8 keys as spelled · HOOK 8s per `PD_SCREENPLAY_STANDARD.v001.md` §16.5 · mid reveal 45–60% · primary 65–85% · nothing new past 92% · callback 70–90% · reset 55–70% · loop = **the ruled line** — motif state 1 at the head of ACT_1 returning as motif state 7 in the last frame (R225/R226 framing); the HOOK is a flash-forward of state 3 and is not the loop · exactly three ⟨HELD⟩, all after the heavy line · per curiam only · families never merged · no numbers.
...
> **尺の目標は 30:00**（オーナー指定 2026-08-04）。この稿の実測は **29:23**＝1,763秒（ナレーション 5,172語・176語/分・gap込み・HOOK は固定8秒）。区分見出しの (mm:ss) はこの一つの計算から導出しており、合計は 29:23 で一致する。最後の新事実（ACT_5 の処分）は 26:56＝**91.7%** で閉じ、92%線 27:02 の手前。契約帯は 27:00–32:00（spec runtime_seconds [1620,1920]）、`manifest.json` の `target_duration_minutes` は 30。

---

## HOOK (0:00-0:08 · voice from frame 0)

【4 hard cuts in 8.0s, from the HOOK plates fixed in `EP65_marmet_CODEX_BATCH_A.v001.md` §5: R001 2.2s → R002 1.8s → R005 2.0s → R003 2.0s. The on-screen line rides R003 into ACT_1. R004 (the empty armchair) comes OUT of the HOOK: two of the four opening cuts were chairs, and the chair is a one-off plant, not a second motif. R004 plays as an ordinary cut in ACT_5 beside R176, so no commissioned plate goes unused.】
【motif state 3, flash-forward, opening image: R001 — a ballpoint pen lying across the ruled line at the foot of an admission form, and on that line one unreadable stroke of ink, seen from directly above at close range. R001 is a top-down macro: it holds the line and nothing else, and it must not be asked to carry the second chair or the desk beyond it. Its payoff is inside the film — motif state 3 in ACT_1 (the four words a family member) and the ACT_5 callback. It is NOT the last image of the film. The macro loop is state 1 (the empty line, head of ACT_1) returning as state 7 (the empty line again, ENDING, plate R226).】

Two papers said this: a dispute about how somebody died goes to a private arbitrator. A dispute about an unpaid bill stays in a courthouse.
...
【motif state 1, and the chair plant, in one frame. Held 3s, no narration: plate R225 — the admission counter seen from the visitor's side, the camera close in over it so the form fills the lower third, the ruled line at the foot of the form legible at a glance and completely bare, and beyond the counter the two pale bent-wood armchairs, the near one drawn out and turned slightly away, the far one pushed in square and empty, late-afternoon daylight gone flat. This is the SAME eye, the same counter and the same far chair as the closing plate R226, which is what lets the loop close on one object instead of one lens. It is also the chair's ONLY appearance until that payoff, about 28 and a half minutes later: the chair is a plant, not a second motif, and R004 / R075 / R176 / R201 are ordinary cuts, not chair beats. NOTE: R222 and R223 were generated and REJECTED on 2026-08-05 — they came back as different rooms with different chairs (correlation −0.233), and R222 was shot so wide and so dark that the ruled line could not be read at all, so state 1 did not exist. R225 and R226 replace them and write the counter, the chairs, the light and the ruled line out in full in BOTH prompts. Do not use R222 or R223. Then cut to plate R152 — a hand comes in from the far side of the desk, presses the stack of sheets square and leaves the frame, plain cuff, no face, no badge. FIRST of the four appearances of that hand. It is the film's only human thread: the side that has the paper signed, executing the form. It never gets a face, a name, a line or a characteristic, per spec forbidden_subjects.】
...
Five weeks later, the same court did the same thing. The second order came on September 29, 2009, and it dismissed Jeffrey Taylor's suit against owners, operators and employees of the same nursing home. Taylor had made the same two arguments. The circuit court concluded that under the admission agreement he was required to arbitrate all of the claims asserted against the nursing home.
...
【rest beat, no narration, held 3s: plate R151 — a ring binder open at a divider tab with nothing filed behind it. Already commissioned; no new plate. The chair is NOT here: it is planted once, at the head of ACT_1 on R225, and is not cut in anywhere between there and R226.】
...
A five-page per curiam opinion is not a narrative.
...
【motif state 4: a hand flattens each sheet against the table, the crease still standing up under the palm, and withdraws (plate R038 — THIRD appearance of the hand); then two sheets laid side by side, the ruled line identical on both, and a third sheet beside them, the printed line above it different.】
...
The first holding went against the patients.
...
West Virginia had described Washington first.
...
【reset beat, 4s, no narration: motif state 5 again — the ink long dry, the page closed, the folder pushed home into the drawer.】
...
【motif state 6: the bound record open flat, one ruled line with its unreadable stroke standing alone on the page, everything around it blank.】

Part Two is one page long. It begins with the holding nobody had appealed.
...
The last clause is the savings clause from section 2, back again.
...
And in the next paragraph: however, in light of the parties' additional briefs and arguments, we modify our conclusions in Brown One.

Both sentences are in the same opinion.
...
On the first of them: the parties have not challenged our holding in Syllabus Point 11 of Brown One regarding the preemption of section 15(c) by section 2 of the FAA, and we need not revisit it.
...
What it kept was the body of contract law it had assembled in 2011 — the adhesion analysis, the sliding scale, the whole apparatus.

On the sliding scale, in the words it reaffirmed: a contract term is unenforceable if it is both procedurally and substantively unconscionable, but both need not be present to the same degree. The more substantively oppressive the contract term, the less evidence of procedural unconscionability is required to come to the conclusion that the clause is unenforceable, and vice versa.

On the substance of a term, this is the sentence it kept. Substantive unconscionability may manifest itself in the form of an agreement requiring arbitration only for the claims of the weaker party. But a choice of forums for the claims of the stronger party. Agreements to arbitrate, the court wrote, must contain at least a modicum of bilaterality to avoid unconscionability.

【callback: motif state 3, held on screen — the ruled line under the single unreadable stroke】
【OST: OTHER THAN CLAIMS TO COLLECT LATE PAYMENTS】
...
It had a sentence of the Supreme Court's own to lean on. Claims of coercion, fraud, or unequal bargaining power in the formation of an arbitration agreement are, that Court has said, best left for resolution in specific cases. Given that position, the state court wrote, further development of the factual record by the parties is proper.

Then its own account of the two records in front of it. The circuit court's order in Brown's case is devoid of any findings of fact or conclusions of law on the question of unconscionability. Taylor's order had some findings of fact, but that court had never comprehensively analysed the question under the guidelines the state court laid down in 2011. There was no evidence to weigh, because nobody had been permitted to take any.
...
Nothing in that second opinion holds these clauses unenforceable. Nothing in it holds them enforceable either. It hands the question back to the circuit courts and stops.
...
Which leaves the story where the documents leave it. Three families sued. Three papers were signed. What the litigation settled was which sovereign's law applied to those papers, and almost nothing about whether they were fair.
...
The clause, once more, as the two papers wrote it. To arbitrate all disputes, other than claims to collect late payments owed by the patient.

⟨HELD⟩

【motif state 7, final image: plate R226 — the foot of an admission form lying on the counter, its ruled line bare and no pen anywhere on it, and beyond the counter, slightly out of focus, the far chair still pushed in square, the late afternoon light gone flat. This is the commissioned plate, shot from the visitor's side at counter height, the camera in exactly the place it stood in ACT_1: the same eye as the plant (R225), which is what closes the plant and the loop together. The loop is the LINE, not one lens — state 1's empty line at the head of ACT_1 returns here as state 7, and the second chair pays off with it. Do NOT substitute R001: R001 is a top-down macro and cannot hold the chair or the desk beyond it.】
【OST: OTHER THAN】
