#!/usr/bin/env python
"""Create EP43 Caniglia dry-run media and a stub asset manifest.
...
PUBLIC = ROOT / "remotion" / "public" / "caniglia_dryrun"
...
def still_item(asset_id: str, scene_id: str, role: str, idx: int, color: tuple[int, int, int]) -> dict:
    ...
    return {
        ...
        "public_path": f"caniglia_dryrun/{rel.as_posix()}",
        "width": 3840,
        "height": 2160,
        "sha256": sha256(path),
        ...
        "also_thumb": scene_id in THUMB_IDS and role == "body",
        "qc": {
            "reviewed": True,
            "on_theme": True,
...
def main() -> int:
    ...
    stills = []
    for i in range(1, 86):
        stills.append(still_item(f"CANIGLIA-S{i:02d}", f"S{i:02d}", "body", i, (18 + i % 36, 36 + i % 42, 70 + i % 74)))
    for i in range(1, 17):
        stills.append(still_item(f"CANIGLIA-MS{i:02d}", f"M{i:02d}", "i2v_source", i, (34 + i % 32, 30 + i % 34, 46 + i % 42)))

    factory = []
    for i in range(1, 94):
        name = f"AF-CANIGLIA-STUB-{i:04d}__symbolic_motion.mp4"
        path = PUBLIC / "factory" / name
        ffmpeg_color(path, color_hex(i, (16, 22, 36)), 4.2, "1920x1080", 30)
        factory.append({
            "asset_id": f"AF-CANIGLIA-STUB-{i:04d}",
            "path": str(path).replace("\\", "/"),
            "public_path": f"caniglia_dryrun/factory/{name}",
            "kind": "video",
            "duration_sec": 4.2,
            "width": 1920,
            "height": 1080,
    ...
    for i in range(1, 17):
        ...
        motion.append({
            "asset_id": f"CANIGLIA-M{i:02d}",
            "source_scene_id": f"S{min(85, i * 5):02d}",
            "path": str(path).replace("\\", "/"),
            "public_path": f"caniglia_dryrun/motion/{name}",
            "kind": "video",
            "duration_sec": 3.417,
            "width": 1920,
            "height": 1080,
    ...
    overlay = []
    for i in range(1, 13):
        name = f"AF-CANIGLIA-OVERLAY-{i:04d}__dust.mp4"
        path = PUBLIC / "overlay" / name
        ffmpeg_color(path, color_hex(i, (8, 10, 14)), 2.0, "1920x1080", 30)
        overlay.append({
            "asset_id": f"AF-CANIGLIA-OVERLAY-{i:04d}",
            "path": str(path).replace("\\", "/"),
            "public_path": f"caniglia_dryrun/overlay/{name}",
            "kind": "video",
            "blend_hint": "screen",
            "sha256": sha256(path),
    ...
    ffmpeg_silence(PUBLIC / "audio" / "silence.m4a", 721.3)
    ffmpeg_color(DRY_EDIT / "caniglia_final_bgm.v002.mp4", "0x0a0a0c", 733.8, "1920x1080", 30)
    ...
    manifest = {
        ...
        "producer": "scripts/make_caniglia_stub_assets.py",
        "is_stub": True,
        "counts": {
            "still_body": 85,
            "still_i2v_source": 16,
