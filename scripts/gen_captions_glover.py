#!/usr/bin/env python
"""Generate EP48 Glover captions from narration_index text."""
...
def sentences(text: str) -> list[str]:
    protected = text.replace("U.S.", "U<S>").replace("v.", "v<dot>").replace("Jr.", "Jr<dot>").replace("No.", "No<dot>")
    parts = [p.strip() for p in re.split(r"(?<=[.?!])\s+", protected) if p.strip()]
    return [p.replace("U<S>", "U.S.").replace("v<dot>", "v.").replace("Jr<dot>", "Jr.").replace("No<dot>", "No.") for p in parts]
...
def cues_from_narr(narr: dict) -> list[dict]:
    ...
    for chunk in narr["chunks"]:
        ...
        for i, part in enumerate(parts):
            e = end if i == len(parts) - 1 else t + span * word_count(part) / total_words
            if e - t < 0.9:
                e = min(end, t + 0.9)
...
def selftest() -> int:
    narr = {"chunks": [{"start": 0, "end": 4, "text": "A plate is run. The stop stands."}]}
...
def main() -> int:
    ...
    ap.add_argument("--narr", type=Path, default=ROOT / "episodes" / EP / "06_audio" / "narration_index.v001.json")
