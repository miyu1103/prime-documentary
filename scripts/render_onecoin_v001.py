#!/usr/bin/env python3
"""Render OneCoin first-cut v001.

Renders OneCoinPremium chapter ranges with Remotion, concatenates them, then
performs the policy-required libx264 slow / CRF 16 final encode. No upload or
...
AUDIO = REMOTION / "public" / "onecoin" / "audio" / "onecoin_final_mix_v001.wav"
...
VISUAL_CONCAT = OUT_DIR / "onecoin_visual_concat_v001.mp4"
FINAL = OUT_DIR / "v001.mp4"
QC = EPDIR / "08_edit" / "renders" / "final.v001.qc.json"
EVENTS = EPDIR / "events" / "events.jsonl"
LOG = OUT_DIR / "render_onecoin_v001.log"
TARGET_SEC = 1800.0
...
def scheduled_rows() -> list[dict]:
    ...
    shots = data["shots"]
    source_total = sum(float(s["seconds"]) for s in shots)
    fixed = opening + endcard + 3.0
    scale = (TARGET_SEC - fixed) / max(1.0, source_total - 3.0)
    rows: list[dict] = []
    cursor = 0.0
    for shot in shots:
        dur = 3.0 if shot["spanId"] == SILENCE_SPAN_ID else float(shot["seconds"]) * scale
...
def chapter_ranges(fps: int) -> tuple[list[dict], int, float]:
    ...
    total_frames = max(1, round(TARGET_SEC * fps))
    ranges: list[dict] = []
    ...
    return ranges, total_frames, TARGET_SEC
        ...
        while cursor <= item["end"]:
            ...
            if chunk_path.exists() and chunk_path.stat().st_size > 500_000 and not force:
                ...
                run(
                    ...
                    f"render OneCoinPremium chapter {i}/5 {item['chapter']} chunk {chunk_index} frames {cursor}-{chunk_end}",
                    cwd=REMOTION,
                )
            cursor = chunk_end + 1
            chunk_index += 1
...
def write_qc() -> None:
    ...
    qc = {
        ...
        "video": video,
        "audio": audio,
        "render_method": "Remotion chapter-range renders, FFmpeg concat, final libx264 slow CRF16 yuv420p AAC 192k",
...
def update_manifest() -> None:
    ...
    manifest["state"] = "first_cut_ready"
    manifest.setdefault("active_revisions", {})["final_render"] = "v001"
    manifest.setdefault("active_revisions", {})["final_qc"] = "v001"
    artifacts = manifest.setdefault("artifacts", [])
    for aid in ["PD-2026-017-onecoin-final-render", "PD-2026-017-onecoin-final-qc"]:
        artifacts[:] = [a for a in artifacts if a.get("artifact_id") != aid]
    artifacts.append(
        {
            "artifact_id": "PD-2026-017-onecoin-final-render",
            "artifact_type": "final_render",
            "revision": "v001",
            "uri": "artifact://H:/pd-media/episodes/PD-2026-017-onecoin/07_edit/v001.mp4",
    ...
    artifacts.append(
        {
            "artifact_id": "PD-2026-017-onecoin-final-qc",
            "artifact_type": "final_render_qc",
            "revision": "v001",
            "uri": "artifact://episodes/PD-2026-017-onecoin/08_edit/renders/final.v001.qc.json",
            "checksum": sha256(QC),
            "status": "candidate",
            "rights_status": "conditional",
            "qc_status": "pass",
...
def append_event() -> None:
    ...
    event = {
        ...
        "revision": "v001",
        "actor": "codex",
        "note": "Rendered OneCoinPremium v001 first cut to H:/pd-media/episodes/PD-2026-017-onecoin/07_edit/v001.mp4 with libx264 slow CRF16. No upload/publish/schedule. Voice is local SAPI draft pending owner GO for ElevenLabs master.",
    }
