#!/usr/bin/env python3
"""Schedule EP49 strieff to YouTube (private, auto-publish at publishAt).
...
CONFIRM no existing strieff video -> resumable PRIVATE upload -> set thumbnail ->
schedule (status.publishAt) -> upload English caption -> re-query + verify.
If a PRIVATE strieff upload already exists (e.g. an agent died mid-upload, the EP40/EP41
...
run; until then the sha256 guard fails closed. APR ref: APR-0001 (approvals/APR-0001.json).
    ...
    py -3.11 scripts/schedule_strieff.py --dry-run
    py -3.11 scripts/schedule_strieff.py
"""
...
VIDEO_FILE = EPDIR / "08_edit" / "strieff_final_bgm.v003_ae.mp4"
THUMB_FILE = PKG / "thumbnail.face.v002.png"
CAPTION_FILE = EPDIR / "08_edit" / "captions.final.v001.srt"
EXPECTED_HASH = "7d1f48278bfbf60a10b1cec2ae0c571f890609981238ff4ba6aa0a5638ba7200"
PUBLISH_AT = "2026-08-01T03:00:00Z"
SCHEDULED_LOCAL = "2026-08-01T12:00:00+09:00"
...
def upload_caption(token, video_id):
    boundary = f"strieff_caption_{int(time.time())}"
...
def is_strieff_match(title, want_title):
    t = title.strip().lower()
    if '#short' in t:
        return False  # never match a Short as the full-episode upload
    return (title.strip() == want_title.strip()
            or "strieff" in t
            or "stop was illegal" in t
            or "stopped for nothing" in t
            or "kept the evidence" in t)
...
def main(argv):
    ...
    # Pre-flight: enumerate recent uploads, detect an existing strieff video (double-upload guard)
    print("  scanning recent channel uploads for an existing strieff video ...")
    recent = list_recent_uploads(token, ch, n=25)
    want_title = meta["title"]
    existing = [(vid, title, priv) for (vid, title, priv) in recent
                if is_strieff_match(title, want_title)]
    for vid, title, priv in recent[:12]:
        flag = "  <-- MATCH" if (vid, title, priv) in existing else ""
        print(f"    [{priv:8}] {vid}  {title[:60]}{flag}")
    if existing:
        print(f"  NOTE: {len(existing)} possible existing strieff upload(s) found.")
    ...
    if args.dry_run:
        ...
        if existing:
            print("[dry-run] existing strieff video(s) present -> real run would FINALIZE the "
                  "private one instead of re-uploading:")
            for vid, title, priv in existing:
                print(f"          {priv} {vid} {title[:50]}")
        return 0

    # Decide: finalize an existing PRIVATE strieff upload, else fresh upload
    reused = False
    video_id = None
    ...
    if priv_existing:
        ...
        reused = True
        print(f"OK reusing existing PRIVATE strieff upload video_id={video_id} (no re-upload)")
        caps = existing_captions(token, video_id)
