#!/usr/bin/env python3
"""Generate SHORT #1 (Miranda v. Arizona) vertical-short narration with the channel voice.

US-market English VO. Voice = channel voice, model = multilingual v2. Paid ElevenLabs (pre-authorized
by SHORTS_REMOTION_SPEC: "ナレ=ElevenLabs生成OK・課金承認待ち不要"). R1 / not sensitive (1966 case).
No real-person likeness. Idempotent: existing non-empty chunks are skipped. Spoken CTA is platform-neutral.

Mirrors scripts/gen_short18_narration.py; only EP/SHORT/LINES/GAP differ.
"""
...
# (line_id, delivery, spoken_text) — US-market English adaptation of SHORTS_EP1-8.md SHORT #1.
LINES: list[tuple[str, str, str]] = [
    ("L1", "intense", "Police read you your rights, and it isn't out of politeness."),
    ("L2", "building", "Decades ago, you could be questioned alone for hours, with no warning at all."),
    ("L3", "building", "Cornered, some people confessed to things they never did. In 1966, the Supreme Court drew a line: warn people before questioning them in custody."),
    ("L4", "intense", "It isn't a courtesy. It's a fix built into the system. Without the warning, a confession can be thrown out. A few seconds changed every arrest in America."),
...
def main() -> int:
    ...
    master = media_root() / "episodes" / EP / "06_voice" / "master" / "short01_vc_master_en_us_v002.mp3"
    master.parent.mkdir(parents=True, exist_ok=True)
    subprocess.run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(concat),
                    "-c:a", "libmp3lame", "-b:a", "192k", str(master)], check=True, capture_output=True)

    idx = ROOT / "episodes" / EP / "06_audio" / "short01_narration_index.v002.en_us.json"
