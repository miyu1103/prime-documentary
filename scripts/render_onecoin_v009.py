#!/usr/bin/env python3
"""Render OneCoin v009 editorial cut.
...
VISUAL_CONCAT = OUT_DIR / "onecoin_visual_concat_v009.mp4"
FINAL = OUT_DIR / "v009.mp4"
QC = EPDIR / "08_edit" / "renders" / "final.v009.qc.json"
EVENTS = EPDIR / "events" / "events.jsonl"
LOG = OUT_DIR / "render_onecoin_v009.log"
...
def scheduled_rows() -> tuple[list[dict], float]:
    ...
    if data.get("timelineMode") != "editorial":
        raise RuntimeError("v009 requires onecoin_roughcut.ts timelineMode='editorial'")
    ...
    for i, item in enumerate(ranges, 1):
        ...
        while cursor <= item["end"]:
            ...
            run(
                ...
                f"render OneCoinPremium v009 chapter {i}/5 {item['chapter']} chunk {chunk_index} frames {cursor}-{chunk_end}",
                cwd=REMOTION,
            )
            cursor = chunk_end + 1
            chunk_index += 1
...
def concat_visual(chapters: list[Path]) -> None:
    list_file = CHAPTER_DIR / "concat_v009.txt"
    list_file.write_text("".join(f"file '{p.as_posix()}'\n" for p in chapters), encoding="utf-8")
    tmp = VISUAL_CONCAT.with_suffix(".tmp.mp4")
    run([FFMPEG, "-y", "-f", "concat", "-safe", "0", "-i", list_file, "-map", "0:v:0", "-c", "copy", tmp], "concat v009 chapter visuals")
...
def final_encode() -> None:
    ...
    run(
        ...
        "final v009 libx264 slow crf16 mux",
    )
    tmp.replace(FINAL)
...
def update_manifest() -> None:
    ...
    manifest["state"] = "editorial_v009_rendered_pending_owner_review"
    active = manifest.setdefault("active_revisions", {})
    active["final_render"] = "v009"
    active["final_qc"] = "v009"
    artifacts = manifest.setdefault("artifacts", [])
    for aid in ["PD-2026-017-onecoin-final-render-v009", "PD-2026-017-onecoin-final-qc-v009"]:
...
def append_event() -> None:
    ...
    event = {
        ...
        "episode_id": EP,
        "stage": "render",
        "event": "editorial_v009_rendered",
        "revision": "v009",
        "actor": "codex",
        "note": "Rendered shorter OneCoin editorial cut v009 with breath-pause captions and improved caption display. No upload/publish/schedule.",
    }
