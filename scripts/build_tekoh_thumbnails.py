#!/usr/bin/env python3
r"""EP44 Tekoh CTR thumbnails v001 -- clones the winning Caniglia/Frazier v003 text treatment.

Vega v. Tekoh, 597 U.S. 175 (2022), a 6-3 Supreme Court decision: officers took an un-Mirandized
statement that was later used at a criminal trial (the defendant was ACQUITTED). He then sued the
officer under 42 U.S.C. section 1983, arguing the un-warned statement violated his rights. The Court
held that a violation of Miranda's procedural rule is NOT itself a violation of the Fifth Amendment
that can support a section 1983 civil damages suit. Miranda itself STILL stands as a rule at criminal
trial; what the Court closed was the separate DOOR to a civil lawsuit over a skipped warning. The
packaging never says "Miranda is dead" and never names the underlying suspected offense (R2, no face).

This reuses the EXACT winning layout from build_caniglia_thumbnails.py:
...
Backgrounds (episode's own SDXL 4K photoreal stills, E:/pd-media/assets/ai/tekoh):
  ...
  option 2  S04 -- a bare interrogation table with a single lit page and NO recorder present
                   (the room where a warning never came).
  option 3  S49 -- a single plain door in a cold marble wall, firmly shut and outlined in cold light
                   (the one door -- a civil suit -- the ruling closed).

R2: a bench / a table / a door -- no faces, no identifiable people, and no underlying-offense imagery.

    py -3.11 scripts/build_tekoh_thumbnails.py
"""
...
LANE = (47, 166, 160)    # #2FA6A0 -- EP44 tekoh lane color (teal)
WHITE = (247, 249, 252)
RED = (206, 44, 44)
CLAMP_LUMA = 188.0
...
def build(photo, brightness, contrast, line1, line2, kicker, out, accent2=LANE, centering=(0.5, 0.42)):
    ...
    if logo.exists():
        ...
        # background (e.g. lit marble columns) -> keeps check_thumb_subject_luma's outline metric valid
...
def main():
    ...
    specs = [
        ("S36.png", 1.80, 1.06, "YOU CAN'T", "SUE", "SUPREME COURT · 6-3", LANE, (0.50, 0.46)),
        ("S04.png", 1.95, 1.06, "AND YOU", "CAN'T SUE", "NO WARNING READ", LANE, (0.50, 0.50)),
        ("S49.png", 1.70, 1.06, "THE DOOR", "CLOSED", "NO WAY TO SUE", LANE, (0.50, 0.45)),
    ]
    paths = []
