#!/usr/bin/env python3
"""Create OneCoin v004 final candidate with ElevenLabs master audio.

Uses the already QC-passed v003 video stream and muxes in the v002 master mix.
...
SRC_VIDEO = Path("H:/pd-media/episodes/PD-2026-017-onecoin/07_edit/v003.mp4")
MASTER_MIX = ROOT / "remotion" / "public" / "onecoin" / "audio" / "onecoin_final_mix_v002.wav"
OUT = Path("H:/pd-media/episodes/PD-2026-017-onecoin/07_edit/v004.mp4")
QC = RENDERS / "final.v004.qc.json"
...
def mux() -> dict:
    ...
    qc = {
        ...
        "video": video,
        "audio": audio,
        "render_method": "v003 QC-passed video stream copied; v002 ElevenLabs master mix encoded AAC 192k",
        "upload_performed": False,
        "publish_performed": False,
        "schedule_performed": False,
        "voice_status": "elevenlabs_master",
    }
    write_json(QC, qc)
    return qc
...
def build_package(qc: dict) -> None:
    ...
    write_json(PACKAGE / "youtube_meta.v004.json", youtube_meta)
    ...
    rights["assets"] = [
        {
            "asset_id": f"{EP}-final-render-v004",
            "type": "final_render",
            "file": str(OUT).replace("\\", "/"),
            "sha256": final_sha,
    ...
    write_json(PACKAGE / "rights_manifest.v004.json", rights)
    ...
    delivery = {
        ...
        "youtube_meta": rel(PACKAGE / "youtube_meta.v004.json"),
        "rights_manifest": rel(PACKAGE / "rights_manifest.v004.json"),
        "owner_review_request": rel(PACKAGE / "OWNER_REVIEW_REQUEST.v004.md"),
    ...
    write_json(PACKAGE / "final_delivery.v004.json", delivery)

    review = f"""# OWNER REVIEW REQUEST v004 - OneCoin First Cut
...
"""
    (PACKAGE / "OWNER_REVIEW_REQUEST.v004.md").write_text(review, encoding="utf-8")
...
def update_manifest(qc: dict) -> None:
    ...
    active.update(
        {
            ...
            "audio_mix": "v002",
            "voice_master": "v001",
            "narration_index": "v002",
        }
    )
    manifest["state"] = "first_cut_packaged_v004_master_voice"
    artifacts = manifest.setdefault("artifacts", [])
    new_artifacts = [
        ("PD-2026-017-onecoin-final-render-v004", "final_render", "v004", f"artifact://{str(OUT).replace(chr(92), '/')}", sha256(OUT), "candidate", "conditional", "pass"),
        ("PD-2026-017-onecoin-final-qc-v004", "final_render_qc", "v004", "artifact://episodes/PD-2026-017-onecoin/08_edit/renders/final.v004.qc.json", sha256(QC), "candidate", "conditional", "pass"),
        ("PD-2026-017-onecoin-youtube-meta-v004", "youtube_meta", "v004", "artifact://episodes/PD-2026-017-onecoin/09_package/youtube_meta.v004.json", sha256(PACKAGE / "youtube_meta.v004.json"), "candidate", "conditional", "pass"),
        ("PD-2026-017-onecoin-rights-manifest-v004", "rights_manifest", "v004", "artifact://episodes/PD-2026-017-onecoin/09_package/rights_manifest.v004.json", sha256(PACKAGE / "rights_manifest.v004.json"), "candidate", "conditional", "pass"),
        ("PD-2026-017-onecoin-final-delivery-v004", "final_delivery", "v004", "artifact://episodes/PD-2026-017-onecoin/09_package/final_delivery.v004.json", sha256(PACKAGE / "final_delivery.v004.json"), "candidate", "conditional", "pass"),
        ("PD-2026-017-onecoin-owner-review-request-v004", "owner_review_request", "v004", "artifact://episodes/PD-2026-017-onecoin/09_package/OWNER_REVIEW_REQUEST.v004.md", sha256(PACKAGE / "OWNER_REVIEW_REQUEST.v004.md"), "candidate", "conditional", "pass"),
    ]
    ...
    with EVENTS.open("a", encoding="utf-8", newline="\n") as fh:
        fh.write(json.dumps({"ts": now(), "episode_id": EP, "stage": "package", "event": "first_cut_package_built", "revision": "v004", "actor": "codex", "provider": "elevenlabs_master", "upload_publish_schedule": False}, ensure_ascii=False) + "\n")
...
def main() -> int:
    ...
    build_package(qc)
    update_manifest(qc)
    print(json.dumps({"render": str(OUT), "sha256": sha256(OUT), "delivery": rel(PACKAGE / "final_delivery.v004.json"), "duration": qc["duration_seconds"]}, indent=2))
    return 0
