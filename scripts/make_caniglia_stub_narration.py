#!/usr/bin/env python
"""Build a deterministic EP43 narration_index stub from the approved script."""
...
SCRIPT = ROOT / "episodes" / "_planning" / "EP43_caniglia_script.en.v001.md"
OUT = ROOT / "episodes" / EP / "06_audio" / "narration_index.stub.v001.json"
TARGET_SECONDS = 721.3

SECTION_HEADINGS = [
    ("HOOK", r"^##\s+\d+\.\s+HOOK\b"),
    ("OP", r"^##\s+\d+\.\s+OPENING\b"),
    ("ACT_1", r"^###\s+ACT 1\b"),
    ("ACT_2", r"^###\s+ACT 2\b"),
    ("ACT_3", r"^###\s+ACT 3\b"),
    ("ENDING", r"^##\s+\d+\.\s+ENDING\b"),
]
STOP_HEADINGS = [r"^## \[BrandOpening\]", r"^## \[BrandEndcard\]", r"^## 事実対応表", r"^## 改稿ログ", r"^# "]
