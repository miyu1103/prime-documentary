#!/usr/bin/env python
"""Generate the 7 vertical stills + thumbnail for SHORT #12 (forced arbitration).

Source: episodes/_planning/SHORTS_EP9-15.md SHORT #12 + EP1-8 §1 style suffix.
Backend: local SDXL (AUTOMATIC1111 API) 127.0.0.1:7860, Juggernaut XL.
Output : H:/pd-media/assets/ai/shorts/short12/short12_01.png .. _07.png + short12_thumb.png
No legible on-screen text (concepts conveyed visually, not by readable words). Idempotent unless --force.
...
OUT = r"H:/pd-media/assets/ai/shorts/short12"
...
SHOTS = [
    ("short12_01", "A finger hovering over a single glowing button on a dark phone screen, faint unreadable fine print scrolling, no legible words"),
    ("short12_02", "Endless lines of unreadable fine print scrolling into darkness, no legible words"),
    ("short12_03", "Glowing chains cutting the links between many small human figures so they cannot join together, no faces"),
    ("short12_04", "A sleek smartphone with a small blank price tag dangling, quiet irony, no legible text"),
    ("short12_05", "A grand public courthouse door quietly closing, the shaft of light narrowing"),
    ("short12_06", "A perfectly level balance scale, a real debate, gold rim, dark background"),
    ("short12_07", "A workplace of isolated workers each alone in a separate pool of light, no faces"),
    ("short12_thumb", "A finger tapping a single glowing button over scrolling unreadable fine print, ominous key visual, no legible text"),
]
...
def gen(stem, core, force, no_hr):
    ...
    if os.path.exists(out_path) and not force:
        ...
        return
    seed = 120000 + int(stem.split("_")[1].replace("thumb", "99"))
    payload = {
...
def main():
    ...
    print("[all ] short12 image generation finished", flush=True)
