#!/usr/bin/env python3
"""Build OneCoin v006 editorial timeline, natural-speed audio, and captions.
...
REMOTION_AUDIO = REMOTION / "public" / "onecoin" / "audio" / "onecoin_final_mix_v004.wav"
MEDIA = Path("H:/pd-media")
WORK = MEDIA / "episodes" / EP / "06_audio" / "editorial_v006"
FIT_DIR = WORK / "fit_wav"
VOICE_INDEX = EPDIR / "06_audio" / "narration_index.v003.json"
CAPTIONS_SRT = EPDIR / "08_edit" / "captions.v004.editorial_v006.srt"
CAPTIONS_JSON = EPDIR / "08_edit" / "captions.v004.editorial_v006.json"
AUDIO_QC = EPDIR / "08_edit" / "audio_mix.v004.editorial_v006.qc.json"
PLAN = EPDIR / "08_edit" / "onecoin_editorial_plan.v006.json"
...
def richer_images(pool: list[str], offset: int, seconds: float) -> list[str]:
    if not pool:
        return []
    count = max(3, min(9, int(seconds / 2.2) + 2))
...
def build_voice_segments(rows: list[dict[str, Any]], voice: dict[str, Any]) -> tuple[list[dict[str, Any]], float]:
    ...
    for row in rows:
        ...
        if sid == HOOK_SPAN:
            ...
            run_ffmpeg([FFMPEG, "-y", "-f", "lavfi", "-i", "anullsrc=r=48000:cl=stereo", "-t", f"{OPENING_SECONDS:.3f}", "-c:a", "pcm_s16le", brand])
    ...
    narration = WORK / "narration_editorial_v006.wav"
...
def split_caption_parts(text: str) -> list[str]:
    ...
    for word in words:
        trial = " ".join(cur + [word])
        if cur and (len(trial) > 54 or len(cur) >= 8 or re.search(r"[.?!]$", cur[-1])):
...
def write_captions(timed: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in timed:
        ...
        for part, weight in zip(parts, weights):
            ...
            part_end = min(part_end, cursor + 4.2)
            if part_end <= cursor:
                part_end = cursor + 0.3
    ...
    CAPTIONS_JSON.write_text(json.dumps({"episode_id": EP, "revision": "v006", "alignment_method": "natural_voice_word_weighted_editorial", "cues": cues}, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    CAPTIONS_TS.write_text(
        "// Editorial v006 captions from natural-speed ElevenLabs master chunks.\n"
        "import type {CaptionCue} from '../compositions/RoughCut';\n\n"
        "export const ONECOIN_CAPTIONS: CaptionCue[] = "
        + json.dumps([{"id": f"onecoin-caption-v006-{i + 1:04d}", "start": cue["start"], "end": cue["end"], "text": cue["text"]} for i, cue in enumerate(cues)], indent=2, ensure_ascii=False)
        + ";\n",
        encoding="utf-8",
    )
    return cues
...
def build_mix(timed: list[dict[str, Any]], total_sec: float) -> None:
    ...
    filters: list[str] = []
    music_segments = [
        (0.0, bounds["cold_open"][1] + OPENING_SECONDS, lib / "music" / "hook" / "mus_20260614_hook_glass_air_bed_v2.mp3", 0.10),
...
def write_roughcut(base: dict[str, Any], rows: list[dict[str, Any]]) -> None:
    out = {
        "episodeId": EP,
        "title": "Nothing - OneCoin Editorial Cut v006",
        "fps": 30,
        "timelineMode": "editorial",
        "narrationSrc": "onecoin/audio/onecoin_final_mix_v004.wav",
        "bgmSrc": None,
        "shots": rows,
    }
    ROUGH_TS.write_text(
        "// EDITORIAL v006 generated by scripts/build_onecoin_editorial_v006.py.\n"
        "// Natural-speed narration, shortened hook, denser visual turnover.\n"
        ...
        json.dumps(
            {
                ...
                "editorial_changes": [
                    "shortened cold open to an 8.2s hook",
                    "removed global 30-minute time stretch",
                    "used natural-speed ElevenLabs chunks",
                    "increased visual turnover via denser image lists",
                    "switched caption display in composition to a lighter lower subtitle band",
                ],
            },
            indent=2,
            ensure_ascii=False,
...
def update_manifest() -> None:
    ...
    active.update({"roughcut": "v006", "audio_mix": "v004_editorial_v006", "captions": "v004_editorial_v006", "onecoin_captions": "v006"})
    manifest["state"] = "editorial_v006_audio_ready"
    manifest["updated_at"] = now()
    manifest_path.write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    with EVENTS.open("a", encoding="utf-8", newline="\n") as fh:
        fh.write(json.dumps({"ts": now(), "episode_id": EP, "stage": "edit", "event": "editorial_v006_audio_timeline_built", "actor": "codex", "external_api_calls": False, "note": "Shortened hook, natural-speed narration, lighter captions. No upload/publish/schedule."}, ensure_ascii=False) + "\n")
