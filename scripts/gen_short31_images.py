...
OUT = r"H:/pd-media/assets/ai/shorts/short31"
...
SHOTS = [
    ("short31_01", "A night car-window point of view with a single glowing smartphone held up in a faceless silhouetted hand, cold blue screen-light the only glow in the dark, no face, no logos, no readable text"),
    ("short31_02", "A modern smartphone standing upright reimagined as a heavy steel bank-safe, a bright ring of light where the lock screen would be, deep navy void around it, no brand marks, no readable text"),
    ("short31_03", "A glowing padlock of pure light hovering over a phone-shaped dark slab while a translucent scroll of light dissolves beside it, permission to search is not the power to open, no seals, no readable document text, no logos"),
    ("short31_04", "A cool blue wireframe of a human head in profile with a small glowing combination lock suspended inside it, a secret held only in the mind, no facial detail, no likeness, no readable text"),
    ("short31_05", "An anonymous gloved hand pressing a featureless fingertip onto a phone's glowing sensor, a ring of light blooming open at the touch, the body opening the lock without thought, no faces, no logos, no readable screen"),
    ("short31_06", "Two identical courthouse doorways side by side under a cold sky, one swinging open in warm gold light and the other sealed shut in blue, a single hairline fracture of light running down the seam between them, no seals, no signage, no readable text"),
    ("short31_07", "A single glowing locked smartphone centered in a dark frame with a bright key of light hovering beside it but never turned, generous negative space around it, no face, no logo, no readable text"),
    ("short31_thumb", "A locked glowing smartphone split by a hairline crack of light with a bright combination-lock symbol over the screen, deep navy and electric blue, iconic key visual, no faces, no legible text"),
]
...
def gen(stem, core, force, no_hr):
    ...
    if os.path.exists(out_path) and not force:
        ...
        return
    seed = 310000 + int(stem.split("_")[1].replace("thumb", "99"))
    payload = {
...
def main():
    ...
    print("[all ] short31 image generation finished", flush=True)
