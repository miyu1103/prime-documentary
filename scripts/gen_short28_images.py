...
Output : H:/pd-media/assets/ai/shorts/short28/short28_01.png .. _07.png + short28_thumb.png
...
OUT = r"H:/pd-media/assets/ai/shorts/short28"
...
SHOTS = [
    ("short28_01", "A modest brick row-house front door at night, sealed and untouchable, washed by cold electric-blue institutional light with the warm home dark behind it, no faces, no notice, no readable text"),
    ("short28_02", "A painter's ladder and paint roller leaning against a modest two-story suburban home in soft warm daylight, an earned lived-in house, no face, no signage, no readable brand"),
    ("short28_03", "An extreme macro of two anonymous twenty-dollar bills passing between gloved hands on a dark rain-slick street, faint red-blue light on brick, no drugs shown, no faces, no readable serial text"),
    ("short28_04", "A cold empty courtroom of vacant benches with one hard shaft of light falling on a single bare table where no one sits, bureaucratic and faceless, no bench seal, no text"),
    ("short28_05", "A single modest house lifted into a stark cold shaft of light as if placed in a dock on trial, isolated in deep navy, the building itself as the accused, no faces, no text"),
    ("short28_06", "Under cold institutional light, dense stacks of seized cash and rows of car keys and a distant impound lot of silent cars, vast and machine-like, dwarfing one small lonely banknote, no faces, no logos, no readable text"),
    ("short28_07", "An ordinary house key turning in a warm front door at golden hour as cold blue light lifts and daylight returns, generous negative space above, no faces, no text"),
    ("short28_thumb", "A modest home held in a cold shaft of light behind a sealed front door, deep navy and cold blue with restrained gold warmth, iconic key visual, no faces, no legible text"),
]
...
def gen(stem, core, force, no_hr):
    ...
    if os.path.exists(out_path) and not force:
        ...
        return
    seed = 280000 + int(stem.split("_")[1].replace("thumb", "99"))
    payload = {
...
def main():
    ...
    print("[all ] short28 image generation finished", flush=True)
