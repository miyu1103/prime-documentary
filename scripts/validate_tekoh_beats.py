#!/usr/bin/env python
"""Validate EP44 AE card beats against Remotion figures."""
...
def evaluate() -> dict:
    ...
    source = ROOT / "scripts" / "ae" / "build_tekoh_hero_cards.py"
    if source.exists() and DISCLOSURE not in source.read_text(encoding="utf-8", errors="ignore"):
        errors.append("build_tekoh_hero_cards.py lacks AI disclosure layer text")
...
def main() -> int:
    ...
    result = evaluate()
    print(("PASS" if result["ok"] else "FAIL") + f" validate_tekoh_beats: {result['beats']}")
