#!/usr/bin/env python3
"""EP46 tlo: composite the rendered After-Effects hero cards onto the finished film
...
overwrites the input. Cloned verbatim from the verified composite_cleveland_hero.py (only the
BEATS path changes to the EP46 deck).

Usage: python scripts/composite_tlo_hero.py <film_in.mp4> <out_v001_ae.mp4>
"""
...
BEATS = ROOT / "episodes" / "PD-2026-046-tlo" / "08_edit" / "ae_hero" / "beats.json"
...
def main() -> int:
    if len(sys.argv) != 3:
        print("Usage: composite_tlo_hero.py <film_in.mp4> <out_v001_ae.mp4>", file=sys.stderr); return 2
