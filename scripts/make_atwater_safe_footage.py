#!/usr/bin/env python
"""Build EP47 ATWATER factory/motion/overlay footage the same SDXL-free way EP46 TLO shipped.

Clone of make_tlo_safe_footage.py (the generator that produced the shipped, working
EP46 TLO footage: F###__tlo_safe_broll_*.mp4 @96f/30fps, M##_rife.mp4 @164f/48fps,
O##__tlo_green_dust_light_*.mp4). Same ffmpeg zoompan / procedural-overlay math,
retargeted to atwater sources, and additionally populates the episode asset_manifest
factory[]/motion[]/overlay[] arrays in the schema build_atwater_film.py expects.
"""
...
def make_overlay() -> list[dict[str, Any]]:
    ...
    for idx in range(N_OVERLAY):
        ...
        seed = idx + 11
        # warm amber-neutral dust to fit an Atwater roadside/arrest theme
        red = 40 + idx * 5
        vf = (
            f"nullsrc=s=1920x1080:r=30:d=6,"
            f"geq=r='{red}+40*random({seed})':"
            f"g='{28 + idx * 3}+22*random({seed + 23})':"
            f"b='{10 + idx}+16*random({seed + 47})',"
            f"drawbox=x='{(idx * 137) % 1800}+12*t':y={60 + idx * 37}:w={70 + idx * 3}:h=960:"
            f"color=0x8F6A3F@0.23:t=fill,"
        ...
        rows.append({
            ...
            "license": "generated_owned",
            "sha256": sha256_file(out),
            "observed_content": "abstract warm dust/light/noise overlay; no people, no readable text, no watermark",
        })
    return rows
...
def _manifest_factory_item(r: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "qc": {
            ...
            "notes": "Generated from already-reviewed atwater Codex still source; same pipeline as shipped EP46 TLO.",
        },
    }
...
def write_manifest(factory_rows, motion_rows, overlay_rows) -> None:
    ...
    data["footage_generator"] = "make_atwater_safe_footage.py (SDXL-free ffmpeg, cloned from shipped EP46 make_tlo_safe_footage.py)"
...
def write_selection_json(factory_rows, overlay_rows) -> None:
    ...
    FACTORY_SELECTION.write_text(json.dumps({
        "episode_id": EP,
        "generated_at": now,
        "selection_policy": "SDXL-free safe b-roll generated locally from Codex episode stills; identical pipeline to shipped EP46 TLO",
    ...
    FACTORY_QC.write_text(json.dumps({
        ...
        "clips": [{
            ...
            "notes": "Generated from already reviewed atwater Codex still source; contact sheet still required before final acceptance.",
...
def main() -> int:
    ...
    args = ap.parse_args()
    if not args.run:
        print("Use --run to generate EP47 atwater factory/motion/overlay footage + populate manifest.")
        return 0
