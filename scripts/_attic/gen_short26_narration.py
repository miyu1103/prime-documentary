...
LINES: list[tuple[str, str, str]] = [
    ("L1", "intense", "The FBI recorded every word he said — and never opened the door. The Court still called it an illegal search."),
    ("L2", "building", "Los Angeles, 1965. Charles Katz stepped into a glass phone booth to place illegal betting calls. To catch him, agents taped a hidden microphone to the outside of the glass, and never set foot inside."),
    ("L3", "building", "For nearly forty years, the rule was simple: no physical trespass, no search. But Katz had shut the door and paid the toll — to keep his words private, not his face."),
    ("L4", "intense", "December eighteenth, 1967 — seven to one. The Fourth Amendment protects people, not places. Recording him without a warrant was unconstitutional — not a ban on wiretaps, just a judge's permission first. Justice Harlan's concurrence gave the reasonable-expectation-of-privacy test that still governs surveillance today."),
...
def main() -> int:
    ...
    master = media_root() / "episodes" / EP / "06_voice" / "master" / "short26_vc_master_en_us_v002.mp3"
    master.parent.mkdir(parents=True, exist_ok=True)
    subprocess.run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(concat),
                    "-c:a", "libmp3lame", "-b:a", "192k", str(master)], check=True, capture_output=True)

    idx = ROOT / "episodes" / EP / "06_audio" / "short26_narration_index.v002.en_us.json"
