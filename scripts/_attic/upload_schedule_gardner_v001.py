#!/usr/bin/env python3
"""Upload EP20 Gardner privately and schedule public release."""
...
RESULT = PKG / f"youtube_schedule_result.{REV}.json"
IN_PROGRESS = PKG / f"youtube_upload_in_progress.{REV}.json"
STATUS_VERIFY = PKG / f"youtube_status_verify.{REV}.json"
CAPTION_RESULT = PKG / f"youtube_captions_result.{REV}.json"
THUMB_RESULT = PKG / f"youtube_thumbnail_result.{REV}.json"
SCHEDULED_AT_LOCAL = "2026-07-05T12:00:00+09:00"
SCHEDULED_AT_UTC = "2026-07-05T03:00:00Z"
...
def upload_caption(token: str, video_id: str, captions: Path) -> dict:
    boundary = f"gardner_caption_{int(time.time())}"
...
def update_records(result: dict, caption_result: dict | None, caption_error: str | None, state: dict) -> None:
    ...
    if MANIFEST.exists():
        ...
        manifest.setdefault("active_revisions", {}).update(
            {"youtube_meta": REV, "final_delivery": REV, "rights_manifest": REV, "youtube_schedule_result": REV}
        )
...
def main(argv: list[str]) -> int:
    ...
    print("OK Gardner v001 approved for exact package")
