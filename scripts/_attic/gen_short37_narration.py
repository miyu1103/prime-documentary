...
# (line_id, delivery, spoken_text) — Mapp v. Ohio (1961). Grade-A public facts only.
# WORDING LOCKS (accuracy — do not dramatize):
#   - Police forced into Dollree Mapp's home claiming a warrant (they waved a paper; no valid warrant was
#     produced). Before this ruling, illegally seized evidence could still be used to convict in state courts.
#   - 1961 Supreme Court, 6-3: the exclusionary rule applies to the states — evidence from an illegal search
#     is inadmissible in any state court. Second person ("you") for stakes; name Mapp only for the case; no
#     real-person likeness (the woman shown is a silhouette, not a portrait).
LINES: list[tuple[str, str, str]] = [
    ("L1", "intense", "Police force their way into your home with a warrant that doesn't exist — and search everything you own."),
    ("L2", "building", "For most of American history, evidence taken in an illegal search could still be used to convict you. The search being illegal simply didn't matter."),
    ("L3", "building", "Then officers pushed into Dollree Mapp's home, waving a piece of paper they claimed was a warrant, and searched her entire house."),
    ("L4", "intense", "In nineteen sixty-one, the Supreme Court drew the line. Evidence from an illegal search can't be used against you — in any state. A search that breaks the rules now protects you."),
...
def main() -> int:
    ...
    master = media_root() / "episodes" / EP / "06_voice" / "master" / "short37_vc_master_en_us_v002.mp3"
    master.parent.mkdir(parents=True, exist_ok=True)
    subprocess.run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(concat),
                    "-c:a", "libmp3lame", "-b:a", "192k", str(master)], check=True, capture_output=True)

    idx = ROOT / "episodes" / EP / "06_audio" / "short37_narration_index.v002.en_us.json"
