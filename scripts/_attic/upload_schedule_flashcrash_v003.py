#!/usr/bin/env python3
"""Upload Flash Crash v003 privately and schedule public release."""
...
RESULT = PKG / "youtube_schedule_result.v003.json"
IN_PROGRESS = PKG / "youtube_upload_in_progress.v003.json"
STATUS_VERIFY = PKG / "youtube_status_verify.v003.json"
CAPTION_RESULT = PKG / "youtube_captions_result.v003.json"
THUMB_RESULT = PKG / "youtube_thumbnail_result.v003.json"
SCHEDULED_AT_LOCAL = "2026-07-03T12:00:00+09:00"
SCHEDULED_AT_UTC = "2026-07-03T03:00:00Z"
APPROVAL_IDS = ["APR-0002", "APR-0003", "APR-0004"]
...
def verify_approvals(meta: dict) -> None:
...
def upload_caption(token: str, video_id: str, captions: Path) -> dict:
    boundary = f"flashcrash_caption_{int(time.time())}"
...
def verify_preconditions() -> tuple[dict, Path, Path, Path]:
    ...
    for path in (META, FINAL_DELIVERY, RIGHTS):
        require_file(path)
    meta = load_json(META)
    ...
    require_hash(RIGHTS, delivery["rights_manifest_sha256"], "rights_manifest")
    ...
    if "Review gates remain closed" in meta.get("description", ""):
...
def update_records(result: dict, caption_result: dict | None, caption_error: str | None, state: dict) -> None:
    ...
    append_event({"ts": now, "episode_id": EP, "stage": "youtube_schedule", "event": "youtube_upload_scheduled", "revision": REV, "actor": "codex", "approval_ids": APPROVAL_IDS, "video_id": video_id, "watch": result["watch"], "studio": result["studio"], "scheduled_at_local": SCHEDULED_AT_LOCAL, "scheduled_at_utc": SCHEDULED_AT_UTC, "privacy": "private", "thumbnail_set": True, "captions_uploaded": caption_result is not None, "caption_error": caption_error})
...
def main(argv: list[str]) -> int:
    ...
    print("OK FlashCrash v003 approved for exact package")
    ...
    result = {"schema_version": "1.0.0", "episode_id": EP, "revision": REV, "approval_ids": APPROVAL_IDS, "video_id": video_id, "watch": f"https://youtu.be/{video_id}", "studio": f"https://studio.youtube.com/video/{video_id}/edit", "channel_id": channel_id, "privacy": privacy, "scheduled_at_local": SCHEDULED_AT_LOCAL, "scheduled_at_utc": SCHEDULED_AT_UTC, "publish_at_platform": publish_at, "video_file": str(video).replace("\\", "/"), "video_sha256": sha(video), "thumbnail_file": rel(thumb), "thumbnail_sha256": sha(thumb), "thumbnail_set": True, "caption_file": rel(captions), "captions_uploaded": caption_result is not None, "caption_error": caption_error, "youtube_meta": rel(META), "final_delivery": rel(FINAL_DELIVERY), "uploaded_at": datetime.now(timezone.utc).isoformat(), "madeForKids": False, "containsSyntheticMedia": True, "external_upload": True, "public_schedule_set": True, "public_immediate_publish": False, "youtube_state_after": state}
