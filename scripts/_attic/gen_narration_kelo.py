#!/usr/bin/env python3
"""Generate PD-2026-010 Kelo narration with the channel voice.

Paid ElevenLabs API call. Owner explicitly authorized EP10 narration in this
Codex thread. Idempotent: existing non-empty chunks with matching keys are
skipped.
"""
...
APPROVAL_ID = "CHAT-EP10-NONSTOP-OWNER"
...
def delivery_for(section: str) -> str:
    ...
    if "hook" in lowered or "act iii" in lowered:
        return "intense"
    if "opening" in lowered or "act i" in lowered or "act ii" in lowered:
        return "building"
    return "calm"
...
def write_index(chunks: list[dict[str, str]], index: list[dict[str, str | float]], master: Path, est: float, chars: int) -> None:
