#!/usr/bin/env python3
"""Finalize Riley v005 YouTube upload: processing, captions, replacement status."""
...
META = PKG / "youtube_meta.v005.json"
SCHEDULE_RESULT = PKG / "youtube_schedule_result.v005.json"
CAPTION_RESULT = PKG / "youtube_captions_result.v005.json"
STATUS_VERIFY = PKG / "youtube_rebuild_status_verify.v005.json"
CAPTION_FILE = EPDIR / "08_edit" / "captions.final.v005.srt"
MANIFEST = EPDIR / "manifest.json"
EVENTS = EPDIR / "events" / "events.jsonl"
OLD_VIDEO_IDS = ["ODCh4-UelI4", "6iWtKdktOGY"]
PAUSED_V3_VIDEO_ID = "u17UoII_hz8"
PAUSED_V4_VIDEO_ID = "UGUAqVqTtRQ"
...
def update_local_records(video_id: str, processing_state: dict, processing_status: str, caption_result: dict, verify: dict) -> None:
    ...
    meta["captions_burned_in"] = True
    meta["old_uploads_verified_private_unscheduled"] = verify["old_uploads_verified_private_unscheduled"]
    meta["paused_v3_verified_private_unscheduled"] = verify["paused_v3_verified_private_unscheduled"]
    meta["paused_v4_verified_private_unscheduled"] = verify["paused_v4_verified_private_unscheduled"]
    meta["v5_verified_private_scheduled"] = verify["v5_verified_private_scheduled"]
    ...
    manifest.setdefault("active_revisions", {})["youtube_captions_result"] = "v005"
    manifest.setdefault("active_revisions", {})["youtube_status_verify"] = "v005"
    if "APR-0007" not in manifest.setdefault("approvals", []):
        manifest["approvals"].append("APR-0007")
    warning = f"Riley v005 YouTube upload {video_id} has burned-in captions, sidecar captions uploaded, and is private/scheduled for 2026-06-22T12:00:00+09:00. Riley v004 {PAUSED_V4_VIDEO_ID} and v003 {PAUSED_V3_VIDEO_ID} verified private and unscheduled."
    if warning not in manifest.setdefault("warnings", []):
        manifest["warnings"].append(warning)
    update_artifact_checksum(manifest, f"{EP}-youtube-captions-result-v005", CAPTION_RESULT, "youtube_captions_result", "v005")
    update_artifact_checksum(manifest, f"{EP}-youtube-status-verify-v005", STATUS_VERIFY, "youtube_status_verify", "v005")
    ...
    append_event({
        ...
        "paused_v3_verified_private_unscheduled": verify["paused_v3_verified_private_unscheduled"],
        "paused_v4_verified_private_unscheduled": verify["paused_v4_verified_private_unscheduled"],
        "v5_verified_private_scheduled": verify["v5_verified_private_scheduled"],
        "ts": now,
    })
...
def main() -> int:
    ...
    all_state = get_videos(token, OLD_VIDEO_IDS + [PAUSED_V3_VIDEO_ID, PAUSED_V4_VIDEO_ID, video_id])
    compact = compact_video_state(all_state.get("items") or [])
    old_states = [item for item in compact if item["id"] in OLD_VIDEO_IDS]
    paused_v3_states = [item for item in compact if item["id"] == PAUSED_V3_VIDEO_ID]
    paused_v4_states = [item for item in compact if item["id"] == PAUSED_V4_VIDEO_ID]
    v5_states = [item for item in compact if item["id"] == video_id]
    old_ok = bool(old_states) and all(item.get("privacyStatus") == "private" and item.get("publishAt") is None for item in old_states)
    paused_v3_ok = bool(paused_v3_states) and paused_v3_states[0].get("privacyStatus") == "private" and paused_v3_states[0].get("publishAt") is None
    paused_v4_ok = bool(paused_v4_states) and paused_v4_states[0].get("privacyStatus") == "private" and paused_v4_states[0].get("publishAt") is None
    v5_ok = bool(v5_states) and v5_states[0].get("privacyStatus") == "private" and v5_states[0].get("publishAt") == "2026-06-22T03:00:00Z"
    ...
    verify = {
        ...
        "old_uploads_status_note": "True only when v001/v002 are returned by videos.list and private/unscheduled; prior API rechecks may omit them.",
        "paused_v3_verified_private_unscheduled": paused_v3_ok,
        "paused_v4_verified_private_unscheduled": paused_v4_ok,
        "v5_verified_private_scheduled": v5_ok,
