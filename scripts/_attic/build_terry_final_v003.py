#!/usr/bin/env python3
"""Build PD-2026-006 Terry v003 local remake.

Uses approved narration. Rebuilds visuals from 180 v002 symbolic plates
with corrected narration-aligned scene timing and standard bookends.
...
VISUAL = EPM / "08_edit" / "terry_visual_v003.mp4"
OUT_MEDIA = EPM / "08_edit" / "terry_final_v003.mp4"
NARR_MASTER = EPM / "06_voice" / "master" / "vc_master_v001.mp3"
NARR_SLOW = EPM / "06_voice" / "master" / "vc_master_v001_slowed_672s_for_v003.mp3"
NARR_INDEX = EPDIR / "06_audio" / "narration_index.v001.json"
CAPTIONS = EPDIR / "08_edit" / "captions.final.v003.srt"
QC_REPO = EPDIR / "08_edit" / "renders" / "final.v003.qc.json"
CONTACT_SHEET = EPDIR / "08_edit" / "renders" / "final.v003.contact_sheet.jpg"
RIGHTS = EPDIR / "09_package" / "rights_manifest.v003.json"
DELIVERY = EPDIR / "09_package" / "final_delivery.v003.json"
...
def write_captions() -> None:
    ...
    for text, item in zip(chunks, index):
        ...
        for part, weight in zip(parts, weights):
            ...
            if part_end <= cursor + 0.12:
                part_end = min(end, cursor + 0.12)
            if part_end <= cursor:
                continue
...
def render_visual() -> None:
    ...
    seq_dir = VISUAL.parent / "terry_visual_v003_sequence"
    ...
    def render_range(start: int, end: int) -> None:
        tmp_dir = seq_dir.parent / f"terry_visual_v003_sequence_tmp_{start:05d}_{end:05d}"
        if tmp_dir.exists():
...
def write_rights(generated: list[Path]) -> None:
    ...
    for i, path in enumerate(generated, start=1):
        ...
        assets.append({
            "asset_id": f"PD-2026-006-terry-V003-IMG-{i:03d}",
            "type": "image",
            "scene": path.parent.name,
            "description": "AI-generated local SDXL symbolic reconstruction plate for EP6 Terry v003; no real-person likeness intended; not authentic footage.",
    ...
    for i, path in enumerate(audio_sources, start=1):
        if not path.exists():
            continue
        assets.append({
            "asset_id": f"PD-2026-006-terry-V003-AUD-{i:03d}",
            "type": "audio_source",
            "scene": "all",
            "description": f"Audio source used in v003 mix: {path.name}",
            "file": str(path),
    ...
    assets.append({
        "asset_id": "PD-2026-006-terry-V003-RENDER-001",
        "type": "render",
        "scene": "all",
        "description": "Local v003 remake render with narration-aligned scenes, standard bookends, and 180 AI symbolic reconstruction plates; not uploaded or published.",
        "file": f"artifact://episodes/{EP}/08_edit/terry_final_v003.mp4",
    ...
    RIGHTS.write_text(json.dumps({
        ...
        "notes": "All v003 visuals are symbolic reconstruction; YouTube synthetic-content disclosure required before any publish/update. No upload performed.",
        "assets": assets,
...
def write_package_and_qc(narr_dur: float, generated: list[Path]) -> None:
    ...
    q = {
        ...
        "episode_id": EP,
        "revision": "v003",
        "render": f"artifact://episodes/{EP}/08_edit/terry_final_v003.mp4",
        ...
        "video": "Remotion TerryPremium v003, narration-aligned structure, standard bookends, clear hook animation, 180 local SDXL symbolic hero plates, crossfades, pan/zoom, light motion",
        "audio": {
        ...
        "captions": f"artifact://episodes/{EP}/08_edit/captions.final.v003.srt",
        "contact_sheet": f"artifact://episodes/{EP}/08_edit/renders/final.v003.contact_sheet.jpg",
    ...
    DELIVERY.write_text(json.dumps({
        ...
        "rights_manifest": f"artifact://episodes/{EP}/09_package/rights_manifest.v003.json",
        "qc_report": f"artifact://episodes/{EP}/08_edit/renders/final.v003.qc.json",
    ...
    EVENTS.open("a", encoding="utf-8").write(json.dumps({
        ...
        "event": "final_v003_timing_structure_fixed",
        "episode_id": EP,
...
def main() -> int:
    ...
    write_captions()
    render_visual()
    with tempfile.TemporaryDirectory(prefix="pd_terry_v003_") as td:
        tmp = Path(td)
