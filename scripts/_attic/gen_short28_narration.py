...
LINES: list[tuple[str, str, str]] = [
    ("L1", "intense", "A court ordered a family's home seized — not because they broke the law, but because their house supposedly did."),
    ("L2", "building", "Christos Sourovelis painted houses and earned a modest home in northeast Philadelphia. In March 2014, his twenty-two-year-old son sold about forty dollars of heroin to a police informant."),
    ("L3", "building", "So the city moved — not against the son, but against the house. Using civil forfeiture, prosecutors won an order to seize and seal the home with no family member present. The property itself becomes the defendant — the State versus one house — and innocent owners must prove their own home did nothing wrong."),
    ("L4", "intense", "And it wasn't unusual. From 2002 to 2014, Philadelphia forfeiture took more than twelve hundred homes and over fifty million dollars in cash — the typical seizure just a hundred seventy-eight dollars — with millions a year flowing into police and prosecutor budgets. The family and the Institute for Justice filed a class action, and in 2018 the city agreed to dismantle the machine."),
...
def main() -> int:
    ...
    master = media_root() / "episodes" / EP / "06_voice" / "master" / "short28_vc_master_en_us_v002.mp3"
    master.parent.mkdir(parents=True, exist_ok=True)
    subprocess.run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(concat),
                    "-c:a", "libmp3lame", "-b:a", "192k", str(master)], check=True, capture_output=True)

    idx = ROOT / "episodes" / EP / "06_audio" / "short28_narration_index.v002.en_us.json"
