#!/usr/bin/env python3
"""Generate SHORT #22 (Michael Milken / "98 charges, 6 pleas") vertical-short narration with the channel voice.
...
R3 / sensitive: living, pardoned. WORDING LOCKS (do not deviate):
  - "pleaded guilty to six," NEVER "convicted" / "convicted of insider trading / racketeering / at trial."
  - Keep CHARGED (98, incl. RICO) distinct from PLEADED (6); the case never went to trial.
  - ~$600M = $200M fine + $400M restitution (not a single "fine").
  - Pardon = clemency, NOT innocence; the guilty plea STANDS; the SEC lifetime ban was NOT lifted.
  - Genius-vs-greed stays attributed. Re-confirm he is alive before publish.
...
# (line_id, delivery, spoken_text) — from SHORTS_EP19-24.md SHORT #22 time table, with R3 wording locks applied.
LINES: list[tuple[str, str, str]] = [
    ("L1", "intense", "He was charged with ninety-eight counts — and pleaded guilty to six."),
    ("L2", "building", "Michael Milken built the high-yield, or junk, bond market, financing companies the big banks shut out. In 1987 his pay was reported near five hundred fifty million dollars — called the largest single-year pay in America."),
    ("L3", "building", "After a cooperating witness pointed investigators his way, a 1989 indictment charged him with ninety-eight counts, including RICO racketeering. But those were accusations — the case never went to trial."),
    ("L4", "intense", "In 1990 he pleaded guilty to six felony counts; the RICO charge and the other ninety-two were dropped. He paid about six hundred million dollars and was barred from the industry for life. In 2020 a presidential pardon forgave the punishment — but it did not erase the guilty plea, and did not lift that lifetime ban."),
...
def main() -> int:
    ...
    master = media_root() / "episodes" / EP / "06_voice" / "master" / "short22_vc_master_en_us_v002.mp3"
    master.parent.mkdir(parents=True, exist_ok=True)
    subprocess.run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(concat),
                    "-c:a", "libmp3lame", "-b:a", "192k", str(master)], check=True, capture_output=True)

    idx = ROOT / "episodes" / EP / "06_audio" / "short22_narration_index.v002.en_us.json"
