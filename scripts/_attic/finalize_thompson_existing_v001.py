#!/usr/bin/env python3
"""Finalize the ALREADY-UPLOADED EP41 thompson video (OkwHIpI7DkE).

Same failure pattern as EP40 lech: the packaging agent uploaded the private
video successfully (title matches, categoryId 27, en) but died before setting
the thumbnail, scheduling publishAt, and uploading the caption. Re-running
schedule_thompson.py would create a DUPLICATE upload. This finalizer targets
the existing id only: set thumbnail -> schedule -> caption -> write result.

    py -3.11 scripts/finalize_thompson_existing_v001.py
"""
...
from schedule_thompson import (
    set_thumbnail, schedule_publish, upload_caption, get_channel_id,
    EP, PKG, THUMB_FILE, CAPTION_FILE, PUBLISH_AT, CHANNEL_ALLOWLIST,
    EXPECTED_HASH, EPDIR,
)

VIDEO_ID = "OkwHIpI7DkE"
...
def main():
    ...
    result = {"schema_version": "1.0.0", "episode_id": EP, "mode": "scheduled",
        ...
        "note": "finalized existing upload; packaging agent uploaded then died before scheduling"}
