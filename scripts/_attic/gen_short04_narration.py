#!/usr/bin/env python3
"""Generate SHORT #4 (FTX) vertical-short narration with the channel voice.

US-market English VO. Voice = channel voice, model = multilingual v2. Paid ElevenLabs (pre-authorized).
R3 / sensitive: living person convicted of fraud (2023). Neutral, ad-safe, non-defamatory: the guilty
verdict is stated as fact and attributed to a jury; the founder is never named and never depicted.
Idempotent: existing non-empty chunks skipped.
...
# (line_id, delivery, spoken_text) — US-market English adaptation of SHORTS_EP1-8.md SHORT #4.
# Causation/defamation safe: jury verdict is fact; founder unnamed, never depicted.
LINES: list[tuple[str, str, str]] = [
    ("L1", "intense", "A crypto exchange was holding billions of its customers' dollars. Then about eight billion of it went missing."),
    ("L2", "building", "FTX was one of the biggest crypto exchanges in the world, and its founder was its most trusted face."),
    ("L3", "building", "But hidden in the code was a secret exception. A private trading firm could pull customer deposits almost without limit."),
    ("L4", "intense", "When everyone asked for their money back, it wasn't there. In 2023, a jury found the founder guilty in one of the largest frauds in U.S. history. Even in crypto, spending customers' money is theft."),
...
def main() -> int:
    ...
    master = media_root() / "episodes" / EP / "06_voice" / "master" / "short04_vc_master_en_us_v002.mp3"
    master.parent.mkdir(parents=True, exist_ok=True)
    subprocess.run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(concat),
                    "-c:a", "libmp3lame", "-b:a", "192k", str(master)], check=True, capture_output=True)

    idx = ROOT / "episodes" / EP / "06_audio" / "short04_narration_index.v002.en_us.json"
