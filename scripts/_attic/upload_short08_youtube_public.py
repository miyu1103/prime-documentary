#!/usr/bin/env python3
"""Publish SHORT #8 Carpenter to YouTube Shorts (US English).

Separate from the long-form Carpenter upload scripts. Only touches the short08
coverfirst render, short08 thumbnail, and short08 package result files.
The published video bakes the thumbnail into a 0.7s cover frame (YouTube Shorts
does not reliably surface an API-set custom image as the Shorts thumbnail).
"""
...
VIDEO = ROOT / "remotion" / "out" / "short08_yt_coverfirst.mp4"
THUMB = ROOT / "remotion" / "out" / "short08_thumb.png"
PUBLISH_MD = EPDIR / "short08_publish.md"
RESULT = PKG / "short08_youtube_publish_result.v001.json"
STATUS_VERIFY = PKG / "short08_youtube_status_verify.v001.json"
EVENTS = EPDIR / "events" / "events.jsonl"

EXPECTED_VIDEO = "6ec9e33b2abfeb9e81580cec2962745f024ff493113c259ccc17e1f2f7d4f7e5"
EXPECTED_THUMB = "66c42b2a74005cb6daaa0e19b1afa2437bdfd1b1ff6ea036c4b26e7d1a723625"

TITLE = "Police Tracked His Phone for 127 Days — No Warrant? #Shorts"
DESCRIPTION = """Your phone quietly logs where you go. Police wanted 127 days of one man's location history — without a warrant.
...
def verify_preconditions() -> None:
    if RESULT.exists():
        raise RuntimeError(f"Existing short08 YouTube publish result found; refusing duplicate upload: {RESULT.relative_to(ROOT)}")
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        print(f"WARN thumbnail set failed HTTP {exc.code}; cover frame is baked in, video remains published")
    ...
    result = {
        ...
        "public_publish": True,
        "owner_instruction": "今日は投稿して。サムネはクリック率最大の判断で君が選定。",
        "published_at": now,
    ...
    append_event(
        {
            "event": "short08_youtube_published",
            "episode_id": EP,
            "short_id": SHORT_ID,
            "stage": "published",
            "actor": "claude",
