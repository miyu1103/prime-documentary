...
SCRIPT_SRC = ROOT / "episodes" / "_planning" / "EP44_tekoh_script.en.v001.md"
SOURCE_SCRIPT_REL = "episodes/_planning/EP44_tekoh_script.en.v001.md"
...
DELIVERY_BY_SECTION = {
    "HOOK": "intense",    # the gun on the table, the packed bag, no warrant no crime
    "OP": "building",     # thesis: can police enter your home just to look after you
    "ACT_1": "building",  # the night: the argument, "shoot me", the hotel
    "ACT_2": "building",  # the welfare check: phone -> porch -> ambulance -> seizure
    "ACT_3": "building",  # doctrinal core: Cady=car, 9-0, headline trap, vacate&remand
    "ENDING": "calm",     # payoff + earned CTA (open door at dawn)
}
...
# match wins). EP43 uses numbered `## 1. HOOK` / `## 2. OPENING` / `## 4. ENDING` and the
# three acts are h3 `### ACT 1 ...`. `## 3. BODY` is a container -> matches nothing here.
SECTION_HEADINGS = [
    ("HOOK", re.compile(r"^\d+\.\s*HOOK\b", re.IGNORECASE)),
    ("OP", re.compile(r"^\d+\.\s*OPENING\b", re.IGNORECASE)),
    # tekoh acts are h2 numbered containers `## N. BODY — ACT 1: "..."` (NOT h3 `### ACT 1`),
    # so accept either the bare `ACT n` form or the `N. BODY ... ACT n` numbered-container form.
    ("ACT_1", re.compile(r"(?:ACT\s*1\b|\d+\.\s*BODY.*ACT\s*1\b)", re.IGNORECASE)),
    ("ACT_2", re.compile(r"(?:ACT\s*2\b|\d+\.\s*BODY.*ACT\s*2\b)", re.IGNORECASE)),
    ("ACT_3", re.compile(r"(?:ACT\s*3\b|\d+\.\s*BODY.*ACT\s*3\b)", re.IGNORECASE)),
    ("ENDING", re.compile(r"^\d+\.\s*ENDING\b", re.IGNORECASE)),
]
# any heading that ends the narration body (appendix / non-VO)
STOP_HEADINGS = [
    re.compile(r"^事実対応表"),
    re.compile(r"^改稿ログ"),
]
...
def assert_clean(chunks: list[dict]) -> None:
    ...
    # EP43 forbidden-language guard (mirror of check_caniglia_facts R-FORBID). ONLY the
    # declarative overreach forms -- deliberately NOT the negation-context near-phrases the
    # verbatim script legitimately contains ("can never enter your home", "Not a wall around
    # your home"). Adding those would false-FAIL on the locked script text.
    forbid = [
        "full victory", "total victory", "complete victory", "total win", "won his case",
        "won outright", "case closed", "case is over", "case ended in his favor", "final win",
        "warrantless entry is illegal", "home is absolutely protected",
        "police can no longer enter", "no more welfare checks",
    ]
    for c in chunks:
        low = c["spoken_text"].lower()
        for f in forbid:
            if f in low:
                bad.append(f"{c['chunk_id']}: FORBIDDEN phrase '{f}' -> {c['spoken_text'][:80]}")
    if bad:
...
def main(argv: list[str]) -> int:
    ...
    print(f"projected @190wpm = {words / 190.0 * 60:.0f}s speech (target ~721.3s; band 690-750 incl gaps)")
    ...
    print(f"MASTER measured = {total:.1f}s ({total / 60:.2f}min)  target ~721.3s  band 690-750 -> "
          f"{'PASS' if 690 <= total <= 750 else 'OUT OF BAND'}")
    print(f"measured wpm = {words / (speech / 60):.1f} (expected ~176-190)")
