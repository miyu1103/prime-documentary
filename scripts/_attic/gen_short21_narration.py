#!/usr/bin/env python3
"""Generate SHORT #21 (D.B. Cooper / "he vanished") vertical-short narration with the channel voice.
...
R2 / sensitive: unsolved hijacking. WORDING LOCKS (do not deviate):
  - UNSOLVED; never assert anyone "was D.B. Cooper"; the FBI never confirmed any suspect.
  - "Dan Cooper" is the alias he ticketed under; "D.B." was a press error.
  - The bomb was a CLAIM, not a confirmed device.
  - 2016 = FBI SUSPENDED the case, not solved. Keep jump time/place broad.
...
# (line_id, delivery, spoken_text) — from SHORTS_EP19-24.md SHORT #21 time table, with R2 wording locks applied.
LINES: list[tuple[str, str, str]] = [
    ("L1", "intense", "He jumped out of a plane with two hundred thousand dollars — and vanished forever."),
    ("L2", "building", "November 24, 1971. A calm man in a dark suit, ticketed as Dan Cooper, hijacked a flight from Portland to Seattle with a note claiming he had a bomb."),
    ("L3", "building", "He demanded two hundred thousand dollars and four parachutes, let the passengers go, then ordered the crew to fly low toward Mexico City. A little after eight p.m., he lowered the plane's rear stairs and parachuted into a freezing night over Washington."),
    ("L4", "intense", "The FBI chased it for forty-five years — the only unsolved hijacking in U.S. aviation history. In 1980 a boy found about fifty-eight hundred dollars of the cash by a river, and the serial numbers matched. It's the only money ever found. In 2016 the FBI suspended the case — suspended, not solved."),
...
def main() -> int:
    ...
    master = media_root() / "episodes" / EP / "06_voice" / "master" / "short21_vc_master_en_us_v002.mp3"
    master.parent.mkdir(parents=True, exist_ok=True)
    subprocess.run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(concat),
                    "-c:a", "libmp3lame", "-b:a", "192k", str(master)], check=True, capture_output=True)

    idx = ROOT / "episodes" / EP / "06_audio" / "short21_narration_index.v002.en_us.json"
