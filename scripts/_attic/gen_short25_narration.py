...
LINES: list[tuple[str, str, str]] = [
    ("L1", "intense", "He never knocked. He never crossed the lawn. The Supreme Court still called it a search of the home."),
    ("L2", "building", "On a freezing night in January 1992, a federal agent sat in a car on a public street in Florence, Oregon. He pointed a thermal-imaging device at Danny Kyllo's home and read the heat leaking off the walls."),
    ("L3", "building", "The garage roof and one wall glowed hot — to the agent, the sign of grow lamps. That helped win a warrant, and inside were over a hundred marijuana plants. But the device never saw through the wall. It only read heat already drifting into open air."),
    ("L4", "intense", "On June 11, 2001, the Court split five to four. Justice Scalia wrote that using a device not in general public use to learn what's inside a home is a search that needs a warrant. The dissent called it just reading heat off the outside. But the majority drew a line at the front door it called firm, and bright."),
...
def main() -> int:
    ...
    master = media_root() / "episodes" / EP / "06_voice" / "master" / "short25_vc_master_en_us_v002.mp3"
    master.parent.mkdir(parents=True, exist_ok=True)
    subprocess.run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(concat),
                    "-c:a", "libmp3lame", "-b:a", "192k", str(master)], check=True, capture_output=True)

    idx = ROOT / "episodes" / EP / "06_audio" / "short25_narration_index.v002.en_us.json"
