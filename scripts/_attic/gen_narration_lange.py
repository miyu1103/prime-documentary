#!/usr/bin/env python3
"""Generate PD-2026-014 Lange narration with the channel voice.

Paid ElevenLabs API call. Idempotent: existing non-empty chunks are skipped.
Also writes Remotion caption/timing data from generated chunk durations.
...
CAPTIONS_TS = ROOT / "remotion" / "src" / "data" / "lange_captions.ts"
TIMING_TS = ROOT / "remotion" / "src" / "data" / "lange_audio_timing.ts"
PUBLIC_AUDIO = ROOT / "remotion" / "public" / "lange" / "audio" / "vc_master_v001.mp3"
...
def parse_script() -> list[dict[str, str]]:
    ...
    for raw in SCRIPT.read_text("utf-8").splitlines():
        ...
        if line.startswith("## "):
            ...
            continue
        if current is None or line.startswith("#") or "[VO:]" not in line:
            continue
...
def wrap_words(words: list[str], max_words: int = 9) -> list[list[str]]:
    lines: list[list[str]] = []
...
def write_captions(index: list[dict[str, str | float]]) -> None:
    captions = []
    cap_no = 1
    for chunk in index:
        text = str(chunk["spoken_text"])
        words = re.findall(r"[A-Za-z0-9]+(?:['-][A-Za-z0-9]+)?|[^\s]", text)
        lines = wrap_words(words, 9)
        start = float(chunk["start"])
        end = float(chunk["end"])
        total_words = max(1, sum(len(line) for line in lines))
        cursor = start
        for line in lines:
            span = max(1.2, (end - start) * (len(line) / total_words))
            cap_end = min(end, cursor + span)
            caption_text = " ".join(line).replace(" ,", ",").replace(" .", ".").replace(" ?", "?").replace(" :", ":").replace(" ;", ";")
            captions.append({"id": f"CAP-{cap_no:04d}", "start": round(cursor, 3), "end": round(cap_end, 3), "text": caption_text})
            cap_no += 1
            cursor = cap_end
    CAPTIONS_TS.write_text(
        "export type LangeCaption = {id: string; start: number; end: number; text: string};\n"
        "export const LANGE_CAPTIONS: LangeCaption[] = "
...
def write_timing(master: Path) -> None:
    seconds = duration(master)
    TIMING_TS.write_text(
        f"export const LANGE_BODY_AUDIO_SEC = {seconds:.3f};\n",
        "utf-8",
    )
...
def generate(chunks: list[dict[str, str]], budget_usd: float) -> int:
    ...
    if failed:
        print(f"ERROR: failed={failed}; master not rebuilt")
        return 1
        ...
        json.dumps(
            {
                ...
                "generated_total_seconds": round(total, 2),
                "master": f"artifact://episodes/{EP}/06_voice/master/vc_master_v001.mp3",
                "public_remotion_audio": "artifact://remotion/public/lange/audio/vc_master_v001.mp3",
                "chunks": index,
            },
            indent=2,
            ensure_ascii=False,
    ...
    shutil.copy2(master, PUBLIC_AUDIO)
    write_captions(index)
    write_timing(master)
    print(f"made={made} skipped={skipped} failed={failed}")
    print(f"master -> {master} ({total:.1f}s)")
    print(f"public -> {PUBLIC_AUDIO.relative_to(ROOT)}")
    print(f"index -> {NARR_INDEX.relative_to(ROOT)}")
    print(f"captions -> {CAPTIONS_TS.relative_to(ROOT)}")
    return 0
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument("--generate", action="store_true", help="make paid ElevenLabs calls")
