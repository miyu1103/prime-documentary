#!/usr/bin/env python3
"""Generate SHORT #3 (Mapp v. Ohio) vertical-short narration with the channel voice.

US-market English VO. Voice = channel voice, model = multilingual v2. Paid ElevenLabs (pre-authorized).
R1 / not sensitive (1961 case). No real-person likeness. Idempotent: existing non-empty chunks skipped.
...
# (line_id, delivery, spoken_text) — US-market English adaptation of SHORTS_EP1-8.md SHORT #3.
LINES: list[tuple[str, str, str]] = [
    ("L1", "intense", "Police search a home illegally and find something. Can they still use it against you?"),
    ("L2", "building", "In 1957, officers forced their way into a woman's house without a valid warrant."),
    ("L3", "building", "The suspect they were after wasn't there. Instead, they found some books, and charged her with a different crime."),
    ("L4", "intense", "In 1961, the Supreme Court ruled: evidence from an illegal search can't be used, in any state. Sometimes a guilty person goes free, because the police broke the rules."),
...
def main() -> int:
    ...
    master = media_root() / "episodes" / EP / "06_voice" / "master" / "short03_vc_master_en_us_v002.mp3"
    master.parent.mkdir(parents=True, exist_ok=True)
    subprocess.run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(concat),
                    "-c:a", "libmp3lame", "-b:a", "192k", str(master)], check=True, capture_output=True)

    idx = ROOT / "episodes" / EP / "06_audio" / "short03_narration_index.v002.en_us.json"
