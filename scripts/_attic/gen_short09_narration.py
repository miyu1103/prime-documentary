#!/usr/bin/env python3
"""Generate SHORT #9 (Timbs v. Indiana) vertical-short narration with the channel voice.

US-market English VO. Voice = same channel voice as the episodes, model = multilingual v2.
...
# (line_id, delivery, spoken_text)  — US-market adaptation of SHORTS_EP9-15.md SHORT #9 (Timbs).
LINES: list[tuple[str, str, str]] = [
    ("L1", "intense", "Police can take your car, your cash, even your house, without ever convicting you of a crime."),
    ("L2", "building", "An Indiana man bought a forty-two-thousand-dollar SUV with the life-insurance money his father left him."),
    ("L3", "building", "After a small drug sale, with no prison time, the state seized the SUV. That's about four times the most he could be fined."),
    ("L4", "intense", "In 2019, the Supreme Court ruled nine to nothing: the ban on excessive fines applies to the states too. He got his car back, but the power to take your property still stands."),
...
def main() -> int:
    ...
    master = media_root() / "episodes" / EP / "06_voice" / "master" / "short09_vc_master_en_us_v002.mp3"
    master.parent.mkdir(parents=True, exist_ok=True)
    subprocess.run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(concat),
                    "-c:a", "libmp3lame", "-b:a", "192k", str(master)], check=True, capture_output=True)

    idx = ROOT / "episodes" / EP / "06_audio" / "short09_narration_index.v002.en_us.json"
