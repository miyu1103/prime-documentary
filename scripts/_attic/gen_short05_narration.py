#!/usr/bin/env python3
"""Generate SHORT #5 (Madoff) vertical-short narration with the channel voice.

US-market English VO. Voice = channel voice, model = multilingual v2. Paid ElevenLabs (pre-authorized).
R3 / sensitive: real Ponzi-scheme operator (deceased 2021). Neutral, ad-safe, non-defamatory: the
collapse and sentence are public fact; he is never named and never depicted. Idempotent.
...
# (line_id, delivery, spoken_text) — US-market English adaptation of SHORTS_EP1-8.md SHORT #5.
# Neutral: collapse and 150-year sentence are fact; operator unnamed, never depicted.
LINES: list[tuple[str, str, str]] = [
    ("L1", "intense", "For decades, he paid steady profits. In reality, he was barely investing the money at all."),
    ("L2", "building", "His was one of the most trusted names on Wall Street, with smooth, steady returns year after year."),
    ("L3", "building", "But investigators found there were almost no real trades."),
    ("L4", "intense", "The profits were just new investors' money handed to old ones. It collapsed in 2008, and he was sentenced to a hundred and fifty years. Returns that look too smooth can be the warning sign."),
...
def main() -> int:
    ...
    master = media_root() / "episodes" / EP / "06_voice" / "master" / "short05_vc_master_en_us_v002.mp3"
    master.parent.mkdir(parents=True, exist_ok=True)
    subprocess.run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(concat),
                    "-c:a", "libmp3lame", "-b:a", "192k", str(master)], check=True, capture_output=True)

    idx = ROOT / "episodes" / EP / "06_audio" / "short05_narration_index.v002.en_us.json"
