#!/usr/bin/env python3
"""Generate SHORT #18 (Flash Crash / Sarao) vertical-short narration with the channel voice.

US-market English VO. Voice = channel voice, model = multilingual v2. Paid ElevenLabs (pre-authorized).
R3 / sensitive: living person who pleaded guilty. CAUSATION LOCK — only "part of the imbalance /
not guilty of causing the crash / no one man alone"; NEVER "caused / single-handedly / broke the market."
Guilty plea (wire fraud + spoofing) stated as fact. No real-person likeness; no nameable firm.
...
# (line_id, delivery, spoken_text) — US-market adaptation of SHORTS_EP16-18.md SHORT #18.
# Causation lock: never says he caused the crash; guilty plea (spoofing + wire fraud) is fact.
LINES: list[tuple[str, str, str]] = [
    ("L1", "intense", "In thirty-six minutes, almost a trillion dollars vanished from the U.S. stock market. Then it came back."),
    ("L2", "building", "May 6, 2010. The market fell nearly a thousand points in minutes, and famous shares briefly traded for a penny."),
    ("L3", "building", "Investigators traced part of the imbalance to a trader in a London bedroom, flashing giant orders he meant to cancel. The law calls it spoofing."),
    ("L4", "intense", "He pleaded guilty to spoofing and wire fraud, but not to causing the crash. No account blames one man alone: a four-billion-dollar sell order and panicking machines were part of it too."),
...
def main() -> int:
    ...
    master = media_root() / "episodes" / EP / "06_voice" / "master" / "short18_vc_master_en_us_v002.mp3"
    master.parent.mkdir(parents=True, exist_ok=True)
    subprocess.run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(concat),
                    "-c:a", "libmp3lame", "-b:a", "192k", str(master)], check=True, capture_output=True)

    idx = ROOT / "episodes" / EP / "06_audio" / "short18_narration_index.v002.en_us.json"
