...
LINES: list[tuple[str, str, str]] = [
    ("L1", "intense", "For thirty years, Alabama planned to execute Anthony Ray Hinton — for two murders he did not commit."),
    ("L2", "building", "The whole case rested on one claim: that bullets from the crime scenes matched an old .38 revolver from his mother's home. No eyewitness. No fingerprints. No confession. Only that match."),
    ("L3", "building", "To fight it he needed a real forensic expert. But his lawyer wrongly believed the state capped that money at a thousand dollars. There was no cap. The expert he could afford was blind in one eye and couldn't work the microscope. A jury convicted him in 1986 — in about an hour."),
    ("L4", "intense", "For nearly thirty years on death row, he kept his mind free — a book club, imagined travels. Then Bryan Stevenson's Equal Justice Initiative proved the bullets matched nothing, and in 2014 a unanimous Supreme Court threw out his conviction. He walked free in 2015 — the 152nd person cleared from death row."),
...
def main() -> int:
    ...
    master = media_root() / "episodes" / EP / "06_voice" / "master" / "short29_vc_master_en_us_v002.mp3"
    master.parent.mkdir(parents=True, exist_ok=True)
    subprocess.run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(concat),
                    "-c:a", "libmp3lame", "-b:a", "192k", str(master)], check=True, capture_output=True)

    idx = ROOT / "episodes" / EP / "06_audio" / "short29_narration_index.v002.en_us.json"
