#!/usr/bin/env python3
"""Generate SHORT #10 (Kelo v. City of New London) vertical-short narration with the channel voice.

US-market English VO. Voice = same channel voice as the episodes, model = multilingual v2.
Paid ElevenLabs call; authorized by SHORTS_REMOTION_SPEC/VIDEO_RULES.
Idempotent: existing non-empty chunks are skipped. Platform-neutral spoken CTA.
"""
...
# (line_id, delivery, spoken_text)  — US-market adaptation of SHORTS_EP9-15.md SHORT #10 (Kelo).
LINES: list[tuple[str, str, str]] = [
    ("L1", "intense", "The government can take your home, in perfect condition, and hand the land to a private developer."),
    ("L2", "building", "In 2005, in New London, Connecticut, one woman's little pink house was perfectly fine. She didn't want to sell."),
    ("L3", "building", "But the city wanted offices and a hotel, for jobs and taxes. The Constitution allows taking property for public use. Was this?"),
    ("L4", "intense", "In 2005, the Supreme Court said yes, five to four. Her house was torn down. But the project was never built. The land sat empty for years."),
...
def main() -> int:
    ...
    master = media_root() / "episodes" / EP / "06_voice" / "master" / "short10_vc_master_en_us_v002.mp3"
    master.parent.mkdir(parents=True, exist_ok=True)
    subprocess.run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(concat),
                    "-c:a", "libmp3lame", "-b:a", "192k", str(master)], check=True, capture_output=True)

    idx = ROOT / "episodes" / EP / "06_audio" / "short10_narration_index.v002.en_us.json"
