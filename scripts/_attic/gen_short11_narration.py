#!/usr/bin/env python3
"""Generate SHORT #11 (Mahanoy Area SD v. B.L.) vertical-short narration with the channel voice.
...
# (line_id, delivery, spoken_text)  — US-market adaptation of SHORTS_EP9-15.md SHORT #11 (Mahanoy).
LINES: list[tuple[str, str, str]] = [
    ("L1", "intense", "A post you made on the weekend, from home. Can your school punish you for it?"),
    ("L2", "building", "A fourteen-year-old didn't make the varsity cheer team, and vented on Snapchat, a post meant to disappear."),
    ("L3", "building", "But disappearing is a myth. A screenshot reached the coach, and the school suspended her from cheer for a year."),
    ("L4", "intense", "In 2021, the Supreme Court ruled eight to one: schools usually can't punish ordinary off-campus speech. Threats and bullying are different. The line between home and school was redrawn."),
...
def main() -> int:
    ...
    master = media_root() / "episodes" / EP / "06_voice" / "master" / "short11_vc_master_en_us_v002.mp3"
    master.parent.mkdir(parents=True, exist_ok=True)
    subprocess.run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(concat),
                    "-c:a", "libmp3lame", "-b:a", "192k", str(master)], check=True, capture_output=True)

    idx = ROOT / "episodes" / EP / "06_audio" / "short11_narration_index.v002.en_us.json"
