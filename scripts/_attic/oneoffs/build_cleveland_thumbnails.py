#!/usr/bin/env python3
r"""EP45 Cleveland CTR thumbnails v001 -- clones the winning Caniglia/Frazier v003 text treatment.

The episode walks the rule of Bearden v. Georgia, 461 U.S. 660 (1983): a court may NOT jail a person
simply because they are too poor to pay a fine or fee without first finding the failure to pay was
willful and considering alternatives. Jailing someone solely for inability to pay a fine -- a modern
"debtors' prison" -- runs against that rule. Packaging never calls the practice "legal"; it frames it
as the constitutional line Bearden drew in 1983. R2: no face, no poverty-porn, no identifiable person.
...
Backgrounds (episode's own SDXL 4K photoreal stills, E:/pd-media/assets/ai/cleveland):
  ...
  option 2  S01 -- a thick stack of unpaid citations, edges printed in overdue crimson, bound with a
                   rubber band on a warm working-class kitchen table (the small debt that grew).
  option 3  S06 -- the pale marble columns of the U.S. Supreme Court (the court that drew the line in 1983).

R2: a door / a stack of paper / a building -- no faces, no people, no distress imagery.

    py -3.11 scripts/build_cleveland_thumbnails.py
"""
...
LANE = (178, 58, 72)     # #B23A48 -- EP45 cleveland lane color (crimson)
WHITE = (247, 249, 252)
RED = (206, 44, 44)
CLAMP_LUMA = 188.0
...
def main():
    ...
    specs = [
        ("S04.png", 1.38, 1.08, "JAILED FOR", "BEING POOR", "DEBTORS' PRISON", LANE, (0.50, 0.45)),
        ("S01.png", 1.62, 1.10, "TOO POOR", "FOR FREEDOM", "A FINE SHE COULDN'T PAY", LANE, (0.50, 0.42)),
        ("S06.png", 1.45, 1.08, "BEING POOR", "ISN'T A CRIME", "SUPREME COURT · 1983", LANE, (0.50, 0.50)),
    ]
    paths = []
