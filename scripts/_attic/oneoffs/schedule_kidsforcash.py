#!/usr/bin/env python3
"""Schedule EP38 kidsforcash to YouTube (private, auto-publish at publishAt).
...
upload English caption. Writes 09_package/youtube_schedule_result.v001.json.
    ...
    py -3.11 scripts/schedule_kidsforcash.py --dry-run
    py -3.11 scripts/schedule_kidsforcash.py
"""
...
VIDEO_FILE = EPDIR / "08_edit" / "kidsforcash_final_bgm.v004_ae.mp4"
THUMB_FILE = PKG / "thumbnail.selected.v002.png"
CAPTION_FILE = EPDIR / "08_edit" / "captions.final.v001.srt"
EXPECTED_HASH = "d83b2023b91212ae2bf41601b629783ab054fe036bb33f0ef7262a688e4c2e78"
PUBLISH_AT = "2026-07-23T03:00:00Z"
...
def upload_caption(token, video_id):
    boundary = f"kfc_caption_{int(time.time())}"
...
def main(argv):
    ...
    if apr.get("video_sha256", "").replace("sha256:", "") != EXPECTED_HASH:
    ...
    if args.dry_run:
        ...
        print(f"[dry-run] would upload {VIDEO_FILE.name}, set thumb, schedule publishAt={PUBLISH_AT}, add caption.")
        return 0
    ...
    for attempt in range(3):
        try:
            ...
            cap_err = str(e)[:150]; time.sleep(15)  # captions may need processing
    if not cap_ok:
    ...
    result = {"schema_version": "1.0.0", "episode_id": EP, "mode": "scheduled",
        ...
        "scheduled_at_local": "2026-07-23T12:00:00+09:00", "title": meta["title"],
        "video_sha256": "sha256:" + EXPECTED_HASH, "thumbnail_set": thumb_ok,
        "captions_uploaded": cap_ok, "caption_error": cap_err,
        "approval_ref": "APR-0001", "owner_instruction": apr.get("owner_instruction"),
