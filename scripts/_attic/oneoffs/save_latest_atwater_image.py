...
def normalize_asset_id(asset_id: str) -> str:
    ...
    if re.fullmatch(r"m\d{2}_src", asset_id, flags=re.IGNORECASE):
        return asset_id[:3].upper() + "_src"
    raise SystemExit(f"bad asset id: {asset_id}")
...
def main() -> None:
    ...
    parser.add_argument("asset_id", help="Still id, e.g. S08 or M01_src")
    parser.add_argument("--overwrite", action="store_true")
    args = parser.parse_args()

    asset_id = normalize_asset_id(args.asset_id)
