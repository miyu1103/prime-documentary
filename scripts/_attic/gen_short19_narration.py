#!/usr/bin/env python3
"""Generate SHORT #19 (Varsity Blues / "the side door") vertical-short narration with the channel voice.

US-market English VO. Voice = channel voice, model = multilingual v2. Paid ElevenLabs (pre-authorized).
Source: episodes/_planning/SHORTS_EP19-24.md SHORT #19 (VO text used verbatim — wording is locked).

R3 / sensitive: living people who pleaded guilty. WORDING LOCKS (do not deviate):
  - "pleaded guilty," NEVER "convicted at trial."
  - Loughlin/Giannulli pleaded guilty to FRAUD CONSPIRACY only (money-laundering & bribery counts were
    DISMISSED) — never "convicted of money laundering/bribery."
  - Schools = victims, not suspects. Do not impute knowledge to any child.
  - Aggregate: 55 charged / 53 convicted, almost all by guilty plea (distinct from 50 initial arrests).
...
# (line_id, delivery, spoken_text) — verbatim from SHORTS_EP19-24.md SHORT #19 time table. Wording locked.
LINES: list[tuple[str, str, str]] = [
    ("L1", "intense", "Wealthy parents paid to sneak their kids into top colleges — through a secret side door."),
    ("L2", "building", "In March 2019, prosecutors unsealed Operation Varsity Blues and arrested fifty people. The mastermind, Rick Singer, ran what prosecutors called a sham charity."),
    ("L3", "building", "Parents paid about twenty-five million dollars for fake athletic spots — kids listed as recruits for sports they never played, with staged photos. Others paid a man to secretly take the SAT, or fix the answers."),
    ("L4", "intense", "Felicity Huffman paid to fix her daughter's test, and pleaded guilty. Lori Loughlin paid five hundred thousand for fake crew spots, and pleaded guilty to fraud conspiracy. The schools were the victims, not the suspects. Of the fifty-five charged, almost all pleaded guilty rather than fight at trial."),
...
def main() -> int:
    ...
    master = media_root() / "episodes" / EP / "06_voice" / "master" / "short19_vc_master_en_us_v002.mp3"
    master.parent.mkdir(parents=True, exist_ok=True)
    subprocess.run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(concat),
                    "-c:a", "libmp3lame", "-b:a", "192k", str(master)], check=True, capture_output=True)

    idx = ROOT / "episodes" / EP / "06_audio" / "short19_narration_index.v002.en_us.json"
