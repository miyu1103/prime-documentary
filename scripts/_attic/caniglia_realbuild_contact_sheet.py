#!/usr/bin/env python
"""Decode 8 real source frames spread across the caniglia_film cut list into a contact sheet so the
...
FILM = json.loads((ROOT / "remotion/src/data/caniglia_film.json").read_text(encoding="utf-8"))
OUT = ROOT / "runs" / "qc" / "caniglia_realbuild_frames.png"
OUT.parent.mkdir(parents=True, exist_ok=True)

cuts = FILM["cuts"]
idxs = [round(i * (len(cuts) - 1) / 7) for i in range(8)]
picks = [cuts[i] for i in idxs]
# guarantee a mix: at least 2 stills + 2 footage in the 8
if sum(1 for c in picks if c["kind"] == "img") < 2:
    for j, c in enumerate(cuts):
        if c["kind"] == "img" and c not in picks:
            picks[-1] = c; break
if sum(1 for c in picks if c["kind"] == "footage") < 2:
    for c in cuts:
        if c["kind"] == "footage" and c not in picks:
            picks[0] = c; break
