...
SCRIPT_SRC = ROOT / "episodes" / "_planning" / "EP50_centralpark_script.en.v001.md"
SOURCE_SCRIPT_REL = "episodes/_planning/EP50_centralpark_script.en.v001.md"
...
DELIVERY_BY_SECTION = {
    "HOOK": "intense",    # cold open: five children, a camera switched off, a confession
    "ACT_1": "building",  # the night in the park, the vacuum, the hinge (no evidence)
    "ACT_2": "building",  # the interrogations -- the moral engine
    "ACT_3": "building",  # the trials: press panic, thin evidence, the tapes convict
    "ACT_4": "building",  # the lost years; Korey Wise in the adult system
    "ACT_5": "building",  # Matias Reyes, the confession, the DNA match
    "ACT_6": "building",  # exoneration, settlement, the reform, the woman, the reckoning
    "ACT_7": "calm",      # ENDING: what a confession is worth (put you back in the room)
}
SECTION_ORDER = ["HOOK", "ACT_1", "ACT_2", "ACT_3", "ACT_4", "ACT_5", "ACT_6", "ACT_7"]
# heading text (leading #'s stripped) regex -> canonical section (order matters; first
# match wins). EP50 headings are `## [HOOK — ...]`, `## [ACT 1 — ...]` ... `## [ACT 7 — ...]`
# (SPACE not underscore, trailing timing annotations). `## [OPENING TITLE — 3.5s]`,
# `## VOICE & GUARDRAILS`, `### DO NOT STATE AS FACT` are NOT mapped -> section=None -> drop.
SECTION_HEADINGS = [
    ...
    ("ACT_4", re.compile(r"^\[ACT\s+4\b", re.IGNORECASE)),
    ("ACT_5", re.compile(r"^\[ACT\s+5\b", re.IGNORECASE)),
    ("ACT_6", re.compile(r"^\[ACT\s+6\b", re.IGNORECASE)),
    ("ACT_7", re.compile(r"^\[ACT\s+7\b", re.IGNORECASE)),
]
# any heading / line that ends the narration body (appendix / non-VO)
STOP_HEADINGS = [
