#!/usr/bin/env python3
"""Finalize Riley v004 YouTube upload: processing, captions, replacement status."""
...
SCHEDULE_RESULT = PKG / "youtube_schedule_result.v004.json"
CAPTION_RESULT = PKG / "youtube_captions_result.v004.json"
STATUS_VERIFY = PKG / "youtube_rebuild_status_verify.v004.json"
...
def update_local_records(video_id: str, processing_state: dict, processing_status: str, caption_result: dict, verify: dict) -> None:
    ...
    meta["v4_verified_private_scheduled"] = verify["v4_verified_private_scheduled"]
    ...
    manifest.setdefault("active_revisions", {})["youtube_captions_result"] = "v004"
    manifest.setdefault("active_revisions", {})["youtube_status_verify"] = "v004"
    if "APR-0006" not in manifest.setdefault("approvals", []):
        manifest["approvals"].append("APR-0006")
    warning = f"Riley v004 YouTube upload {video_id} has burned-in captions, sidecar captions uploaded, and is private/scheduled for 2026-06-22T12:00:00+09:00. Riley v003 {PAUSED_V3_VIDEO_ID} verified private and unscheduled."
    if warning not in manifest.setdefault("warnings", []):
        manifest["warnings"].append(warning)
    update_artifact_checksum(manifest, f"{EP}-youtube-captions-result-v004", CAPTION_RESULT, "youtube_captions_result", "v004")
    update_artifact_checksum(manifest, f"{EP}-youtube-status-verify-v004", STATUS_VERIFY, "youtube_status_verify", "v004")
    ...
    append_event({
        ...
        "v4_verified_private_scheduled": verify["v4_verified_private_scheduled"],
        "ts": now,
    })
...
def main() -> int:
    ...
    all_state = get_videos(token, OLD_VIDEO_IDS + [PAUSED_V3_VIDEO_ID, video_id])
    ...
    v4_states = [item for item in compact if item["id"] == video_id]
    ...
    v4_ok = bool(v4_states) and v4_states[0].get("privacyStatus") == "private" and v4_states[0].get("publishAt") == "2026-06-22T03:00:00Z"
    ...
    verify = {
        ...
        "v4_verified_private_scheduled": v4_ok,
