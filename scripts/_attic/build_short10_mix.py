#!/usr/bin/env python3
"""Build SHORT #10 audio mix + caption timing from the English narration index.

Same structure as build_short09_mix.py (Kelo): caption split, 4-layer ducked mix at -14 LUFS,
bed/tension/ambience bases high enough that the 2.70s inter-line gaps don't sag.
"""
...
INDEX = ROOT / "episodes" / EP / "06_audio" / "short10_narration_index.v002.en_us.json"
LIB = MEDIA / "library"
OUT_AUDIO = MEDIA / "episodes" / EP / "07_audio"
REM_PUBLIC = ROOT / "remotion" / "public" / "shorts" / "short10" / "audio"
...
def write_timing(lines: list[dict], caps: list[dict], total: float) -> None:
    ...
    out = REM_DATA / "short10_timing.ts"
    text = "// AUTO-GENERATED by scripts/build_short10_mix.py — narration-driven timing for SHORT #10.\n"
    text += "import type {ShortCaption} from '../compositions/Short';\n\n"
    text += "export type LineWindow = {id: string; start: number; end: number};\n\n"
    text += f"export const SHORT10_TOTAL_SEC = {round(total, 3)};\n\n"
    text += "export const LINE_WINDOWS: LineWindow[] = " + json.dumps(windows, ensure_ascii=False, indent=2) + ";\n\n"
    text += "export const SHORT10_CAPTIONS: ShortCaption[] = " + json.dumps(caps, ensure_ascii=False, indent=2) + ";\n"
...
def build_mix(narration: Path, total: float, lines: list[dict]) -> Path:
    ...
    sfx = [
        ...
        (LIB / "sfx" / "sfx_page_turn.mp3", lines[1]["start"] + 0.2, 0.24),
    ...
    out = OUT_AUDIO / "short10_final_mix_v002_en_us.mp3"
