#!/usr/bin/env python3
"""Generate SHORT #15 (Theranos / when a bold promise becomes a crime) vertical-short narration.

US-market English VO. Voice = channel voice, model = multilingual v2. Paid ElevenLabs (pre-authorized).
R3 topic: neutral, factual (public-record conviction), no real-person likeness referenced. Fraud finding
is attributed to the jury, not asserted independently. Idempotent: existing non-empty chunks are skipped.
"""
...
# (line_id, delivery, spoken_text) — US-market adaptation of SHORTS_EP9-15.md SHORT #15.
# Neutral, factual; the fraud finding is attributed to the jury (public record, 2022).
LINES: list[tuple[str, str, str]] = [
    ("L1", "intense", "One drop of blood, every test, a nine-billion-dollar company. But the machine didn't work."),
    ("L2", "building", "Its founder was hailed as the next Steve Jobs. Then whistleblowers said the device didn't work as advertised."),
    ("L3", "building", "Many tests secretly ran on other machines, the finger-prick results unreliable, as investors poured in hundreds of millions."),
    ("L4", "intense", "In 2022, a jury convicted the founder of defrauding investors. Not failure, but intent: knowingly selling a lie. A bold promise becomes a crime when the deception is deliberate."),
...
def main() -> int:
    ...
    master = media_root() / "episodes" / EP / "06_voice" / "master" / "short15_vc_master_en_us_v002.mp3"
    master.parent.mkdir(parents=True, exist_ok=True)
    subprocess.run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(concat),
                    "-c:a", "libmp3lame", "-b:a", "192k", str(master)], check=True, capture_output=True)

    idx = ROOT / "episodes" / EP / "06_audio" / "short15_narration_index.v002.en_us.json"
