...
LINES: list[tuple[str, str, str]] = [
    ("L1", "intense", "The stop was already over — the warning written, the papers handed back. So how did the next seven minutes break the Constitution?"),
    ("L2", "building", "Just after midnight in Nebraska, a K-9 officer pulled over Dennys Rodriguez for drifting onto the shoulder. He ran the checks, wrote a warning, and handed everything back. The stop's mission was done."),
    ("L3", "building", "But the officer never said you're free to go. He held Rodriguez seven to eight more minutes, waited for a second unit, then walked a drug dog around the car. The dog alerted — after the stop was already finished."),
    ("L4", "intense", "In 2015, six to three, the Supreme Court drew the line. A stop becomes unlawful, Justice Ginsburg wrote, when it is prolonged beyond the time reasonably required to complete the mission. Once the traffic work is done, the clock stops — extra time for a dog needs its own reason. It didn't ban sniffs, and it didn't free him; it sent the case back."),
...
def main() -> int:
    ...
    master = media_root() / "episodes" / EP / "06_voice" / "master" / "short27_vc_master_en_us_v002.mp3"
    master.parent.mkdir(parents=True, exist_ok=True)
    subprocess.run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(concat),
                    "-c:a", "libmp3lame", "-b:a", "192k", str(master)], check=True, capture_output=True)

    idx = ROOT / "episodes" / EP / "06_audio" / "short27_narration_index.v002.en_us.json"
