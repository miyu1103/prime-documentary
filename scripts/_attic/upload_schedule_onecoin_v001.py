#!/usr/bin/env python3
"""Upload PD-2026-017 OneCoin v011 privately and schedule public release."""
...
SCHEDULED_AT_LOCAL = "2026-07-02T12:00:00+09:00"
SCHEDULED_AT_UTC = "2026-07-02T03:00:00Z"
...
def verify_approvals(meta: dict) -> dict:
    ...
    if apr_data["APR-0004"].get("target_type") != "upload_publish_schedule":
...
def upload_caption(token: str, video_id: str, captions: Path) -> dict:
    boundary = f"onecoin_caption_{int(time.time())}"
...
def verify_preconditions() -> tuple[dict, Path, Path, Path]:
    ...
    require_hash(META, delivery["youtube_meta_sha256"], "youtube_meta")
    require_hash(FINAL_DELIVERY, load_json(APPROVALS / "APR-0004.json")["final_delivery_sha256"], "final_delivery")
    require_hash(RIGHTS, load_json(APPROVALS / "APR-0004.json")["rights_manifest_sha256"], "rights_manifest")
    ...
    if "Review gates remain closed" in meta.get("description", "") or "No upload/schedule has been performed" in meta.get("description", ""):
        raise RuntimeError("youtube_meta description still contains internal gate text")
    fact_text = resolve_path(meta["fact_recheck"]).read_text(encoding="utf-8")
    if "Status: PASS_WITH_OWNER_EDITORIAL_OVERRIDE" not in fact_text:
        raise RuntimeError("fact re-check is not marked PASS_WITH_OWNER_EDITORIAL_OVERRIDE")
    return meta, video, thumb, captions


def update_artifact(manifest: dict, artifact_id: str, path: Path, artifact_type: str, revision: str, status: str = "scheduled") -> None:
    checksum = sha(path)
    uri = f"artifact://{path.relative_to(ROOT).as_posix()}" if path.is_relative_to(ROOT) else str(path).replace("\\", "/")
...
def update_records(result: dict, caption_result: dict | None, caption_error: str | None, state: dict) -> None:
    ...
    meta.update(
        {
            ...
            "captions_result": rel(CAPTION_RESULT),
    ...
    delivery["external_side_effects"] = {"upload": True, "publish": False, "schedule": True, "paid_api": False}
    ...
    update_artifact(manifest, f"{EP}-youtube_metadata-{REV}", META, "youtube_metadata", REV)
    update_artifact(manifest, f"{EP}-final_delivery-{REV}", FINAL_DELIVERY, "final_delivery", REV)
    update_artifact(manifest, f"{EP}-youtube_schedule_result-v001", RESULT, "youtube_schedule_result", "v001")
    update_artifact(manifest, f"{EP}-youtube_status_verify-v001", STATUS_VERIFY, "youtube_status_verify", "v001")
    update_artifact(manifest, f"{EP}-youtube_thumbnail_result-v001", THUMB_RESULT, "youtube_thumbnail_result", "v001")
    update_artifact(manifest, f"{EP}-youtube_captions_result-v001", CAPTION_RESULT, "youtube_captions_result", "v001")
    ...
    append_event(
        {
            ...
            "caption_error": caption_error,
            "youtube_state": state,
            "note": "Private upload, flashy thumbnail, sidecar caption attempt, and scheduled public release completed. No immediate public publish performed.",
        }
    )
...
def main(argv: list[str]) -> int:
    ...
    meta, video, thumb, captions = verify_preconditions()
    print("OK APR-0001..APR-0004 approved for exact OneCoin v012 package")
