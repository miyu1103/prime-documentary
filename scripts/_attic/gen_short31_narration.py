...
LINES: list[tuple[str, str, str]] = [
    ("L1", "intense", "An officer at your window wants your phone open. He can take your face. He can take your thumb. But your mind may be the one lock he can't force."),
    ("L2", "building", "Your phone isn't a phone — it's your whole life. In 2014, Riley versus California said police generally need a warrant to search it. But a warrant is worthless if they can't get in — so the real fight is whether they can force you to open it."),
    ("L3", "building", "That jumps to the Fifth Amendment, which shields the contents of your mind. A passcode lives in your head, so many courts say it may be protected. But your face and thumb are on your body — no thought required — and the law often treats them like a booking fingerprint."),
    ("L4", "intense", "So in 2024, one federal court let officers press a man's own thumb to unlock. A year later, another called almost the same act unconstitutional. States split on passcodes too — and the Supreme Court has refused to settle it. Your right can change at a state line."),
...
def main() -> int:
    ...
    master = media_root() / "episodes" / EP / "06_voice" / "master" / "short31_vc_master_en_us_v002.mp3"
    master.parent.mkdir(parents=True, exist_ok=True)
    subprocess.run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(concat),
                    "-c:a", "libmp3lame", "-b:a", "192k", str(master)], check=True, capture_output=True)

    idx = ROOT / "episodes" / EP / "06_audio" / "short31_narration_index.v002.en_us.json"
