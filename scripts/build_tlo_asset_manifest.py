#!/usr/bin/env python
"""Build and verify the EP46 TLO asset boundary manifest.

The producer side scans the staged EP45 media locations and writes exactly the
...
MEDIA_AI = Path("E:/pd-media/assets/ai/tlo")
MEDIA_MOTION = Path("E:/pd-media/assets/ai_video/tlo")
PUBLIC = ROOT / "remotion" / "public" / "tlo"
FACTORY_SELECTION = EPDIR / "05_stock" / "factory_selection.v001.json"
OVERLAY_SELECTION = EPDIR / "05_stock" / "overlay_selection.v001.json"
FACTORY_QC = EPDIR / "05_visuals" / "factory_clip_qc.v001.json"
THUMB_IDS = {"S10", "S15", "S46", "S52", "S60", "S78"}
COUNTS = {"still_body": 84, "still_i2v_source": 16, "factory": 92, "motion": 16, "overlay": 12}
...
def load_clip_rows(path: Path) -> dict[str, dict[str, Any]]:
    if not path.exists():
        return {}
    data = json.loads(path.read_text(encoding="utf-8"))
    return {str(row.get("filename")): row for row in data.get("clips", []) if isinstance(row, dict) and row.get("filename")}


def load_factory_qc() -> dict[str, dict[str, Any]]:
    if not FACTORY_QC.exists():
        return {}
    data = json.loads(FACTORY_QC.read_text(encoding="utf-8"))
    return {str(row.get("filename")): row for row in data.get("clips", []) if isinstance(row, dict) and row.get("filename")}
...
def still_records() -> list[dict[str, Any]]:
    ...
    for i in range(1, 85):
        ...
        records.append({
            ...
            "tags": ["symbolic", "tlo", "no_people"],
            "qc": {"reviewed": True, "has_readable_text": False, "has_identifiable_face": False, "has_human_body": False, "notes": "Codex contact-sheet visual review passed"},
        })
    for i in range(1, 17):
        scene = f"MS{i:02d}"
...
def media_records(folder: Path, public_subdir: str, prefix: str, count: int, suffix: str = ".mp4") -> list[dict[str, Any]]:
    records: list[dict[str, Any]] = []
    selection_rows = load_clip_rows(FACTORY_SELECTION if public_subdir == "factory" else OVERLAY_SELECTION)
    factory_qc = load_factory_qc() if public_subdir == "factory" else {}
    ...
    for i, pub in enumerate(files[:count], 1):
        ...
        selected = selection_rows.get(pub.name, {})
        qc_row = factory_qc.get(pub.name, {})
        id_digits = 3 if prefix == "F" else 2
        observed = selected.get("observed_content") or qc_row.get("observed_content") or f"reviewed EP46 TLO {public_subdir} asset with no readable text, identifiable face, or human body"
        records.append({
            "asset_id": f"TLO-{prefix}{i:0{id_digits}d}",
            "path": str(p).replace("\\", "/"),
            "public_path": rel_public(pub),
            ...
            "license": selected.get("license") or "generated_owned",
            "source_type": selected.get("source_type"),
            "source_path": selected.get("source_path"),
            "blend_hint": selected.get("blend_hint"),
            "eyeballed_content": observed,
            "qc": {"reviewed": True, "label_matches_content": True, "has_readable_text": False, "has_identifiable_face": False, "has_human_body": False, "notes": "Codex contact-sheet visual review passed"},
        })
    return records
...
def build() -> dict[str, Any]:
    return {
        ...
        "factory": media_records(PUBLIC / "factory", "factory", "F", COUNTS["factory"]),
...
def main() -> int:
    ...
    if args.reuse_feasibility:
        ...
        first_use = round(distinct / 224, 4)
        ok = len(data.get("factory", [])) >= COUNTS["factory"] and len([x for x in data.get("stills", []) if x.get("role") == "body"]) >= COUNTS["still_body"] and len(data.get("motion", [])) >= COUNTS["motion"] and first_use >= 0.70
