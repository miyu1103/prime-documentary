#!/usr/bin/env python3
"""Schedule EP42 young to YouTube (private, auto-publish at publishAt).
...
CONFIRM no existing young video -> resumable PRIVATE upload -> set thumbnail ->
schedule (status.publishAt) -> upload English caption -> re-query + verify.
If a PRIVATE young upload already exists (e.g. an agent died mid-upload, the EP40/EP41
...
re-uploading. Writes 09_package/youtube_schedule_result.v001.json.
    ...
    py -3.11 scripts/schedule_young.py --dry-run
    py -3.11 scripts/schedule_young.py
"""
...
VIDEO_FILE = EPDIR / "08_edit" / "young_final_bgm.v003_ae.mp4"
THUMB_FILE = PKG / "thumbnail.selected.v001.png"
CAPTION_FILE = EPDIR / "08_edit" / "captions.final.v001.srt"
EXPECTED_HASH = "d55bf28d7a38891bf5f523449bb174db3989c990f121f2ab6f118ffaaae0a392"
PUBLISH_AT = "2026-07-27T03:00:00Z"
SCHEDULED_LOCAL = "2026-07-27T12:00:00+09:00"
...
def upload_caption(token, video_id):
    boundary = f"young_caption_{int(time.time())}"
...
def is_young_match(title, want_title):
    t = title.strip().lower()
    return (title.strip() == want_title.strip()
            or "raided the wrong house" in t
            or "the law owed her" in t
            or "anjanette young" in t)
...
def main(argv):
    ...
    # Guard 3: hash (final v003_ae; APR-0001 must bind to this exact sha)
    ...
    # Pre-flight: enumerate recent uploads, detect an existing young video (double-upload guard)
    print("  scanning recent channel uploads for an existing young video ...")
    recent = list_recent_uploads(token, ch, n=25)
    want_title = meta["title"]
    existing = [(vid, title, priv) for (vid, title, priv) in recent
                if is_young_match(title, want_title)]
    for vid, title, priv in recent[:12]:
        flag = "  <-- MATCH" if (vid, title, priv) in existing else ""
        print(f"    [{priv:8}] {vid}  {title[:60]}{flag}")
    if existing:
        print(f"  NOTE: {len(existing)} possible existing young upload(s) found.")
    ...
    if args.dry_run:
        ...
        if existing:
            print("[dry-run] existing young video(s) present -> real run would FINALIZE the "
                  "private one instead of re-uploading:")
            for vid, title, priv in existing:
                print(f"          {priv} {vid} {title[:50]}")
        return 0

    # Decide: finalize an existing PRIVATE young upload, else fresh upload
    reused = False
    video_id = None
    ...
    if priv_existing:
        ...
        reused = True
        print(f"OK reusing existing PRIVATE young upload video_id={video_id} (no re-upload)")
        caps = existing_captions(token, video_id)
