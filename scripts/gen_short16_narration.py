#!/usr/bin/env python3
"""Generate SHORT #16 (Titan / OceanGate) vertical-short narration with the channel voice.

US-market English VO. Voice = channel voice, model = multilingual v2. Paid ElevenLabs (pre-authorized).
R2 / sensitive (real, deceased people; ongoing litigation): neutral, factual, elegiac, non-sensational.
The "preventable" finding and cause are attributed to the U.S. Coast Guard Board, NOT the narrator.
No likeness/voice of victims. Idempotent: existing non-empty chunks are skipped.
"""
...
# (line_id, delivery, spoken_text) — US-market adaptation of SHORTS_EP16-18.md SHORT #16.
# Elegiac, restrained. Cause/"preventable" attributed to the Coast Guard (public finding, 2025).
LINES: list[tuple[str, str, str]] = [
    ("L1", "intense", "The world watched a four-day countdown to save them. They were already gone."),
    ("L2", "building", "In June 2023, five people sealed inside an experimental sub called Titan dove toward the Titanic, two and a half miles down."),
    ("L3", "building", "They had been warned. Its safety chief was fired for flagging the hull, experts called the design catastrophic, and it was steered with a game controller."),
    ("L4", "intense", "Ninety minutes down, a last message: \"All good here.\" Then silence. The hull had imploded in an instant, that first day. In 2025, the Coast Guard called it preventable."),
...
def main() -> int:
    ...
    master = media_root() / "episodes" / EP / "06_voice" / "master" / "short16_vc_master_en_us_v002.mp3"
    master.parent.mkdir(parents=True, exist_ok=True)
    subprocess.run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(concat),
                    "-c:a", "libmp3lame", "-b:a", "192k", str(master)], check=True, capture_output=True)

    idx = ROOT / "episodes" / EP / "06_audio" / "short16_narration_index.v002.en_us.json"
