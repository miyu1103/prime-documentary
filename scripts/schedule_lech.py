#!/usr/bin/env python3
"""Schedule EP40 lech to YouTube (private, auto-publish at publishAt).

Guards (fail closed): APR-0002 approved by owner + video sha256 match + channel allowlist.
APR-0002 re-binds owner approval to the final v003_ae sha (moji-gire fix; supersedes APR-0001's v002_ae sha).
...
upload English caption. Writes 09_package/youtube_schedule_result.v001.json.
    ...
    py -3.11 scripts/schedule_lech.py --dry-run
    py -3.11 scripts/schedule_lech.py
"""
...
APR_PATH = EPDIR / "approvals" / "APR-0002.json"
VIDEO_FILE = Path("H:/pd-media/episodes/PD-2026-040-lech/08_edit/lech_final_bgm.v003_ae.mp4")
THUMB_FILE = PKG / "thumbnail.selected.v002.png"
CAPTION_FILE = EPDIR / "08_edit" / "captions.final.v001.srt"
EXPECTED_HASH = "c8d66fcee38cd9453f9fce31ec21bda28d2cd99d1c41032f64817320658d538e"
PUBLISH_AT = "2026-07-25T03:00:00Z"
SCHEDULED_LOCAL = "2026-07-25T12:00:00+09:00"
...
def upload_caption(token, video_id):
    boundary = f"lech_caption_{int(time.time())}"
...
def main(argv):
    ...
    if not APR_PATH.exists():
        print("BLOCKED: APR-0002.json not found."); return 1
    apr = json.loads(APR_PATH.read_text(encoding="utf-8"))
    if apr.get("decision") != "approved":
        print(f"BLOCKED: APR-0002 decision={apr.get('decision')!r}"); return 1
    if apr.get("decided_by") != "owner":
        print(f"BLOCKED: APR-0002 decided_by={apr.get('decided_by')!r}"); return 1
    print("OK APR-0002 approved by owner")
    # Guard 2: files
    for f in (VIDEO_FILE, THUMB_FILE, META_PATH, CAPTION_FILE):
        if not f.exists(): print(f"BLOCKED: missing {f}"); return 1
    # Guard 3: hash (final v003_ae; APR-0002 must bind to this exact sha)
    ...
    print("OK sha256 matches APR-0002")
