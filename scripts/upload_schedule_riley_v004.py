#!/usr/bin/env python3
"""Upload EP7 Riley v004 burned-caption render to YouTube and schedule public release.
...
FINAL_QC = EPDIR / "08_edit" / "renders" / "final.v004_burned_captions.qc.json"
VIDEO = Path(r"H:\pd-media\episodes\PD-2026-007-riley\08_edit\riley_final_v004_burned_captions.mp4")
MANIFEST = EPDIR / "manifest.json"
RESULT = EPDIR / "09_package" / "youtube_schedule_result.v004.json"
EVENTS = EPDIR / "events" / "events.jsonl"

EXPECTED_VIDEO = "764c928a9d90012990a1e841975605e14e8bae3bfea2e6204647b31bb61688a2"
...
def update_local_package_state(result: dict) -> None:
    ...
    meta.setdefault("pre_publish_checks", {})["supersedes_video_id"] = "u17UoII_hz8"
    ...
    manifest.setdefault("active_revisions", {})["edit"] = "v003"
    manifest.setdefault("active_revisions", {})["captions_final"] = "v003"
    manifest.setdefault("active_revisions", {})["final_qc"] = "v004"
    manifest.setdefault("active_revisions", {})["final_delivery"] = "v004"
    manifest.setdefault("active_revisions", {})["final_render"] = "v004"
    manifest.setdefault("active_revisions", {})["youtube_meta"] = "v004"
    manifest.setdefault("active_revisions", {})["youtube_schedule_result"] = "v004"
    approvals = manifest.setdefault("approvals", [])
    if APR_ID not in approvals:
        approvals.append(APR_ID)
    warning = f"YouTube private upload completed for Riley v004 burned-caption render and scheduled under {APR_ID}: video_id {result['video_id']}, public release {SCHEDULED_AT_LOCAL}. Riley v003 u17UoII_hz8 was paused because subtitles were not visible by default."
    if warning not in manifest.setdefault("warnings", []):
        manifest["warnings"].append(warning)
    update_artifact_checksum(manifest, f"{EP}-final-qc-v004", FINAL_QC, "qc_report")
    update_artifact_checksum(manifest, f"{EP}-final-delivery-v004", EPDIR / "09_package" / "final_delivery.v004.json", "final_delivery_manifest")
    update_artifact_checksum(manifest, f"{EP}-final-render-v004", VIDEO, "final_render", "v004")
    update_artifact_checksum(manifest, f"{EP}-youtube-meta-v004", META, "youtube_metadata")
    update_artifact_checksum(manifest, f"{EP}-approval-{APR_ID}", APR, "approval")
    update_artifact_checksum(manifest, f"{EP}-youtube-schedule-result-v004", RESULT, "youtube_schedule_result")
    write_json(MANIFEST, manifest)


def verify_preconditions() -> tuple[dict, dict, Path]:
    if RESULT.exists():
        raise RuntimeError(f"Existing Riley v004 schedule result found; refusing duplicate upload: {RESULT.relative_to(ROOT)}")
    if not APR.exists():
...
def main(argv: list[str]) -> int:
    ...
    append_event({
        ...
        "actor": "codex",
        "approval_id": APR_ID,
        "detail": f"Uploaded EP7 Riley v004 burned-caption render to YouTube as private video_id={video_id} and scheduled public release for {SCHEDULED_AT_LOCAL}. Selected thumbnail set. Riley v003 u17UoII_hz8 remains private and unscheduled after caption issue. No public immediate publish performed.",
        "video_id": video_id,
        "watch": f"https://youtu.be/{video_id}",
        "studio": f"https://studio.youtube.com/video/{video_id}/edit",
        "scheduled_at_local": SCHEDULED_AT_LOCAL,
