#!/usr/bin/env python3
"""Schedule EP43 caniglia to YouTube (private, auto-publish at publishAt).
...
CONFIRM no existing caniglia video -> resumable PRIVATE upload -> set thumbnail ->
schedule (status.publishAt) -> upload English caption -> re-query + verify.
If a PRIVATE caniglia upload already exists (e.g. an agent died mid-upload, the EP40/EP41
...
run; until then the sha256 guard fails closed. APR ref: APR-0001 (approvals/APR-0001.json).
    ...
    py -3.11 scripts/schedule_caniglia.py --dry-run
    py -3.11 scripts/schedule_caniglia.py
"""
...
VIDEO_FILE = EPDIR / "08_edit" / "caniglia_final_bgm.v001_ae.mp4"
THUMB_FILE = PKG / "thumbnail.selected.v001.png"
CAPTION_FILE = EPDIR / "08_edit" / "captions.final.v001.srt"
EXPECTED_HASH = "613a022289f2538e90a4f5e3f82688f11a630c82c1e9f54906d4f2f25307247c"
PUBLISH_AT = "2026-07-28T03:00:00Z"
SCHEDULED_LOCAL = "2026-07-28T12:00:00+09:00"
...
def upload_caption(token, video_id):
    boundary = f"caniglia_caption_{int(time.time())}"
...
def is_caniglia_match(title, want_title):
    t = title.strip().lower()
    return (title.strip() == want_title.strip()
            or "caniglia" in t
            or "enter your home" in t
            or "welfare check" in t)
...
def main(argv):
    ...
    # Pre-flight: enumerate recent uploads, detect an existing caniglia video (double-upload guard)
    print("  scanning recent channel uploads for an existing caniglia video ...")
    recent = list_recent_uploads(token, ch, n=25)
    want_title = meta["title"]
    existing = [(vid, title, priv) for (vid, title, priv) in recent
                if is_caniglia_match(title, want_title)]
    for vid, title, priv in recent[:12]:
        flag = "  <-- MATCH" if (vid, title, priv) in existing else ""
        print(f"    [{priv:8}] {vid}  {title[:60]}{flag}")
    if existing:
        print(f"  NOTE: {len(existing)} possible existing caniglia upload(s) found.")
    ...
    if args.dry_run:
        ...
        if existing:
            print("[dry-run] existing caniglia video(s) present -> real run would FINALIZE the "
                  "private one instead of re-uploading:")
            for vid, title, priv in existing:
                print(f"          {priv} {vid} {title[:50]}")
        return 0

    # Decide: finalize an existing PRIVATE caniglia upload, else fresh upload
    reused = False
    video_id = None
    ...
    if priv_existing:
        ...
        reused = True
        print(f"OK reusing existing PRIVATE caniglia upload video_id={video_id} (no re-upload)")
        caps = existing_captions(token, video_id)
