            ...
            rows.append(
                {
                    ...
                    # The ID LIST comes from a cached sweep; the PRIVACY must not.
                    "privacy": it["status"].get("privacyStatus"),
                }
            )

    scheduled = sorted([r for r in rows if r["publishAt"]], key=lambda r: r["publishAt"])
    # A VIDEO THAT HAS ALREADY GONE PUBLIC HAS NO publishAt ANY MORE -- YouTube clears it once it
    # fires. Reading "no publishAt" as "stuck, will never publish" therefore reports every short
    # that went live since the cached sweep as a problem. Measured 2026-08-21: 2 of 2 so-called
    # UNSCHEDULED shorts were already public with the date correctly consumed, and acting on the
    # report would have meant re-scheduling live videos. The API answer was already in hand --
    # only the field was being thrown away.
    published = [r for r in rows if not r["publishAt"] and r["privacy"] == "public"]
    stuck = [r for r in rows if not r["publishAt"] and r["privacy"] != "public"]
    for r in scheduled:
        print(f"{r['publishAt']}  {r['id']}  {r['title'][:60]}")
    print(f"\nscheduled={len(scheduled)}  already-published={len(published)}  "
          f"private-with-no-date={len(stuck)}")
    for r in published:
        print(f"  published     {r['id']}  {r['title'][:60]}")
    for r in stuck:
        print(f"  UNSCHEDULED   {r['id']}  {r['title'][:60]}   <-- will never publish on its own")
    out = ROOT / "runs" / "shorts_thumbs" / "yt_scheduled.v001.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps({"scheduled": scheduled, "published": published, "unscheduled": stuck}, ensure_ascii=False, indent=1), encoding="utf-8")
    print(f"wrote {out}")
    return 0
