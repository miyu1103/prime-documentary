#!/usr/bin/env python3
"""EP47 atwater: replace the baked narration with the authoritative master VO and add a
ducked 2-layer BGM + ambience bed, two-pass loudnorm to -14 LUFS, mux into the CaseFilm render.

Film layout = hook(8.0) + OPENING(3.5) + narration(686.512) + ENDCARD(9). Body/VO starts at
...
build_cleveland_bgm_real.py; only IDX/VO/EP paths changed (same 3-act HOOK/OP/ACT_1..3/ENDING
section layout, same music/ambience library cues).

Usage: python scripts/build_atwater_bgm_real.py <render_in.mp4> <final_out.mp4>
"""
...
IDX = ROOT / "episodes" / "PD-2026-047-atwater" / "06_audio" / "narration_index.v001.json"
VO = Path(r"E:/pd-media/episodes/PD-2026-047-atwater/06_voice/master/vc_master_v001.mp3")
...
def main() -> int:
    ...
    # EP47 atwater is a 3-act film (HOOK/OP/ACT_1/ACT_2/ACT_3/ENDING) -- no ACT_4.
    ...
    work = out.parent / "_atwater_bgm_pre.wav"
