...
LINES: list[tuple[str, str, str]] = [
    ("L1", "intense", "To catch a billionaire trader, the FBI used the same tool it used on the mob: wiretaps."),
    ("L2", "building", "Raj Rajaratnam ran Galleon, a hedge fund that peaked near seven billion dollars. His sources sat inside the companies — an Intel executive, a McKinsey partner, an IBM executive — each charged separately."),
    ("L3", "building", "It was the first big insider case built on court-authorized wiretaps. Prosecutors played the jury his own phone calls — the scheme in his own voice. On May 11, 2011, a jury convicted him on all fourteen counts."),
    ("L4", "intense", "He got eleven years — at the time, prosecutors called it the longest insider-trading sentence in U.S. history — and had to forfeit nearly fifty-four million dollars. In 2013 an appeals court upheld the wiretaps as lawful. A separate trial even convicted a Goldman Sachs director of feeding him secrets."),
...
def main() -> int:
    ...
    master = media_root() / "episodes" / EP / "06_voice" / "master" / "short24_vc_master_en_us_v002.mp3"
    master.parent.mkdir(parents=True, exist_ok=True)
    subprocess.run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(concat),
                    "-c:a", "libmp3lame", "-b:a", "192k", str(master)], check=True, capture_output=True)

    idx = ROOT / "episodes" / EP / "06_audio" / "short24_narration_index.v002.en_us.json"
