...
CONTACT = ROOT / "episodes" / EP / "08_edit" / "renders" / "final.v002.contact_sheet.jpg"
EXPECTED_DURATION = 640.5
...
def loudness(path: Path) -> dict[str, float]:
    ...
    return {
        "input_i": find(r"I:\s*(-?[0-9.]+) LUFS"),
        "input_tp": find(r"Peak:\s*(-?[0-9.]+) dBFS"),
        "input_lra": find(r"LRA:\s*([0-9.]+) LU"),
    }
...
def caption_qc() -> dict:
    ...
    durations_ok = all(0.45 <= float(cue["end"]) - float(cue["start"]) <= 4.2 for cue in cues)
    line_lengths_ok = all(max(len(line) for line in str(cue["text"]).split("\n")) <= 46 for cue in cues)
    return {
        ...
        "basis": "Caption cues are forced-aligned from fitted narration word timestamps; locked script text is preserved.",
    }
...
def main() -> int:
    ...
    checks = {
        "duration_ok": abs(duration - EXPECTED_DURATION) < 0.25,
        "frame_count_ok": abs(int(video.get("nb_frames", 0)) - round(EXPECTED_DURATION * 30)) <= 10,
        ...
        "loudness_ok": -14.6 <= loud["input_i"] <= -13.4,
        "true_peak_ok": loud["input_tp"] <= -1.0,
        "captions_ok": caps["monotonic"] and caps["durations_ok"] and caps["line_lengths_ok"],
        "four_part_structure_sampled": CONTACT.exists(),
    }
    data = {
        "episode_id": EP,
        "revision": "v002",
