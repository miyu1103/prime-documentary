#!/usr/bin/env python3
"""Schedule the AE-ENHANCED shorts short50-51 (EP48 glover / EP49 strieff) to YouTube (private + publishAt).
Uses the *_coverfirst_ae.mp4 renders (After Effects hero-beats composited in). Old non-AE uploads were deleted first.

Clone of schedule_new_shorts_v002.py, retargeted to short50/51. Coverfirst render
(designed cover baked into first ~1.5s) uploaded PRIVATE with status.publishAt;
YouTube flips to public at that time. Sets the cover as the custom thumbnail too.
Idempotent/resumable: skips any short whose result file exists OR whose title already
appears on the channel (avoids the mid-batch double-upload orphan pattern).

    py -3.11 scripts/schedule_new_shorts_v003.py --dry-run
    py -3.11 scripts/schedule_new_shorts_v003.py
"""
...
SHORTS = [
    {"n": 50, "publishAt": "2026-08-12T03:00:00Z",
     "title": "A Cop Stopped This Truck Over a Name on the Plate. The Court Said Yes. #Shorts",
     "desc": "Kansas v. Glover (2020), 8-1: an officer stopped a truck knowing only that the registered owner's license was revoked, never seeing the driver. The Supreme Court said reasonable suspicion allowed the brief stop - but only until the officer can plainly see the driver isn't the owner. It does not let police stop any car."},
    {"n": 51, "publishAt": "2026-08-13T03:00:00Z",
     "title": "The Stop Was Illegal. The Evidence Still Put Him Away. #Shorts",
     "desc": "Utah v. Strieff (2016), 5-3: the State conceded the stop was illegal, but a pre-existing arrest warrant \"attenuated\" the chain, so the evidence was admissible - the exclusionary rule narrowed, not abolished. Justice Sotomayor, dissenting, warned of a \"carceral state, just waiting to be cataloged.\""},
]
...
def main(argv):
    ...
    # preflight
    for s in SHORTS:
        v = OUT / f"short{s['n']}_yt_coverfirst_ae.mp4"; t = THUMBS / f"short{s['n']}.png"
    ...
    for s in SHORTS:
        ...
        if s["title"] in existing:
            ...
            v = OUT / f"short{n}_yt_coverfirst_ae.mp4"; size = v.stat().st_size
