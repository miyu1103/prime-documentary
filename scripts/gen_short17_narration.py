#!/usr/bin/env python3
"""Generate SHORT #17 (OneCoin / "she sold nothing") vertical-short narration.

US-market English VO. Voice = channel voice, model = multilingual v2. Paid ElevenLabs (pre-authorized).
R3 / HIGH sensitivity: living, CHARGED-BUT-NOT-CONVICTED fugitive. Strict guardrails:
  - "no real blockchain" / fraud anchored to PROSECUTORS + the co-founder's GUILTY PLEA, not her proven conduct.
  - Never "guilty/fraudster/thief" about her; she is "charged, not convicted, and still missing."
  - Scale (EUR 4B) attributed to prosecutors / OneCoin's own records. Do not assert her intent or fate.
...
# (line_id, delivery, spoken_text) — US-market adaptation of SHORTS_EP16-18.md SHORT #17.
# All fraud/no-blockchain claims attributed to prosecutors + the co-founder's guilty plea.
LINES: list[tuple[str, str, str]] = [
    ("L1", "intense", "She sold a cryptocurrency that, prosecutors say, never really existed."),
    ("L2", "building", "OneCoin launched in 2014 in Bulgaria. You bought token packages, and you got paid to recruit more buyers."),
    ("L3", "building", "But prosecutors say, and the co-founder pleaded guilty, there was no real blockchain. The price was just numbers the company typed in."),
    ("L4", "intense", "Prosecutors cite over four billion euros in sales. Secretly charged in 2017, founder Ruja Ignatova flew to Athens and vanished. She is charged, not convicted, and still missing."),
...
def main() -> int:
    ...
    master = media_root() / "episodes" / EP / "06_voice" / "master" / "short17_vc_master_en_us_v002.mp3"
    master.parent.mkdir(parents=True, exist_ok=True)
    subprocess.run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(concat),
                    "-c:a", "libmp3lame", "-b:a", "192k", str(master)], check=True, capture_output=True)

    idx = ROOT / "episodes" / EP / "06_audio" / "short17_narration_index.v002.en_us.json"
