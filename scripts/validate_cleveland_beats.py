#!/usr/bin/env python
"""Validate EP45 AE card beats against Remotion figures."""
...
def evaluate() -> dict:
    ...
    film_path = ROOT / "remotion" / "src" / "data" / "cleveland_film.json"
    if film_path.exists():
    ...
    source = ROOT / "scripts" / "ae" / "build_cleveland_hero_cards.py"
    if source.exists() and DISCLOSURE not in source.read_text(encoding="utf-8", errors="ignore"):
        errors.append("build_cleveland_hero_cards.py lacks AI disclosure layer text")
...
def main() -> int:
    ...
    result = evaluate()
    print(("PASS" if result["ok"] else "FAIL") + f" validate_cleveland_beats: {result['beats']}")
