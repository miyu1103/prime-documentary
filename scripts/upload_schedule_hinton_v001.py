...
SCHEDULED_AT_LOCAL = "2026-07-14T12:00:00+09:00"
SCHEDULED_AT_UTC = "2026-07-14T03:00:00Z"
...
def upload_caption(token: str, video_id: str) -> dict:
    boundary = f"hinton_caption_{int(time.time())}"
    metadata = json.dumps(
        {
            "snippet": {
                "videoId": video_id,
...
def verify_preconditions() -> tuple[dict, dict, Path, Path, str]:
    ...
    if "AI-generated" not in description and "AI generated" not in description:
        raise RuntimeError("Description missing the synthetic-media disclosure")
