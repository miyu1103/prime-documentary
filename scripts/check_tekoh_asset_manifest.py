#!/usr/bin/env python
"""Validate EP44 Tekoh asset manifest from the consumer side."""
...
def evaluate(path: Path) -> dict:
    ...
    errors: list[str] = []
    if data.get("schema_version") != "tekoh_assets.v1":
    ...
    if data.get("is_stub") is True:
        errors.append("is_stub must be false for EP44")
...
def main() -> int:
    ...
    if not args.assets.exists():
        print(f"FAIL tekoh_asset_manifest: missing {args.assets}")
        return 1
    result = evaluate(args.assets)
    if args.json:
        print(json.dumps(result, ensure_ascii=False, indent=2))
    else:
        print(("PASS" if result["ok"] else "FAIL") + f" tekoh_asset_manifest: {args.assets}")
