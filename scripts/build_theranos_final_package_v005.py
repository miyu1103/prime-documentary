#!/usr/bin/env python3
"""Build the Theranos final v005 package records.
...
FINAL_VIDEO = Path("H:/pd-media/episodes/PD-2026-015-theranos/08_edit/renders/final/theranos_premium_final_v004.mp4")
CAPTIONS = EPDIR / "08_edit" / "captions.v001.srt"
ACCEPTANCE = EPDIR / "08_edit" / "renders" / "final.v004.acceptance.json"
...
def write_rights_manifest(video_sha: str, captions_sha: str, thumb_sha: str, final_duration: float) -> Path:
    ...
    out = PKG / "rights_manifest.v005.json"
...
def write_youtube_meta(video_sha: str, captions_sha: str, thumb_sha: str) -> Path:
    ...
    out = PKG / "youtube_meta.v005.json"
    write_json(out, data)
    (PKG / "title.v005.txt").write_text(TITLE + "\n", encoding="utf-8")
    write_json(PKG / "chapters.v005.json", {"episode_id": EP, "revision": "v005", "chapters": chapters()})
    write_json(PKG / "tags.v005.json", {"episode_id": EP, "revision": "v005", "tags": tags()})
    return out
...
def write_final_delivery(video_sha: str, captions_sha: str, thumb_sha: str, probe: dict[str, Any], rights: Path, meta: Path) -> Path:
    ...
    out = PKG / "final_delivery.v005.json"
    write_json(out, data)
    return out
...
def write_review_packets(video_sha: str, captions_sha: str, thumb_sha: str, final_delivery: Path, rights: Path, meta: Path) -> tuple[Path, Path, Path]:
    ...
    legal = {
        ...
        "r3_review_points": [
            ...
            "Factory/stock materials are generic illustrative assets with provenance in rights_manifest.v005.",
        ],
    }
    legal_json = PKG / "legal_rights_review_packet.v005.json"
    write_json(legal_json, legal)
    legal_md = PKG / "legal_rights_review_packet.v005.md"
    legal_md.write_text(
        "# Theranos v005 R3 Legal/Rights Review Packet\n\n"
    ...
    owner_json = PKG / "OWNER_REVIEW_REQUEST.v005.json"
    write_json(owner_json, owner)
    owner_md = PKG / "OWNER_REVIEW_REQUEST.v005.md"
    owner_md.write_text(
        "# OWNER_REVIEW_REQUEST v005 - Theranos Final Candidate\n\n"
        f"- Final video: `{str(FINAL_VIDEO).replace(chr(92), '/')}`\n"
        f"- SHA-256: `{video_sha}`\n"
    ...
    approval = {
        ...
        "external_side_effects": {"paid_api": False, "upload": False, "publish": False, "schedule": False},
        "drafts": [
            {"draft_id": "APR-0002-DRAFT", "gate": "final_review_video", "decision": "pending_owner_decision", "target_revision": "v005", "video": str(FINAL_VIDEO).replace("\\", "/"), "video_sha256": video_sha, "owner_review_request": rel(owner_json), "owner_review_request_sha256": sha256(owner_json)},
            {"draft_id": "APR-0003-DRAFT", "gate": "title_thumbnail", "decision": "pending_owner_decision", "target_revision": "v005", "title": TITLE, "thumbnail": rel(THUMB), "thumbnail_sha256": thumb_sha, "youtube_meta": rel(meta), "youtube_meta_sha256": meta_sha},
            {"draft_id": "APR-0004-DRAFT", "gate": "final_narration_release", "decision": "pending_owner_decision", "target_revision": "v005", "narration_index": rel(NARR_INDEX), "audio_mix": rel(AUDIO_MIX), "provider": "ElevenLabs"},
            {"draft_id": "APR-0005-DRAFT", "gate": "legal_rights", "decision": "pending_owner_or_legal_decision", "target_revision": "v005", "legal_rights_review_packet": rel(legal_json), "legal_rights_review_packet_sha256": sha256(legal_json), "rights_manifest": rel(rights), "rights_manifest_sha256": rights_sha},
            {"draft_id": "APR-0006-DRAFT", "gate": "publish_schedule", "decision": "pending_owner_decision", "target_revision": "v005", "video": str(FINAL_VIDEO).replace("\\", "/"), "video_sha256": video_sha, "final_delivery": rel(final_delivery), "final_delivery_sha256": fd_sha, "youtube_meta": rel(meta), "youtube_meta_sha256": meta_sha, "rights_manifest": rel(rights), "rights_manifest_sha256": rights_sha},
        ],
    }
    approval_json = PKG / "approval_drafts.v005.json"
    write_json(approval_json, approval)
    (PKG / "approval_drafts.v005.md").write_text(
        "# Theranos Approval Drafts v005\n\n"
...
def update_manifest(files: dict[str, Path], video_sha: str) -> None:
    ...
    active.update(
        {
            "captions_final": "v001",
            "audio_mix": "v001",
            "final_acceptance": "v004",
            "youtube_meta": "v005",
            "rights_manifest": "v005",
            "final_delivery": "v005",
            "owner_review_request": "v005",
            "approval_drafts": "v005",
            "legal_rights_review_packet": "v005",
            "chapters": "v005",
            "tags": "v005",
            "title": "v005",
        }
    ...
    add_artifact("final_delivery", "v005", files["final_delivery"])
    add_artifact("youtube_metadata", "v005", files["youtube_meta"])
    add_artifact("rights_manifest", "v005", files["rights_manifest"])
    add_artifact("owner_review_request", "v005", files["owner_review_request"])
    add_artifact("approval_drafts", "v005", files["approval_drafts"])
    add_artifact("legal_rights_review_packet", "v005", files["legal_packet"])
    add_artifact("final_acceptance", "v004", ACCEPTANCE)
    manifest["updated_at"] = now()
    manifest["warnings"] = [
        warning
        for warning in manifest.get("warnings", [])
        if "Current review_proxy v005 is 624.96s" not in warning
    ]
    manifest.setdefault("warnings", []).append(
        f"Final candidate v005 rendered and independently accepted: {str(FINAL_VIDEO).replace(chr(92), '/')} sha256={video_sha}; public release remains blocked by APR-0002..APR-0006 including R3 legal review."
    )
...
def append_event(video_sha: str, fd: Path) -> None:
    event = {
        ...
        "episode_id": EP,
        "stage": "final_package",
        "event": "final_candidate_v005_packaged_owner_stop",
        "revision": "v005",
        "actor": "codex",
