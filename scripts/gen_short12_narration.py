#!/usr/bin/env python3
"""Generate SHORT #12 (forced arbitration) vertical-short narration with the channel voice.
...
# (line_id, delivery, spoken_text)  — US-market adaptation of SHORTS_EP9-15.md SHORT #12.
LINES: list[tuple[str, str, str]] = [
    ("L1", "intense", "You probably gave up your right to sue, the moment you tapped \"I agree.\""),
    ("L2", "building", "Buried in your phone, bank, and job contracts is one clause: disputes go to private arbitration. No court. No class action."),
    ("L3", "building", "It started with about thirty dollars in tax on a \"free\" phone. Small harms only matter when people can join together."),
    ("L4", "intense", "In 2011, the Supreme Court said companies can enforce this. In 2018, it reached the workplace too. Two ordinary rights, gone, in a line nobody reads."),
...
def main() -> int:
    ...
    master = media_root() / "episodes" / EP / "06_voice" / "master" / "short12_vc_master_en_us_v002.mp3"
    master.parent.mkdir(parents=True, exist_ok=True)
    subprocess.run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(concat),
                    "-c:a", "libmp3lame", "-b:a", "192k", str(master)], check=True, capture_output=True)

    idx = ROOT / "episodes" / EP / "06_audio" / "short12_narration_index.v002.en_us.json"
