#!/usr/bin/env python3
"""Schedule EP43 atwater to YouTube (private, auto-publish at publishAt).
...
CONFIRM no existing atwater video -> resumable PRIVATE upload -> set thumbnail ->
schedule (status.publishAt) -> upload English caption -> re-query + verify.
If a PRIVATE atwater upload already exists (e.g. an agent died mid-upload, the EP40/EP41
...
run; until then the sha256 guard fails closed. APR ref: APR-0001 (approvals/APR-0001.json).
    ...
    py -3.11 scripts/schedule_atwater.py --dry-run
    py -3.11 scripts/schedule_atwater.py
"""
...
VIDEO_FILE = Path("H:/pd-media/episodes/PD-2026-047-atwater/07_render/atwater_final_bgm.v002_ae.mp4")
THUMB_FILE = PKG / "thumbnail.selected.v001.png"
CAPTION_FILE = EPDIR / "08_edit" / "captions.final.v001.srt"
EXPECTED_HASH = "84af9bb93b7cb7c806e9a34d7f8a66414432de44b5b1b0f98d9f3aa67cd8c751"
PUBLISH_AT = "2026-08-15T03:00:00Z"
SCHEDULED_LOCAL = "2026-08-15T12:00:00+09:00"
...
def upload_caption(token, video_id):
    boundary = f"atwater_caption_{int(time.time())}"
...
def is_atwater_match(title, want_title):
    t = title.strip().lower()
    if '#short' in t:
...
def main(argv):
    ...
    # Pre-flight: enumerate recent uploads, detect an existing atwater video (double-upload guard)
    print("  scanning recent channel uploads for an existing atwater video ...")
    recent = list_recent_uploads(token, ch, n=25)
    want_title = meta["title"]
    existing = [(vid, title, priv) for (vid, title, priv) in recent
                if is_atwater_match(title, want_title)]
    for vid, title, priv in recent[:12]:
        flag = "  <-- MATCH" if (vid, title, priv) in existing else ""
        print(f"    [{priv:8}] {vid}  {title[:60]}{flag}")
    if existing:
        print(f"  NOTE: {len(existing)} possible existing atwater upload(s) found.")
    ...
    if args.dry_run:
        ...
        if existing:
            print("[dry-run] existing atwater video(s) present -> real run would FINALIZE the "
                  "private one instead of re-uploading:")
            for vid, title, priv in existing:
                print(f"          {priv} {vid} {title[:50]}")
        return 0

    # Decide: finalize an existing PRIVATE atwater upload, else fresh upload
    reused = False
    video_id = None
    ...
    if priv_existing:
        ...
        reused = True
        print(f"OK reusing existing PRIVATE atwater upload video_id={video_id} (no re-upload)")
        caps = existing_captions(token, video_id)
