...
SCRIPT_SRC = ROOT / "episodes" / "_planning" / "EP42_young_script.en.v001.md"
SOURCE_SCRIPT_REL = "episodes/_planning/EP42_young_script.en.v001.md"
...
DELIVERY_BY_SECTION = {
    "HOOK": "intense",    # the wrong-house raid, wrists cuffed, "the wrong house"
    "OP": "building",     # thesis: does the law owe you anything
    "ACT_1": "building",  # the raid, the ankle monitor, the body camera
    "ACT_2": "building",  # the tape: hidden footage, CBS airs it, COPA
    "ACT_3": "building",  # doctrinal core: Hudson, 5-4, still a command, Part IV
    "ACT_4": "building",  # the reach: $2.9M, 48-0 reported, 10-4 voted down
    "ENDING": "calm",     # payoff + earned CTA + sign-off
}
SECTION_ORDER = ["HOOK", "OP", "ACT_1", "ACT_2", "ACT_3", "ACT_4", "ENDING"]
# `## <heading>` regex -> canonical section (order matters; first match wins)
SECTION_HEADINGS = [
    ("HOOK", re.compile(r"^\[HOOK\]")),
    ("OP", re.compile(r"^\[OPENING NARRATION\]")),
    ("ACT_1", re.compile(r"^ACT 1\b")),
    ("ACT_2", re.compile(r"^ACT 2\b")),
    ("ACT_3", re.compile(r"^ACT 3\b")),
    ("ACT_4", re.compile(r"^ACT 4\b")),
    ("ENDING", re.compile(r"^ENDING\b")),
]
# any `## <heading>` that ends the narration body (appendix / non-VO)
STOP_HEADINGS = [
    re.compile(r"^\[BrandEndcard\]"),
    re.compile(r"^事実対応表"),
    re.compile(r"^改稿ログ"),
]
...
# near the 720.9s target and gaps must stay SMALL to avoid blowing past the 750s ceiling.
...
# `〔SILENCE 1.8s ...〕` or `【SILENCE 1.5s -- ...】` anywhere in a line -> capture seconds
...
def extract_events(md: str) -> list[tuple]:
    ...
    for raw in md.splitlines():
        line = raw.strip()
        if line.startswith("## "):
            sec, is_stop = section_for_heading(line[3:].strip())
            if is_stop:
                break
            section = sec           # None for BrandOpening / title etc. (drops until next act)
            continue
        if section is None or not line:
            continue
        sm = SILENCE_LINE.search(line)
        if sm:                      # scripted silence -> injected at concat, never spoken
...
def assert_clean(chunks: list[dict]) -> None:
    ...
    # EP42 forbidden-language guard (mirror of check_young_facts R-FORBID) -- never voice these
    forbid = ["no-knock", "no knock", "unconstitutional", "she changed the law", "changed the law"]
    for c in chunks:
...
def write_index(chunks: list[dict], offsets: dict[str, dict], master: Path,
    ...
    index = {
        ...
        "provenance": {
            "producer": "scripts/gen_narration_young.py",
            "provider": "ElevenLabs",
            "voice_id": VOICE_ID,
            "model_id": MODEL,
            ...
            "note": "start/end are per-chunk master offsets (inter-chunk silence excluded from each "
                    "window). Four scripted SILENCE beats (HOOK 1.8s, ACT_1 1.5s, ACT_2 0.9s, "
                    "ENDING 1.0s) are injected at their positions.",
        },
    }
...
def main(argv: list[str]) -> int:
    ...
    print(f"projected @190wpm = {words / 190.0 * 60:.0f}s speech (target ~720.9s; band 690-750 incl gaps)")
    ...
    print(f"MASTER measured = {total:.1f}s ({total / 60:.2f}min)  target ~720.9s  band 690-750 -> "
