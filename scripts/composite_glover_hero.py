#!/usr/bin/env python3
"""EP48 glover: composite the rendered After-Effects hero cards onto the finished BGM film
...
overwrites the input. Cloned verbatim from the verified composite_cleveland_hero.py /
composite_tlo_hero.py (only the BEATS path changes to the EP48 deck).

Usage: python scripts/composite_glover_hero.py <bgm_in.mp4> <out_v005_ae.mp4>
"""
...
BEATS = ROOT / "episodes" / "PD-2026-048-glover" / "08_edit" / "ae_hero" / "beats.json"
...
def main() -> int:
    if len(sys.argv) != 3:
        print("Usage: composite_glover_hero.py <bgm_in.mp4> <out_v005_ae.mp4>", file=sys.stderr); return 2
