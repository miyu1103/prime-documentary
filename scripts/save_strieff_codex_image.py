#!/usr/bin/env python
"""Save the latest Codex-generated image as an EP49 Strieff asset."""
...
def main() -> int:
    ...
    if not args.force and (media.exists() or public.exists()):
        raise SystemExit(f"{asset_id} already exists; pass --force to replace this run's asset")

    MEDIA_DIR.mkdir(parents=True, exist_ok=True)
    PUBLIC_DIR.mkdir(parents=True, exist_ok=True)
    with Image.open(src) as im:
        fitted = ImageOps.fit(im.convert("RGB"), (3840, 2160), Image.Resampling.LANCZOS)
        fitted.save(media, quality=95)
        fitted.save(public, quality=95)
