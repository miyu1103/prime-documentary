...
Output : H:/pd-media/assets/ai/shorts/short21/short21_01.png .. _07.png + short21_thumb.png
...
OUT = r"H:/pd-media/assets/ai/shorts/short21"
...
SHOTS = [
    ("short21_01", "A lone parachute silhouette falling into a vast black rainy night sky, tiny against the dark, cold and final, no face, wide negative space"),
    ("short21_02", "A 1971 airliner cabin aisle in warm dim light, an anonymous seated silhouette from behind in a dark suit, no visible face, period-neutral"),
    ("short21_03", "A briefcase opened just enough to show ambiguous wires and red shapes in shadow, a folded note beside it, menace implied not shown, no readable text"),
    ("short21_04", "A banded stack of plain twenty-dollar bills under a hard light on a cold surface, generic currency, no legible serial detail, tense"),
    ("short21_05", "The lowered aft airstair of a jet at night, rain and dark forest below, a void of black air, dramatic scale, no figure"),
    ("short21_06", "A child's hand uncovering decaying banded bills in wet riverbank sand, cold blue dawn, the only clue, no faces"),
    ("short21_07", "A closed worn dark case folder lying flat in a shaft of cold light, plain blank cover, frayed edges, absolutely no title, no lettering, no engraved text, no seal, no emblem, suspended-not-solved mood"),
    ("short21_thumb", "A single parachute vanishing into a black night sky over a dark pine forest, ominous and final, cold blue and black, iconic key visual, no legible text"),
]
...
def gen(stem, core, force, no_hr):
    ...
    if os.path.exists(out_path) and not force:
        ...
        return
    seed = 210000 + int(stem.split("_")[1].replace("thumb", "99"))
    payload = {
...
def main():
    ...
    print("[all ] short21 image generation finished", flush=True)
