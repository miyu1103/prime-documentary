#!/usr/bin/env python3
"""Schedule EP43 tlo to YouTube (private, auto-publish at publishAt).
...
CONFIRM no existing tlo video -> resumable PRIVATE upload -> set thumbnail ->
schedule (status.publishAt) -> upload English caption -> re-query + verify.
If a PRIVATE tlo upload already exists (e.g. an agent died mid-upload, the EP40/EP41
...
run; until then the sha256 guard fails closed. APR ref: APR-0001 (approvals/APR-0001.json).
    ...
    py -3.11 scripts/schedule_tlo.py --dry-run
    py -3.11 scripts/schedule_tlo.py
"""
...
VIDEO_FILE = Path("E:/pd-media/episodes/PD-2026-046-tlo/07_render/tlo_final_bgm.v002_ae.mp4")
THUMB_FILE = PKG / "thumbnail.selected.v001.png"
CAPTION_FILE = EPDIR / "08_edit" / "captions.final.v001.srt"
EXPECTED_HASH = "2c8db4ebe58e137ed55aaa08bd30ead36deeb8c3e9a46fff3bfd91b86c51e5c4"
PUBLISH_AT = "2026-08-14T03:00:00Z"
SCHEDULED_LOCAL = "2026-08-14T12:00:00+09:00"
...
def upload_caption(token, video_id):
    boundary = f"tlo_caption_{int(time.time())}"
...
def main(argv):
    ...
    # Pre-flight: enumerate recent uploads, detect an existing tlo video (double-upload guard)
    print("  scanning recent channel uploads for an existing tlo video ...")
    recent = list_recent_uploads(token, ch, n=25)
    want_title = meta["title"]
    existing = [(vid, title, priv) for (vid, title, priv) in recent
                if is_tlo_match(title, want_title)]
    for vid, title, priv in recent[:12]:
        flag = "  <-- MATCH" if (vid, title, priv) in existing else ""
        print(f"    [{priv:8}] {vid}  {title[:60]}{flag}")
    if existing:
        print(f"  NOTE: {len(existing)} possible existing tlo upload(s) found.")
    ...
    if args.dry_run:
        ...
        if existing:
            print("[dry-run] existing tlo video(s) present -> real run would FINALIZE the "
                  "private one instead of re-uploading:")
            for vid, title, priv in existing:
                print(f"          {priv} {vid} {title[:50]}")
        return 0

    # Decide: finalize an existing PRIVATE tlo upload, else fresh upload
    reused = False
    video_id = None
    ...
    if priv_existing:
        ...
        reused = True
        print(f"OK reusing existing PRIVATE tlo upload video_id={video_id} (no re-upload)")
        caps = existing_captions(token, video_id)
