#!/usr/bin/env python
"""EP47 Atwater - AE hero-card JSX generator (ledger-locked, AE-2026-JP pipeline).

Cloned VERBATIM from the audited EP45 build_cleveland_hero_cards.py; only paths, the
violet accent, the 8-card DECK, and a DATE_STAMP layout (d01) + measured-fit SPLIT_COMPARE
(c01 phrase labels) were changed. All machine-trap handling is unchanged. EP47-specific:
  * accent = civil-violet #7A5CD0 [0.478,0.361,0.816] (NOT any prior-ep lane colour)
  * deck = DESIGN §6 / CODEX_B §7.2 (s01 n01 c01 d01 v01 q02 q01 l01), 5 layouts used
  * ledger = atwater_facts.v001.json A-IDs; only $50 (A03) is a numeric field; all years /
    citations (2001/532/318/24) live in strings so R-NUM never scans them
  * v01 keeps "THE ARREST STANDS"+"CONSTITUTIONAL" (C-1); q02 Souter "for the Court - yet
    permitted" (C-2); q01 O'Connor attributed "dissenting" (C-3, §2 APPROVED_QUOTES verbatim)

EP45-cloned notes below (retained for the shared machine-trap contract):
...
FACTS = ROOT / "episodes" / EP / "03_script" / "atwater_facts.v001.json"
FILM = ROOT / "remotion" / "src" / "data" / "atwater_film.json"
...
# §7.3 EP47 colour constants (0..1 float). civil-violet lane (#7A5CD0), NOT any prior ep.
ACCENT = [0.478, 0.361, 0.816]   # #7A5CD0 civil-violet (EP47 lane) numbers / underline / dissent
...
# Subject-anchored assertion forms ONLY (EP47 C-1/C-3). Do NOT add bare
# "illegal"/"unconstitutional" (would false-trip on legitimate context strings).
FORBIDDEN = ["the arrest was illegal", "illegal arrest", "unconstitutional arrest",
             "the court struck it down", "struck down the arrest",
             "o'connor, for the court", "o'connor wrote for the court",
             "souter dissented", "1997"]
...
#  EP47 deck = DESIGN §6 / CODEX_B §7.2 VERBATIM (8 cards, start-ascending, ledger-locked,
#  6-constraint compliant). still=None on every card -> vectorBed (no symbolic-still asset
#  dependency; matches the proven cleveland deck). All em-dashes are ASCII "-" (trap #3).
#  Only ledger numbers {4,5,25,42,50,318,532,1983,2001} may appear as a numeric field; every
#  year/citation ("2001","532","318","24") lives in a STRING (never scanned by R-NUM).
DECK = [
    # 1. s01 ACT_1: the maximum penalty is $50 -- and no jail at all (A03).
    dict(id="s01", layout="CENTER_STACK", count_type="CT_MONEY", anchor=48.0, dur=6.5,
         top="MAXIMUM FINE", value=50, prefix="$",
         bottom="NO JAIL - THIS IS THE ENTIRE PENALTY", still=None,
         nums=[("A03", 50)]),
    # 2. n01 ACT_1: a fine-only misdemeanor (A02).
    dict(id="n01", layout="CENTER_STACK", count_type="CT_TEXT", anchor=108.0, dur=6.0,
         top="A FINE-ONLY MISDEMEANOR", hero_text="NO JAIL OFFENSE",
         bottom="THE MOST THE LAW CAN DO IS TAKE FIFTY DOLLARS", still=None,
         nums=[]),
    # 3. c01 ACT_2: what the Court ALLOWED -- a fine-only offense vs a full arrest (A10/A02).
    dict(id="c01", layout="SPLIT_COMPARE", anchor=232.0, dur=6.5,
         top="WHAT THE COURT ALLOWED", left="FINE-ONLY OFFENSE", right="FULL ARREST",
         strike="", bottom="PROBABLE CAUSE WAS ENOUGH", still=None,
         nums=[]),
    # 4. d01 ACT_3 open: decision date + citation (A18). Numbers live in strings only.
    dict(id="d01", layout="DATE_STAMP", anchor=360.0, dur=6.0,
         top="DECIDED", date="APRIL 24, 2001",
         place="ATWATER v. CITY OF LAGO VISTA - 532 U.S. 318", still=None,
         nums=[]),
    # 5. v01 ACT_3: the 5-4 vote; the arrest STANDS, constitutional (A11/A10). C-1: never "illegal".
    dict(id="v01", layout="VOTE_SPLIT", anchor=406.0, dur=6.5,
         top="THE ARREST STANDS", left="5", right="4",
         bottom="CONSTITUTIONAL - SOUTER MAJORITY, O'CONNOR DISSENT", still=None,
         nums=[]),
    # 6. q02 ACT_3: Souter's concession -- "pointless indignity" -- FOR THE COURT, yet permitted (A14). C-2.
    dict(id="q02", layout="CENTER_STACK", count_type="CT_TEXT", anchor=442.0, dur=6.5,
         top="THE MAJORITY CALLED IT THIS", hero_text="POINTLESS INDIGNITY",
         bottom="JUSTICE SOUTER, FOR THE COURT - YET PERMITTED IT", still=None,
         nums=[]),
    # 7. q01 ACT_3: O'Connor verbatim (A17), attributed DISSENTING (C-3). §2 APPROVED_QUOTES verbatim.
    dict(id="q01", layout="QUOTE_CARD", anchor=498.0, dur=7.5,
         top="THE DISSENT",
         quote=("THE COURT NEGLECTS THE FOURTH AMENDMENT'S EXPRESS COMMAND IN THE NAME "
                "OF ADMINISTRATIVE EASE. IN SO DOING, IT CLOAKS THE POINTLESS INDIGNITY "
                "THAT GAIL ATWATER SUFFERED WITH THE MANTLE OF REASONABLENESS."),
         attribution="JUSTICE O'CONNOR, DISSENTING",
         attribution_canonical="Justice O'Connor, dissenting",
         still=None, nums=[]),
    # 8. l01 ACT_3->END: the majority's remedy belongs to legislatures, not the 4A (A15).
    dict(id="l01", layout="CENTER_STACK", count_type="CT_TEXT", anchor=556.0, dur=6.0,
         top="THE MAJORITY'S REMEDY", hero_text="LEFT TO LEGISLATURES",
         bottom="NOT THE FOURTH AMENDMENT - A DOOR LEFT TO THE STATES", still=None,
         nums=[]),
]
...
def build(dryrun: bool) -> int:
    ...
    for c in DECK:
        ...
        if c["layout"] == "ACT_TITLE_CARD":
            ...
            b["leftSize"] = fit_size(c["left"], 120, 760, ANTON_ADV)
            b["rightSize"] = fit_size(c["right"], 120, 760, ANTON_ADV)
            b["hero"] = f"{c['left']} - {c['right']}"
        elif c["layout"] == "DATE_STAMP":
            b["date"] = c["date"]
            b["place"] = c["place"]
            b["hero"] = c["date"]
            b["dateSize"] = fit_size(c["date"], 200, 1500, ANTON_ADV)
        ...
        blob = " ".join(str(x) for x in (
            ...
            b.get("place"), b.get("date"), b.get("left"), b.get("right"),
        ) if x).lower()
    ...
    doc = {
        "schema_version": "atwater_ae_beats.v1",
        ...
        "offset_note": (
            ...
            "atwater_film.json figures[]). Absolute time in the finished bookended "
        ...
        "accent": "#7A5CD0",
        "ledger_note": (
            "The only numeric field burned is $50 (A03 verified:true). Every year and "
            "citation (2001 / 532 / 318 / 24) lives in a display STRING, never a numeric "
            "field, so R-NUM never scans it. All EP47 prohibited assertion forms were "
            "asserted absent from every card string at build time (FORBIDDEN gate in the "
            "builder); this note is deliberately term-free so it cannot trip R-FORBID."),
    ...
    jsx = jsx.replace("__AEP__", str(media / "atwater_hero.aep").replace("\\", "/"))
    (media / "atwater_hero.jsx").write_text(jsx, encoding="utf-8-sig")
    ...
    print("[gen] jsx  ->", media / "atwater_hero.jsx")
    print("[gen] aep  ->", media / "atwater_hero.aep")
...
(function () {
  ...
  try { for (var ri = proj.numItems; ri >= 1; ri--) {
    ...
    if (itx instanceof CompItem && String(itx.name).indexOf("ATWATER_") === 0) itx.remove();
  } } catch (e) {}
  try { proj.gpuAccelType = GpuAccelType.SOFTWARE; } catch (e) {}
  try { proj.bitsPerChannel = 8; } catch (e) {}
  app.beginUndoGroup("atwater_hero");
  ...
  function buildQuote(comp, spec) {
    // EP47 q01 is a LONG (280-char) dissent quote -> wrap, then MEASURE-fit BOTH width and
    // height so the block never collides with the top label or the attribution (q01 clip fix).
    topLabel(comp, spec, H * 0.105, 42);
    var wrapped = wrapText(spec.quote, 34);
    var L = text(comp, wrapped, FONT_LBL, 72, WHITE, 8);
    var maxW = 1480, maxH = H * 0.50, guard = 0;
    fitText(L, maxW, comp.duration * 0.7);
    var rr = srect(L, comp.duration * 0.7);
    while (rr.height > maxH && guard < 24) {
      var tp = L.property("ADBE Text Properties").property("ADBE Text Document");
      var cur = tp.value.fontSize;
      var ns = Math.floor(cur * (maxH / rr.height) * 0.985);
      if (ns >= cur) ns = cur - 1;
      if (ns < 12) ns = 12;
      setFontSize(L, ns);
      rr = srect(L, comp.duration * 0.7); guard++;
      if (ns === 12) break;
    }
    ...
    // quote marks framing the phrase (ASCII-safe char codes), civil-violet
    var q1 = text(comp, String.fromCharCode(8220), FONT_NUM, 130, ACCENT, 0);
    tg(q1).property("ADBE Position").setValue([W * 0.06, H * 0.235]);
    key2(tg(q1).property("ADBE Opacity"), 0.3, 0, 0.8, 55, 70);
    var q2 = text(comp, String.fromCharCode(8221), FONT_NUM, 130, ACCENT, 0);
    tg(q2).property("ADBE Position").setValue([W * 0.94, H * 0.235]);
    key2(tg(q2).property("ADBE Opacity"), 0.4, 0, 0.9, 55, 70);
    accent(comp, H * 0.775, 560);
    var a = text(comp, spec.attribution_display, FONT_LBL, 40, SILVER, 160);
    fitText(a, 1500, comp.duration * 0.8); centerX(a, comp.duration * 0.8);
    revealUp(a, 1.3, W / 2, H * 0.845, 40);
  }
  ...
  function buildCompare(comp, spec) {
    // EP47 c01: left/right are PHRASES (not short years) -> each is MEASURED-fit into its
    // own half so a long label never clips (EP40 lesson). Word-wrapped to 2 lines then fit.
    topLabel(comp, spec, H * 0.20, 44);
    var yc = H * 0.45, halfW = W * 0.40, lx = W * 0.275, rx = W * 0.725;
    var nL = text(comp, wrapText(spec.left, 16), FONT_NUM, spec.leftSize || 120, WHITE, 0);
    nL.motionBlur = true;
    centerBlock(nL, comp.duration * 0.8); fitText(nL, halfW, comp.duration * 0.8);
    centerBlock(nL, comp.duration * 0.8);
    tg(nL).property("ADBE Position").setValue([lx, yc]);
    var loo = tg(nL).property("ADBE Opacity");
    loo.setValueAtTime(0.5, 0); loo.setValueAtTime(0.75, 100);
    key2(tg(nL).property("ADBE Scale"), 0.5, [62, 62], 0.9, [100, 100], 78);
    var nR = text(comp, wrapText(spec.right, 16), FONT_NUM, spec.rightSize || 120, ACCENT, 0);
    nR.motionBlur = true;
    centerBlock(nR, comp.duration * 0.8); fitText(nR, halfW, comp.duration * 0.8);
    centerBlock(nR, comp.duration * 0.8);
    tg(nR).property("ADBE Position").setValue([rx, yc]);
    var roo = tg(nR).property("ADBE Opacity");
    roo.setValueAtTime(0.7, 0); roo.setValueAtTime(0.95, 100);
    key2(tg(nR).property("ADBE Scale"), 0.7, [62, 62], 1.1, [100, 100], 78);
    // thin vertical accent divider between the two halves (violet), wipes down
    var div = rect(comp, "divider", 5, H * 0.30, ACCENT, 90, W / 2, yc);
    div.motionBlur = true;
    key2(tg(div).property("ADBE Scale"), 0.55, [100, 0], 1.05, [100, 100], 90);
    tg(div).property("ADBE Opacity").setValue(90);
    // optional strike stamp (unused in EP47 deck; drawn only if provided)
    if (spec.strike && String(spec.strike).length) {
  ...
  function buildDate(comp, spec) {
    // EP47 d01 DATE_STAMP: kicker (top) -> big date (Anton, violet, scale-in) -> accent bar
    // -> citation/place line (Oswald). All numbers live in these display strings only.
    topLabel(comp, spec, H * 0.22, 46);
    var L = text(comp, spec.date, FONT_NUM, spec.dateSize || 200, ACCENT, 4);
    fitText(L, 1560, comp.duration * 0.85);
    centerX(L, comp.duration * 0.85);
    L.motionBlur = true;
    tg(L).property("ADBE Position").setValue([W / 2, H * 0.45]);
    var o = tg(L).property("ADBE Opacity");
    ...
    sc.setValueAtTime(0.4, [118, 118]); sc.setValueAtTime(0.9, [100, 100]);
    ease(sc, 80);
    key2(tg(L).property("ADBE Position"), 0.4, [W / 2, H * 0.45 + 34], 0.9, [W / 2, H * 0.45], 80);
    accent(comp, H * 0.585, 620);
    var pl = text(comp, spec.place, FONT_LBL, 46, WHITE, 60);
    fitText(pl, 1620, comp.duration * 0.85); centerX(pl, comp.duration * 0.85);
    revealUp(pl, 1.4, W / 2, H * 0.70, 44);
  }

  function build(spec) {
    var comp = proj.items.addComp("ATWATER_" + spec.id, W, H, 1.0, spec.dur, FPS);
