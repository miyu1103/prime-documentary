#!/usr/bin/env python3
"""EP49 strieff: composite the rendered After-Effects hero cards onto the finished
BGM film as full-frame hero beats at their narration-matched windows.

FIX (was a Codex bug): the prior version overlaid at beats[].start with NO offset,
which places every card 11.5s too early (over the hook/opening). beats.json
start/end are BODY/narration-relative; film time = start + film_offset_sec
(hook 8.0 + opening 3.5 = 11.5s). This version reads film_offset_sec from
beats.json and adds it, matching the shipped composite_atwater_hero.py.

Each card (beats[].out) is a 1920x1080 opaque render that fully replaces the base
during its window. A REQUIRED beat whose render is missing aborts (no silent gap).
Never overwrites the input.

Usage: python scripts/ae/composite_strieff_hero.py <bgm_in.mp4> <out_v001_ae.mp4>
"""
...
BEATS = ROOT / "episodes" / "PD-2026-049-strieff" / "08_edit" / "ae_hero" / "beats.json"
...
def main() -> int:
    if len(sys.argv) != 3:
        print("Usage: composite_strieff_hero.py <bgm_in.mp4> <out_v001_ae.mp4>", file=sys.stderr); return 2
    ...
    r = subprocess.run([FFMPEG, "-y", "-hide_banner", *inputs, "-filter_complex", fg,
                        ...
                        "-pix_fmt", "yuv420p", "-colorspace", "bt709", "-c:a", "copy", str(out)],
