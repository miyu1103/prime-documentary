#!/usr/bin/env python
"""EP49 Strieff - AE hero-card JSX generator (ledger-locked, AE-2026-JP pipeline).

Cloned from the audited EP45 build_cleveland_hero_cards.py (FIXED version). EP49-specific:
  * accent = somber-plum #9C6BAA (NOT the EP45 crimson) -- lane separation
  * 5 layouts used from the proven set: ACT_TITLE_CARD / CENTER_STACK /
    QUOTE_CARD / VOTE_SPLIT / SPLIT_COMPARE (MONEY_STACK unused -- no money)
  * every card burns an "AI-assisted visualization" disclosure (R1) bottom-right
  * count types CT_TEXT only (CT_TEXT = no count-up; the year 2016 / cite 579/232
    are static, must NEVER animate/comma)

LEDGER LOCK (episodes/PD-2026-049-strieff/03_script/strieff_facts.v001.json). Every
burned number is a verified:true S-ID value. No off-ledger figure can ship: a
...
Accuracy 6-constraint compliance baked into the strings (§2 of CODEX_B):
  C-1 the stop was ILLEGAL   -> c01 "THE STOP WAS ILLEGAL / THE STATE CONCEDED ..."
                                (never "the stop was legal")
  C-2 rule narrowed, warrant -> a02 / cmp01 "THE WARRANT BROKE THE CHAIN"
                                (never "exclusionary rule abolished")
  C-4 5-3, Scalia seat empty -> v01 "5 / 3" + "AN 8-JUSTICE COURT - SCALIA'S SEAT
                                EMPTY" (never 6-3 / 8-3); q01/q02 attributed to
                                "JUSTICE ..., DISSENTING" (never for the Court)
  Forbidden strings (the stop was legal / exclusionary rule abolished / we are all
  harmed / 6-3 / 8-3 / ...) appear on NO card. Asserted at build time.
...
FACTS = ROOT / "episodes" / EP / "03_script" / "strieff_facts.v001.json"
...
# §7.3 EP49 colour constants (0..1 float). somber-plum lane, illegal-stop tone.
ACCENT = [0.612, 0.420, 0.667]   # #9C6BAA somber-plum (EP49 lane) numbers / underline / dissent
...
RED = [0.780, 0.290, 0.243]      # #C74A3E strike mark only

STILLS = Path("E:/pd-media/assets/ai/strieff")
...
FORBIDDEN = ["the stop was legal", "a lawful stop", "the stop was lawful",
             "reasonable suspicion existed", "the police were allowed to stop",
             "exclusionary rule was abolished", "exclusionary rule abolished",
             "abolished the exclusionary rule", "struck down the exclusionary rule",
             "exclusionary rule was struck down", "the exclusionary rule no longer",
             "sotomayor, for the court", "kagan, for the court",
             "the majority dissented", "we are all harmed",
...
DECK = [
    # 1. a01: act title -- the stop was illegal, and everyone knew it (S06).
    dict(id="a01", layout="ACT_TITLE_CARD", anchor=118.0, dur=6.0,
         main="THE ILLEGAL STOP", top="UTAH v. STRIEFF", still=None,
         nums=[]),
    # 2. c01: the stop was ILLEGAL; the State conceded no reasonable suspicion (S03).
    #    Keep "ILLEGAL" + "CONCEDED" -- the C-1 / R-LEGAL frame. Never "legal".
    dict(id="c01", layout="CENTER_STACK", count_type="CT_TEXT", anchor=175.0, dur=6.0,
         top="EVERYONE AGREED", hero_text="THE STOP WAS ILLEGAL",
         bottom="THE STATE CONCEDED THERE WAS NO REASONABLE SUSPICION", still=None,
         nums=[]),
    # 3. a02: attenuation -- the exception; the warrant broke the chain (S07/S08).
    #    Admission is attributed to the WARRANT (C-2/C-3/R-ATTEN). Never "abolished".
    dict(id="a02", layout="CENTER_STACK", count_type="CT_TEXT", anchor=300.0, dur=6.5,
         top="THE EXCEPTION TO THE RULE", hero_text="ATTENUATION",
         bottom="THE WARRANT BROKE THE CHAIN - THE EVIDENCE STAYED", still=None,
         nums=[]),
    # 4. cmp01: an illegal stop, yet evidence admitted -- only because of the warrant (S08).
    dict(id="cmp01", layout="SPLIT_COMPARE", anchor=370.0, dur=6.5,
         top="WHAT THE COURT ALLOWED", left="AN ILLEGAL STOP", right="EVIDENCE ADMITTED",
         strike="STILL ILLEGAL",
         bottom="ONLY BECAUSE A WARRANT BROKE THE CHAIN", still=None,
         nums=[]),
    # 5. v01: 5-3, an 8-justice court, Scalia's seat empty (S10). Keep the 8-justice
    #    framing so 5/3 never reads as 8-3 (C-4 / R-VOTE).
    dict(id="v01", layout="VOTE_SPLIT", anchor=430.0, dur=6.5,
         top="THE EVIDENCE STAYS", left="5", right="3",
         bottom="AN 8-JUSTICE COURT - SCALIA'S SEAT EMPTY", still=None,
         nums=[("S10", 5), ("S10", 3), ("S10", 8)]),
    # 6. q01: Sotomayor "carceral state" -- verbatim dissent (S13). DISSENTING.
    dict(id="q01", layout="QUOTE_CARD", anchor=475.0, dur=7.0,
         top="THE DISSENT",
         quote=("IT IMPLIES THAT YOU ARE NOT A CITIZEN OF A DEMOCRACY BUT THE SUBJECT "
         ...
         still=None, nums=[]),
    # 7. q02: Kagan incentive -- verbatim dissent (S16). DISSENTING.
    dict(id="q02", layout="QUOTE_CARD", anchor=510.0, dur=6.5,
         top="THE DISSENT",
         quote="THE OFFICER'S INCENTIVE TO VIOLATE THE CONSTITUTION THUS INCREASES.",
         attribution="JUSTICE KAGAN, DISSENTING",
         attribution_canonical="Justice Kagan, dissenting",
         still=None, nums=[]),
    # 8. d01: decided 2016, Utah v. Strieff, 579 U.S. 232 (S00). CT_TEXT -- no count-up.
    dict(id="d01", layout="CENTER_STACK", count_type="CT_TEXT", anchor=545.0, dur=6.0,
         top="DECIDED", hero_text="2016",
         bottom="UTAH v. STRIEFF - 579 U.S. 232", still=None,
         nums=[("S00", 2016), ("S00", 579), ("S00", 232)]),
]
...
def load_facts() -> dict:
    ...
    if isinstance(d.get("ledger"), list):
        return {str(row.get("id")): row for row in d["ledger"] if isinstance(row, dict) and row.get("id")}
    return d
...
def clear(s: float, d: float, figs, placed, total: float, buf: float = 2.1) -> bool:
    e = s + d
...
def build(dryrun: bool) -> int:
    ...
    doc = {
        "schema_version": "strieff_ae_beats.v1",
        ...
        "offset_note": (
            ...
            "strieff_film.json figures[]). Absolute time in the finished bookended "
        ...
        "accent": "#9C6BAA",
        "ledger_note": (
            "Every burned number is a strieff_facts.v001.json verified:true S-ID "
            "value. All EP49 prohibited terms were "
    ...
    jsx = jsx.replace("__AEP__", str(media / "strieff_hero.aep").replace("\\", "/"))
    (media / "strieff_hero.jsx").write_text(jsx, encoding="utf-8-sig")
    ...
    print("[gen] jsx  ->", media / "strieff_hero.jsx")
    print("[gen] aep  ->", media / "strieff_hero.aep")
    print("[gen] beats->", REPO_BEATS_DIR / "beats.json")
    return 0


JSX = r"""// AUTO-GENERATED - EP49 Strieff AE hero cards. Do not hand-edit.
(function () {
  ...
  try { for (var ri = proj.numItems; ri >= 1; ri--) {
    ...
    if (itx instanceof CompItem && String(itx.name).indexOf("STRIEFF_") === 0) itx.remove();
  } } catch (e) {}
  try { proj.gpuAccelType = GpuAccelType.SOFTWARE; } catch (e) {}
  try { proj.bitsPerChannel = 8; } catch (e) {}
  app.beginUndoGroup("strieff_hero");
  ...
  function buildVote(comp, spec) {
    ...
    // pips = the ACTUAL split (5 silver majority + 3 accent dissent = 8-justice court).
    // Was hardcoded pn=9 (5+4) which mis-reads an 8-justice 5-3 vote as 5-4.
    var majN = parseInt(spec.left, 10) || 5, disN = parseInt(spec.right, 10) || 3;
  ...
  function buildCompare(comp, spec) {
    ...
    // left/right can be long phrases -> MEASURE-fit each into a bounded half with a
    // wide centre gutter so they never overlap the dash or clip the frame edge.
    // (Codex bug: 240px "AN ILLEGAL STOP"/"EVIDENCE ADMITTED" overran and collided.)
    var yc = H * 0.45, gap = W * 0.245, halfMax = 720;
    var lx = W / 2 - gap, rx = W / 2 + gap;
    var nL = text(comp, spec.left, FONT_NUM, 150, WHITE, 0);
    fitText(nL, halfMax, comp.duration * 0.85); centerX(nL, comp.duration * 0.85);
    nL.motionBlur = true;
    tg(nL).property("ADBE Position").setValue([lx, yc]);
    var loo = tg(nL).property("ADBE Opacity");
    loo.setValueAtTime(0.5, 0); loo.setValueAtTime(0.75, 100);
    key2(tg(nL).property("ADBE Scale"), 0.5, [62, 62], 0.9, [100, 100], 78);
    var nR = text(comp, spec.right, FONT_NUM, 150, ACCENT, 0);
    fitText(nR, halfMax, comp.duration * 0.85); centerX(nR, comp.duration * 0.85);
    nR.motionBlur = true;
    tg(nR).property("ADBE Position").setValue([rx, yc]);
    var roo = tg(nR).property("ADBE Opacity");
  ...
  function build(spec) {
    var comp = proj.items.addComp("STRIEFF_" + spec.id, W, H, 1.0, spec.dur, FPS);
