#!/usr/bin/env python
"""EP42 Young - AE hero-card JSX generator (ledger-locked, AE-2026-JP pipeline).

Cloned from the audited EP41 build_thompson_hero_cards.py. EP42-specific:
  * accent = warrant-blue #3B7DD8 (NOT the EP41 gold) -- lane separation
...
LEDGER LOCK (episodes/PD-2026-042-young/03_script/young_facts.v001.json). Every
...
FACTS = ROOT / "episodes" / EP / "03_script" / "young_facts.v001.json"
FILM = ROOT / "remotion" / "src" / "data" / "young_film.json"
...
# §7.3 EP42 colour constants (0..1 float). warrant-blue lane, night-Chicago tone.
ACCENT = [0.231, 0.490, 0.847]   # #3B7DD8 numbers / underline / dissent
...
# --------------------------------------------------------------- the card deck
# still names were eyeballed (contact sheet in this session): all people-free
# symbolic objects, no readable text, no faces/bodies (R1 / C-5 / C-6).
#   S05 dark 4-panel door | S30 night apartment block | S45 illegible papers
#   S63 vote-tally board (illegible dots) + empty chairs | S70 unused gavel
#   S60 blank check + blank fault box on marble | S84 Chicago skyline at dusk
# k01 renders as a pure vector card on INK (no still) -- the quote is the hero.
DECK = [
    dict(id="t01", layout="ACT_TITLE_CARD", anchor=44.8, dur=5.5,
         top="ACT ONE", main="THE WRONG DOOR", still="S05.png",
         nums=[]),
    dict(id="n01", layout="CENTER_STACK", count_type="CT_INT", anchor=87.7, dur=6.0,
         top="THAT NIGHT", value=12, prefix="ABOUT ", suffix=" OFFICERS",
         bottom="A SEARCH WARRANT, SIGNED BY A JUDGE", still="S30.png",
         nums=[("F02", 12)]),
    dict(id="c01", layout="CENTER_STACK", count_type="CT_INT", anchor=165.0, dur=6.5,
         top="~16 MONTHS", value=100, prefix="~", suffix=" ALLEGED",
         bottom="POLICE-OVERSIGHT INVESTIGATION", still="S45.png",
         nums=[("F07", 16), ("F08", 100)]),
    dict(id="k01", layout="QUOTE_CARD", anchor=267.0, dur=7.0,
         top="THE FOURTH AMENDMENT",
         quote="STILL A COMMAND OF THE FOURTH AMENDMENT",
         attribution="JUSTICE SCALIA, FOR THE MAJORITY",
         attribution_canonical="Justice Scalia, for the majority",
         still=None, nums=[]),
    dict(id="v01", layout="VOTE_SPLIT", anchor=309.5, dur=6.5,
         top="SUPREME COURT - 2006", left="5", right="4",
         bottom="THE EXCLUSION REMEDY, DENIED", still="S63.png",
         nums=[("F15", 5), ("F15", 4)]),
    dict(id="e01", layout="CENTER_STACK", count_type="CT_TEXT", anchor=476.0, dur=6.0,
         top="THE REMAINING REMEDY", hero_text="SECTION 1983",
         bottom="THE CIVIL REMEDY", still="S70.png",
         nums=[("F19", 1983)]),
    dict(id="m01", layout="MONEY_STACK", count_type="CT_MONEY", anchor=505.5, dur=6.5,
         top="THE CITY SETTLED", value=2.9, prefix="$", suffix="M", decimals=1,
         bottom="REPORTED - NO FINDING OF FAULT", still="S60.png",
         nums=[("F03", 2.9)]),
    dict(id="r01", layout="SPLIT_COMPARE", anchor=590.5, dur=6.5,
         top="ANJANETTE YOUNG ORDINANCE", left="10", right="4",
         strike="REJECTED", bottom="VOTED DOWN - STILL LEGAL", still="S84.png",
         nums=[("F12", 10), ("F12", 4)]),
]
...
def build(dryrun: bool) -> int:
    ...
    doc = {
        "schema_version": "young_ae_beats.v1",
        ...
        "offset_note": (
            ...
            "young_film.json figures[]). Absolute time in the finished bookended "
        ...
        "accent": "#3B7DD8",
        "ledger_note": (
            "Every burned number is a young_facts.v001.json verified:true F-ID "
            "(12/16/100/5/4/1983/2.9M/10). $2.9M is F03 ($2,900,000) in the "
            "design's prescribed M-suffix form. All EP42 prohibited terms were "
    ...
    jsx = jsx.replace("__AEP__", str(media / "young_hero.aep").replace("\\", "/"))
    (media / "young_hero.jsx").write_text(jsx, encoding="utf-8-sig")
    ...
    print("[gen] jsx  ->", media / "young_hero.jsx")
    print("[gen] aep  ->", media / "young_hero.aep")
    print("[gen] beats->", REPO_BEATS_DIR / "beats.json")
    return 0


JSX = r"""// AUTO-GENERATED - EP42 Young AE hero cards. Do not hand-edit.
(function () {
  ...
  try { for (var ri = proj.numItems; ri >= 1; ri--) {
    ...
    if (itx instanceof CompItem && String(itx.name).indexOf("YOUNG_") === 0) itx.remove();
  } } catch (e) {}
  try { proj.gpuAccelType = GpuAccelType.SOFTWARE; } catch (e) {}
  try { proj.bitsPerChannel = 8; } catch (e) {}
  app.beginUndoGroup("young_hero");
  ...
  function build(spec) {
    var comp = proj.items.addComp("YOUNG_" + spec.id, W, H, 1.0, spec.dur, FPS);
