#!/usr/bin/env python
"""Composite EP48 Glover AE hero cards over a base render."""
...
def main() -> int:
    ...
    args = ap.parse_args()
    work = EDIT / "_preview" / "ae_hero" if args.preview else EDIT / "ae_hero"
    base = args.base or (EDIT / "_preview" / "glover_final_bgm.v002.mp4" if args.preview else EDIT / "glover_final_bgm.v002.mp4")
    out = args.out or (EDIT / "_preview" / "glover_final_bgm.v003_ae.mp4" if args.preview else EDIT / "glover_final_bgm.v003_ae.mp4")
    ...
    if not base.exists():
        ...
        return 2
    _doc = json.loads((work / "beats.json").read_text(encoding="utf-8"))
    beats = _doc.get("beats", [])
    # beats start/end are body/narration-relative; the base mp4 is the full
    # bookended film (hook+opening in front). Overlay windows MUST be shifted by
    # film_offset_sec so each card lands over its own narration, not 11.5s early.
    off = float(_doc.get("film_offset_sec", 0.0))
    print(f"[offset] film_offset_sec={off}")
    render = work / "render"
    ...
    for i, b in enumerate(valid):
        inputs += ["-i", str(b["mp4"])]
        idx = i + 1
        parts.append(f"[{idx}:v]setpts=PTS-STARTPTS+{float(b['start'])+off:.3f}/TB,format=yuv420p[b{i}]")
        lbl = f"v{i}"
        parts.append(f"[{prev}][b{i}]overlay=0:0:eof_action=pass:enable='between(t,{float(b['start'])+off:.3f},{float(b['end'])+off:.3f})'[{lbl}]")
        prev = lbl
