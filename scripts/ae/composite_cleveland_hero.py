#!/usr/bin/env python
"""Composite EP45 Cleveland AE hero cards over a base render."""
...
def main() -> int:
    ...
    base = args.base or (EDIT / "_dryrun" / "cleveland_final_bgm.v002.mp4" if args.dryrun else EDIT / "cleveland_final_bgm.v002.mp4")
    out = args.out or (EDIT / "_dryrun" / "cleveland_final_bgm.v003_ae.mp4" if args.dryrun else EDIT / "cleveland_final_bgm.v003_ae.mp4")
    ...
    for i, b in enumerate(valid):
        ...
        prev = lbl
    parts.append(f"[{prev}]noise=alls=3:allf=t+u[vout]")
    out.parent.mkdir(parents=True, exist_ok=True)
    cmd = [FFMPEG, "-y", "-hide_banner", *inputs, "-filter_complex", ";".join(parts), "-map", "[vout]", "-map", "0:a?", "-r", str(FPS), "-c:v", "libx264", "-preset", "slow", "-crf", "14", "-pix_fmt", "yuv420p", "-colorspace", "bt709", "-c:a", "copy", str(out)]
