#!/usr/bin/env python
"""EP48 Glover - AE hero-card JSX generator (ledger-locked, AE-2026-JP pipeline).

Cloned from the audited EP45 build_cleveland_hero_cards.py. EP48-specific:
  * accent = patrol-steel #5B8DB8 (NOT the EP45 crimson) -- lane separation
  * 6 layouts available from the proven set: ACT_TITLE_CARD / CENTER_STACK /
    QUOTE_CARD / VOTE_SPLIT / MONEY_STACK / SPLIT_COMPARE (deck uses 4)
  * every card burns an "AI-assisted visualization" disclosure (R1) bottom-right
  * count types CT_MONEY / CT_TEXT (CT_TEXT = no count-up; the year 2020 is a
    date, must NEVER animate/comma)

LEDGER LOCK (episodes/PD-2026-048-glover/03_script/glover_facts.v001.json). Every
burned number is a verified:true G-ID value. No off-ledger figure can ship: a
...
Accuracy 6-constraint compliance baked into the strings (§2 of CODEX_B):
  C-1 stop UPHELD 8-1        -> v01 "THE STOP STANDS" / "A NARROW RULE"
                               (never "the stop was illegal" / "struck down")
  C-2 narrow inference       -> od01 "UNLESS THE OFFICER KNOWS OTHERWISE"
                               (the inference dissolves; never "any car")
  C-3 reasonable suspicion   -> rs01 "A LOWER BAR THAN PROBABLE CAUSE"
                               (never "probable cause required")
  C-4 neutral attribution    -> q01 "JUSTICE SOTOMAYOR, DISSENTING";
                                k01 "JUSTICE KAGAN, CONCURRING"
  Forbidden strings (the stop was illegal / struck down / police can stop any car
  / probable cause required / dochighlight) appear on NO card. Asserted at build.
...
FACTS = ROOT / "episodes" / EP / "03_script" / "glover_facts.v001.json"
...
# §7.3 EP48 colour constants (0..1 float). patrol-steel lane, dusk-highway tone.
ACCENT = [0.357, 0.553, 0.722]   # #5B8DB8 patrol-steel (EP48 lane) numbers / underline / dissent
...
RED = [0.780, 0.290, 0.243]      # #C74A3E split-compare strike mark only

STILLS = Path("H:/pd-media/assets/ai/glover")
...
FORBIDDEN = ["the stop was illegal", "illegal stop", "unconstitutional stop",
             "struck down", "police can stop any car", "stop any car",
             "probable cause required", "needed probable cause", "dochighlight"]
...
DECK = [
    # 1. rs01 (ACT2): reasonable suspicion is a LOWER bar than probable cause (G09).
    dict(id="rs01", layout="CENTER_STACK", count_type="CT_TEXT", anchor=192.3, dur=6.0,
         top="A LOWER BAR THAN PROBABLE CAUSE", hero_text="REASONABLE SUSPICION",
         bottom="SPECIFIC FACTS, NOT CERTAINTY - A BRIEF INVESTIGATIVE STOP", still=None,
         nums=[]),
    # 2. od01 (ACT2->ACT3): the commonsense inference, and its limit (G08/G12).
    #    SPLIT_COMPARE renders left/right big & unfitted -> keep them short (OWNER/DRIVER);
    #    the full clauses live in top/bottom. "UNLESS THE OFFICER KNOWS OTHERWISE" (the
    #    C-2 limit) must NOT be dropped, or the inference reads as a blank check.
    dict(id="od01", layout="SPLIT_COMPARE", anchor=212.2, dur=6.0,
         top="THE OWNER IS PROBABLY DRIVING", left="OWNER", right="DRIVER",
         strike="NOT AUTOMATIC",
         bottom="UNLESS THE OFFICER KNOWS OTHERWISE - REASONABLE ONLY UNTIL THE FACTS SAY NO",
         still=None, nums=[]),
    # 3. d01 (ACT3 open): Kansas v. Glover, decided April 6, 2020 (G18). CENTER_STACK,
    #    the year is shown via CENTER_STACK (no date-stamp layout exists in the deck).
    #    "2020" is a date -> CT_TEXT, never comma/animate.
    dict(id="d01", layout="CENTER_STACK", count_type="CT_TEXT", anchor=242.4, dur=6.0,
         top="SUPREME COURT", hero_text="2020",
         bottom="KANSAS v. GLOVER - DECIDED APRIL 6", still=None,
         nums=[("G18", 2020)]),
    # 4. v01 (ACT3 vote): 8-1, the stop stands under a NARROW rule (G13/G10).
    #    "A NARROW RULE" + "THE STOP STANDS" must stay (no overclaim, no illegality read).
    dict(id="v01", layout="VOTE_SPLIT", anchor=324.5, dur=6.0,
         top="A NARROW RULE", left="8", right="1",
         bottom="THE STOP STANDS - THOMAS MAJORITY, SOTOMAYOR DISSENT", still=None,
         nums=[("G13", 8), ("G13", 1)]),
    # 5. q01 (ACT3 Sotomayor dissent, verbatim G17). attribution = DISSENTING (never Court).
    dict(id="q01", layout="QUOTE_CARD", anchor=542.95, dur=5.5,
         top="THE LONE DISSENT",
         quote=("THE MAJORITY TODAY HAS PAVED THE ROAD TO FINDING REASONABLE SUSPICION "
                "BASED ON NOTHING MORE THAN A DEMOGRAPHIC PROFILE."),
         attribution="JUSTICE SOTOMAYOR, DISSENTING",
         attribution_canonical="Justice Sotomayor, dissenting",
         still=None, nums=[]),
    # 6. k01 (ACT3 Kagan concurrence, G14): revocation, not suspension. CONCURRING (not dissent).
    dict(id="k01", layout="CENTER_STACK", count_type="CT_TEXT", anchor=549.45, dur=5.5,
         top="WHAT MADE THIS STOP REASONABLE", hero_text="REVOCATION, NOT SUSPENSION",
         bottom="A DRIVER WHO KEEPS IGNORING THE LAW - JUSTICE KAGAN, CONCURRING", still=None,
         nums=[]),
    # ---- EP48 v005 PREMIUM AE EXPANSION (owner 2026-07-24 "AEもがっつり使ってほしい") ----
    # 12 additional hero beats using ONLY the 6 proven layouts. CT_TEXT / QUOTE / SPLIT /
    # ACT_TITLE carry no ledger numbers (nums=[]); the two verbatim quotes are the same
    # Court/concurrence text already vetted in glover_film.json figures[]. No forbidden
    # strings; SPLIT_COMPARE left/right kept SHORT (rendered big & unfitted).
    # 7. a_stop: ACT ONE title (the stop).
    dict(id="a_stop", layout="ACT_TITLE_CARD", anchor=116.0, dur=5.5,
         top="ACT ONE", main="THE STOP", nums=[]),
    # 8. rev1: the revoked license (the fact the whole case rests on).
    dict(id="rev1", layout="CENTER_STACK", count_type="CT_TEXT", anchor=147.0, dur=5.5,
         top="GLOVER'S KANSAS LICENSE", hero_text="REVOKED",
         bottom="THE STATE HAD PULLED HIS PRIVILEGE TO DRIVE", still=None, nums=[]),
    # 9. nf1: a name on a screen vs a look at the driver.
    dict(id="nf1", layout="SPLIT_COMPARE", anchor=167.0, dur=5.5,
         top="ALL THE DEPUTY HAD", left="A NAME", right="A FACE", strike="UNCONFIRMED",
         bottom="A NAME ON A SCREEN - NOT A LOOK AT THE DRIVER", still=None, nums=[]),
    # 10. std1: reasonable suspicion sits between a hunch and proof.
    dict(id="std1", layout="CENTER_STACK", count_type="CT_TEXT", anchor=262.0, dur=5.5,
         top="NOT A HUNCH - NOT PROOF", hero_text="IN BETWEEN",
         bottom="SPECIFIC FACTS THAT POINT TOWARD SOMETHING WRONG", still=None, nums=[]),
    # 12. a_rule: ACT THREE title (the ruling).
    dict(id="a_rule", layout="ACT_TITLE_CARD", anchor=352.0, dur=5.5,
         top="ACT THREE", main="THE RULING", nums=[]),
    # 13. qT1: Thomas majority holding, verbatim, neutral attribution.
    dict(id="qT1", layout="QUOTE_CARD", anchor=376.0, dur=6.0,
         top="THE RULE THE COURT LAID DOWN",
         quote=("WHEN THE OFFICER LACKS INFORMATION NEGATING AN INFERENCE THAT THE OWNER "
                "IS DRIVING THE VEHICLE, THE STOP IS REASONABLE."),
         attribution="JUSTICE THOMAS, FOR THE COURT",
         attribution_canonical="Justice Thomas, for the Court", still=None, nums=[]),
    # 14. narrow1: the Court stressed the holding was NARROW.
    dict(id="narrow1", layout="CENTER_STACK", count_type="CT_TEXT", anchor=427.0, dur=5.5,
         top="THE COURT SAID ITS HOLDING WAS", hero_text="NARROW",
         bottom="IT DISSOLVES THE MOMENT THE FACTS CUT AGAINST IT", still=None, nums=[]),
    # 15. qK1: Kagan concurrence, verbatim, concurring attribution.
    dict(id="qK1", layout="QUOTE_CARD", anchor=486.0, dur=6.0,
         top="THE CONCURRENCE",
         quote=("CONSIDER, FOR EXAMPLE, IF KANSAS HAD SUSPENDED RATHER THAN REVOKED "
                "GLOVER'S LICENSE."),
         attribution="JUSTICE KAGAN, CONCURRING",
         attribution_canonical="Justice Kagan, concurring", still=None, nums=[]),
    # 16. cite1: the citation (verified G18; shown as static text, never a comma count-up).
    dict(id="cite1", layout="CENTER_STACK", count_type="CT_TEXT", anchor=568.0, dur=5.5,
         top="KANSAS v. GLOVER", hero_text="589 U.S.",
         bottom="NO. 18-556 - DECIDED APRIL 6, 2020", still=None, nums=[("G18", 589)]),
    # 17. a_your: ENDING title (your car).
    dict(id="a_your", layout="ACT_TITLE_CARD", anchor=602.0, dur=5.5,
         top="THE ENDING", main="YOUR CAR", nums=[]),
    # 18. edge1: a careful yes with a hard edge.
    dict(id="edge1", layout="CENTER_STACK", count_type="CT_TEXT", anchor=622.0, dur=5.5,
         top="AFTER 2020 THE ANSWER IS", hero_text="A CAREFUL YES",
         bottom="ONLY FOR THE BRIEF MOMENT OF NOT KNOWING", still=None, nums=[]),
]
...
def load_facts() -> dict:
    ...
    if isinstance(d.get("facts"), dict):
        return d["facts"]
    if isinstance(d.get("ledger"), list):
        return {str(row.get("id")): row for row in d["ledger"] if row.get("id")}
    return d
...
def clear(s: float, d: float, figs, placed, total: float, buf: float = 2.05) -> bool:
    e = s + d
...
def place(anchor: float, dur: float, figs, placed, total: float, window: float = 20.0) -> float:
    # keep a card within +/- `window` of its intended narration beat: search forward then
    # backward but never wander far from the anchor (a card shoved 100s away lands on the
    # wrong narration). Out-of-window => raise so the caller skips it cleanly.
    lo = 21.0
    if placed:
        lo = max(lo, placed[-1][1] + 1.0)
    start = max(anchor, lo)
    hi_bound = min(total - 12.0 - dur, anchor + window)
    s = start
    while s <= hi_bound:
...
def build(preview: bool) -> int:
    ...
    # place() advances a monotonic cursor (lo = last placed end + 1.0), so cards MUST be
    # processed in ascending-anchor order or an early-anchored card added after a late one
    # gets shoved to the end / fails to place. Sort the deck chronologically first.
    DECK.sort(key=lambda c: float(c["anchor"]))
    offset = round(8.0 + OPENING_SEC, 3)
    if FILM.exists():
        d = json.loads(FILM.read_text(encoding="utf-8"))
        offset = round(float(d.get("hookSeconds", 8.0)) + OPENING_SEC, 3)

    work = (ROOT / "episodes" / EP / "08_edit" / ("_preview/ae_hero" if preview else "ae_hero"))
    ...
    skipped_cards = []
    for c in DECK:
        # The film carries 36 dense Remotion figure-beats; AE hero cards must avoid them
        # (2.05s buffer). In congested stretches a card may have no free slot -- skip it
        # gracefully instead of failing the whole build, so every card that DOES fit ships.
        try:
            start = place(c["anchor"], float(c["dur"]), figs, placed, total)
        except SystemExit:
            skipped_cards.append(c["id"])
            print(f"[skip] {c['id']} anchor={c['anchor']} dur={c['dur']} - no clear slot (figure-dense)")
            continue
    ...
    doc = {
        "schema_version": "glover_ae_beats.v1",
        ...
        "offset_note": (
            ...
            "glover_film.json figures[]). Absolute time in the finished bookended "
        ...
        "accent": "#5B8DB8",
        "ledger_note": (
            "Every burned number is a glover_facts.v001.json verified:true G-ID "
            "value. All EP48 prohibited terms were asserted absent from every "
            "card string at build time (FORBIDDEN gate in the builder); this "
            "note is deliberately term-free so it cannot trip R-FORBID."),
    ...
    jsx = jsx.replace("__AEP__", str(media / "glover_hero.aep").replace("\\", "/"))
    (media / "glover_hero.jsx").write_text(jsx, encoding="utf-8-sig")

    print(f"[gen] {len(beats)} cards, total {sum(b['dur'] for b in beats):.1f}s"
          f" | offset={offset}s | preview={preview}")
    for b in beats:
        print(f"  {b['id']:<4} {b['layout']:<15} start={b['start']:>6.1f} "
              f"end={b['end']:>6.1f}  {b['hero']}")
    print("[gen] jsx  ->", media / "glover_hero.jsx")
    print("[gen] aep  ->", media / "glover_hero.aep")
    print("[gen] beats->", REPO_BEATS_DIR / "beats.json")
    return 0


JSX = r"""// AUTO-GENERATED - EP48 Glover AE hero cards. Do not hand-edit.
(function () {
  ...
  try { for (var ri = proj.numItems; ri >= 1; ri--) {
    ...
    if (itx instanceof CompItem && String(itx.name).indexOf("GLOVER_") === 0) itx.remove();
  } } catch (e) {}
  try { proj.gpuAccelType = GpuAccelType.SOFTWARE; } catch (e) {}
  try { proj.bitsPerChannel = 8; } catch (e) {}
  app.beginUndoGroup("glover_hero");
  ...
  function buildVote(comp, spec) {
    ...
    // 9 pips: 8 silver (majority) then 1 accent (dissent) -- Kansas v. Glover 8-1
  ...
  function build(spec) {
    var comp = proj.items.addComp("GLOVER_" + spec.id, W, H, 1.0, spec.dur, FPS);
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    raise SystemExit(build(args.preview))
