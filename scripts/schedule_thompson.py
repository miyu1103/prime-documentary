#!/usr/bin/env python3
"""Schedule EP41 thompson to YouTube (private, auto-publish at publishAt).
...
CONFIRM no existing thompson video -> resumable PRIVATE upload -> set thumbnail ->
schedule (status.publishAt) -> upload English caption -> re-query + verify.
If a PRIVATE thompson upload already exists (e.g. an agent died mid-upload, the EP40
...
re-uploading. Writes 09_package/youtube_schedule_result.v001.json.
    ...
    py -3.11 scripts/schedule_thompson.py --dry-run
    py -3.11 scripts/schedule_thompson.py
"""
...
VIDEO_FILE = EPDIR / "08_edit" / "thompson_final_bgm.v003_ae.mp4"
THUMB_FILE = PKG / "thumbnail.face.v001.png"
CAPTION_FILE = EPDIR / "08_edit" / "captions.final.v001.srt"
EXPECTED_HASH = "54350c2eabe2bb37b0f4909da7170358a4f89b736e05e6055a5389c4dfd1a82b"
PUBLISH_AT = "2026-07-26T03:00:00Z"
SCHEDULED_LOCAL = "2026-07-26T12:00:00+09:00"
...
def upload_caption(token, video_id):
    boundary = f"thompson_caption_{int(time.time())}"
...
def main(argv):
    ...
    # Guard 3: hash (final v002_ae; APR-0001 must bind to this exact sha)
    ...
    # Pre-flight: enumerate recent uploads, detect an existing thompson video (double-upload guard)
    print("  scanning recent channel uploads for an existing thompson video ...")
    recent = list_recent_uploads(token, ch, n=25)
    want_title = meta["title"]
    existing = [(vid, title, priv) for (vid, title, priv) in recent
                if title.strip() == want_title.strip()
                and "#shorts" not in title.lower()]
    for vid, title, priv in recent[:12]:
        flag = "  <-- MATCH" if (vid, title, priv) in existing else ""
        print(f"    [{priv:8}] {vid}  {title[:60]}{flag}")
    if existing:
        print(f"  NOTE: {len(existing)} possible existing thompson upload(s) found.")
    ...
    if args.dry_run:
        ...
        if existing:
            print("[dry-run] existing thompson video(s) present -> real run would FINALIZE the "
                  "private one instead of re-uploading:")
            for vid, title, priv in existing:
                print(f"          {priv} {vid} {title[:50]}")
        return 0

    # Decide: finalize an existing PRIVATE thompson upload, else fresh upload
    reused = False
    video_id = None
    ...
    if priv_existing:
        ...
        reused = True
        print(f"OK reusing existing PRIVATE thompson upload video_id={video_id} (no re-upload)")
        caps = existing_captions(token, video_id)
