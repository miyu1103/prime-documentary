#!/usr/bin/env python3
"""Render OneCoin v006 editorial cut.
...
AUDIO = REMOTION / "public" / "onecoin" / "audio" / "onecoin_final_mix_v004.wav"
MEDIA = Path("E:/pd-media")
OUT_DIR = MEDIA / "episodes" / EP / "07_edit"
CHAPTER_DIR = OUT_DIR / "chapters_v006"
CHUNK_DIR = OUT_DIR / "chapter_chunks_v006"
VISUAL_CONCAT = OUT_DIR / "onecoin_visual_concat_v006.mp4"
FINAL = OUT_DIR / "v006.mp4"
QC = EPDIR / "08_edit" / "renders" / "final.v006.qc.json"
EVENTS = EPDIR / "events" / "events.jsonl"
LOG = OUT_DIR / "render_onecoin_v006.log"
...
def scheduled_rows() -> tuple[list[dict], float]:
    ...
    if data.get("timelineMode") != "editorial":
        raise RuntimeError("v006 requires onecoin_roughcut.ts timelineMode='editorial'")
    ...
    for shot in data["shots"]:
        ...
        cursor += dur
        if shot["spanId"] == EDITORIAL_OPENING_AFTER_SPAN_ID:
            cursor += opening
    cursor += endcard
    return rows, cursor
    ...
    for i, item in enumerate(ranges, 1):
        ...
        while cursor <= item["end"]:
            ...
            run(
                ...
                f"render OneCoinPremium v006 chapter {i}/5 {item['chapter']} chunk {chunk_index} frames {cursor}-{chunk_end}",
                cwd=REMOTION,
            )
            cursor = chunk_end + 1
            chunk_index += 1
...
def concat_visual(chapters: list[Path]) -> None:
    list_file = CHAPTER_DIR / "concat_v006.txt"
    list_file.write_text("".join(f"file '{p.as_posix()}'\n" for p in chapters), encoding="utf-8")
    tmp = VISUAL_CONCAT.with_suffix(".tmp.mp4")
    run([FFMPEG, "-y", "-f", "concat", "-safe", "0", "-i", list_file, "-map", "0:v:0", "-c", "copy", tmp], "concat v006 chapter visuals")
...
def final_encode() -> None:
    ...
    run(
        ...
        "final v006 libx264 slow crf16 mux",
    )
    tmp.replace(FINAL)
...
def update_manifest() -> None:
    ...
    manifest["state"] = "editorial_v006_rendered_pending_owner_review"
    active = manifest.setdefault("active_revisions", {})
    active["final_render"] = "v006"
    active["final_qc"] = "v006"
    artifacts = manifest.setdefault("artifacts", [])
    for aid in ["PD-2026-017-onecoin-final-render-v006", "PD-2026-017-onecoin-final-qc-v006"]:
        artifacts[:] = [a for a in artifacts if a.get("artifact_id") != aid]
    artifacts.append(
        {
            "artifact_id": "PD-2026-017-onecoin-final-render-v006",
            "artifact_type": "final_render",
            "revision": "v006",
            "uri": "artifact://E:/pd-media/episodes/PD-2026-017-onecoin/07_edit/v006.mp4",
    ...
    artifacts.append(
        {
            "artifact_id": "PD-2026-017-onecoin-final-qc-v006",
            "artifact_type": "final_render_qc",
            "revision": "v006",
            "uri": "artifact://episodes/PD-2026-017-onecoin/08_edit/renders/final.v006.qc.json",
            "checksum": sha256(QC),
            "status": "candidate",
            "rights_status": "conditional",
...
def append_event() -> None:
    ...
    event = {
        ...
        "episode_id": EP,
        "stage": "render",
        "event": "editorial_v006_rendered",
        "revision": "v006",
        "actor": "codex",
        "note": "Rendered shorter OneCoin editorial cut v006. Natural-speed narration, 8.2s hook before opening, lighter captions, denser visual turnover. No upload/publish/schedule.",
    }
