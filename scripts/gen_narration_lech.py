#!/usr/bin/env python3
"""Generate PD-2026-040-lech narration with the channel voice (ElevenLabs).

Cloned from scripts/gen_narration_frazier.py (EP39, shipped). Paid API. Source of
truth = the FINAL script markdown `episodes/_planning/EP40_lech_script.en.v001.md`.
Like EP39, EP40 has no narration_index yet, so this runner EXTRACTS the spoken
narration out of the markdown and writes both:
  - episodes/<EP>/06_audio/voice_plan.v001.json     (same schema as rolin/frazier)
...
Everything below is DROPPED and never sent to TTS:
  - 〔CARD: ...〕 on-screen typography (WHOLE-LINE *and* INLINE spans -- EP40 puts
    some cards mid-paragraph, e.g. "Five thousand dollars. 〔CARD: $5,000〕 That...")
  - 【OST: ...】 on-screen text lines/spans
  - [SOUND: ...] / [Bookends: ...] / [Sxx] stage directions and still-image IDs
  - *(silence -- 1.5s ...)* beat lines (markdown italic parentheticals)
  - markdown headings (## HOOK ... ## 事実対応表), the frontmatter bullet block,
    horizontal rules, and every Japanese production note
  - the whole tail from `## 事実対応表` onward
A hard guard (assert_clean) re-scans every spoken_text for CJK characters and for
bracket/markup markers and REFUSES to spend money if any survive.

Voice is PINNED here (channel voice, measured ~176-178 wpm). Same voice + model as
EP39 -- do NOT swap it; the runtime length gate is calibrated on this voice.
...
EP = "PD-2026-040-lech"
SCRIPT_SRC = ROOT / "episodes" / "_planning" / "EP40_lech_script.en.v001.md"
SOURCE_SCRIPT_REL = "episodes/_planning/EP40_lech_script.en.v001.md"
...
VOICE_ID = "nPczCjzI2devNBz1zQrb"   # PINNED channel voice -- same as EP39, do not change
...
DELIVERY_BY_SECTION = {
    "HOOK": "intense",           # cold open, the house that held for nineteen hours
    "FLASH_FORWARD": "building", # eleven years later, the signature in a stranger's case
    "OPENING": "building",       # Prime Documentary -- two doors thesis
    "ACT_1": "building",         # 4219 South Alton Street, the barricade
    "ACT_2": "building",         # what it costs to be the address
    "ACT_3": "intense",          # police power, not eminent domain -- the rule
    "ACT_4": "building",         # the address is the only qualification (case survey)
    "ENDING": "calm",            # emotional payoff + earned CTA + next-ep teaser
}
SECTION_ORDER = ["HOOK", "FLASH_FORWARD", "OPENING", "ACT_1", "ACT_2", "ACT_3", "ACT_4", "ENDING"]
HEADING_TO_SECTION = {
    "HOOK": "HOOK",
    "FLASH-FORWARD": "FLASH_FORWARD",
    "OP": "OPENING",                 # brand thesis voiced after the 3.5s BrandOpening sting
    "ACT 1": "ACT_1", "ACT 2": "ACT_2", "ACT 3": "ACT_3", "ACT 4": "ACT_4",
    "ENDING": "ENDING",
    "ED": "ENDING",                  # endcard teaser voiced after the 9s BrandEndcard
}
# Started at EP39's 0.30/2.5. MEASURED 2026-07-20: at 0.30 the EP40 master ran 764.2s
# (185 chunks -> 177 beat gaps + 7 section breaths on top of 693.6s of speech), which is
# over the 750s narration ceiling with no headroom for the scripted [SILENCE] beats and
# BrandOpening/BrandEndcard bookends added at assembly. Tightened ONLY the mechanical
# inter-chunk beat (0.30 -> 0.16); the 7 section breaths stay at 2.5s (scripted craft on
# act boundaries). At 0.16 the master lands ~739s (ratio 1.066 of speech), under the
# ceiling with ~11s headroom, and speech (693.6s) already clears the 690s floor so there
# is no undershoot risk.
GAP_BEAT, GAP_SECTION = 0.16, 2.5
...
def extract_lines(md: str) -> list[tuple[str, str]]:
    ...
    for raw in md.splitlines():
        ...
        if line.startswith("#"):
            continue
        if line.startswith(DROP_PREFIX):   # 【OST】/〔CARD〕/[SOUND]/*(silence)*/quote/#
            continue
        if line.startswith("- ") or line.startswith("* "):
            continue
        line = clean_inline(line)           # remove any mid-paragraph 〔CARD〕/[..] spans
        if not line:
            continue
...
def concat_master(chunks: list[dict[str, str]], outdir: Path, master: Path) -> dict[str, dict]:
    ...
    sil_beat = outdir / "_silence_beat.mp3"
    sil_sec = outdir / "_silence_section.mp3"
...
def write_index(chunks: list[dict[str, str]], offsets: dict[str, dict],
    ...
    index = {
        ...
        "provenance": {
            "producer": "scripts/gen_narration_lech.py",
            "provider": "ElevenLabs",
            "voice_id": VOICE_ID,
            "model_id": MODEL,
...
def main(argv: list[str]) -> int:
    ...
    if args.dry_run:
        for c in chunks:
            print(f"  {c['chunk_id']} {c['section']:13s} {c['delivery']:8s} | {c['spoken_text']}")
        by: dict[str, int] = {}
        for c in chunks:
