#!/usr/bin/env python3
"""Upload EP7 Riley v005 180-plate burned-caption render to YouTube and schedule public release.
...
META = EPDIR / "09_package" / "youtube_meta.v005.json"
APR_ID = "APR-0007"
APR = EPDIR / "approvals" / f"{APR_ID}.json"
RIGHTS = EPDIR / "09_package" / "rights_manifest.v001.json"
FINAL_QC = EPDIR / "08_edit" / "renders" / "final.v005_180_burned_captions.qc.json"
VIDEO = Path(r"H:\pd-media\episodes\PD-2026-007-riley\08_edit\riley_final_v005_180_burned_captions.mp4")
MANIFEST = EPDIR / "manifest.json"
RESULT = EPDIR / "09_package" / "youtube_schedule_result.v005.json"
EVENTS = EPDIR / "events" / "events.jsonl"

EXPECTED_VIDEO = "e0fbb9e3155e2d849abaf1a847bff6eeeaf40497ad30a21011ed8b4124919bb9"
EXPECTED_THUMB = "70a81a4a7307eb0792cd55ca3145623a841a91e98d99c4a00b3a4656174cb9ba"
...
def update_local_package_state(result: dict) -> None:
    ...
    meta.setdefault("pre_publish_checks", {})["supersedes_video_id"] = "UGUAqVqTtRQ"
    meta.setdefault("pre_publish_checks", {})["supersedes_previous_video_id"] = "u17UoII_hz8"
    ...
    manifest.setdefault("active_revisions", {})["edit"] = "v005"
    manifest.setdefault("active_revisions", {})["captions_final"] = "v005"
    manifest.setdefault("active_revisions", {})["final_qc"] = "v005"
    manifest.setdefault("active_revisions", {})["final_delivery"] = "v005"
    manifest.setdefault("active_revisions", {})["final_render"] = "v005"
    manifest.setdefault("active_revisions", {})["youtube_meta"] = "v005"
    manifest.setdefault("active_revisions", {})["youtube_schedule_result"] = "v005"
    approvals = manifest.setdefault("approvals", [])
    if APR_ID not in approvals:
        approvals.append(APR_ID)
    warning = f"YouTube private upload completed for Riley v005 180-plate burned-caption render and scheduled under {APR_ID}: video_id {result['video_id']}, public release {SCHEDULED_AT_LOCAL}. Riley v004 UGUAqVqTtRQ and v003 u17UoII_hz8 were paused before this replacement upload."
    if warning not in manifest.setdefault("warnings", []):
        manifest["warnings"].append(warning)
    update_artifact_checksum(manifest, f"{EP}-final-qc-v005", FINAL_QC, "qc_report")
    update_artifact_checksum(manifest, f"{EP}-final-delivery-v005", EPDIR / "09_package" / "final_delivery.v005.json", "final_delivery_manifest")
    update_artifact_checksum(manifest, f"{EP}-final-render-v005", VIDEO, "final_render", "v005")
    update_artifact_checksum(manifest, f"{EP}-youtube-meta-v005", META, "youtube_metadata")
    update_artifact_checksum(manifest, f"{EP}-approval-{APR_ID}", APR, "approval")
    update_artifact_checksum(manifest, f"{EP}-youtube-schedule-result-v005", RESULT, "youtube_schedule_result")
    write_json(MANIFEST, manifest)


def verify_preconditions() -> tuple[dict, dict, Path]:
    if RESULT.exists():
        raise RuntimeError(f"Existing Riley v005 schedule result found; refusing duplicate upload: {RESULT.relative_to(ROOT)}")
    if not APR.exists():
...
def main(argv: list[str]) -> int:
    ...
    append_event({
        ...
        "actor": "codex",
        "approval_id": APR_ID,
        "detail": f"Uploaded EP7 Riley v005 180-plate burned-caption render to YouTube as private video_id={video_id} and scheduled public release for {SCHEDULED_AT_LOCAL}. Selected thumbnail set. Riley v004 UGUAqVqTtRQ and v003 u17UoII_hz8 remain private and unscheduled after replacement. No public immediate publish performed.",
        "video_id": video_id,
        "watch": f"https://youtu.be/{video_id}",
        "studio": f"https://studio.youtube.com/video/{video_id}/edit",
        "scheduled_at_local": SCHEDULED_AT_LOCAL,
