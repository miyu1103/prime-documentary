#!/usr/bin/env python3
"""EP46 tlo: replace the baked narration with the authoritative master VO and add a
ducked 2-layer BGM + ambience bed, two-pass loudnorm to -14 LUFS, mux into the film render.

Film layout = hook(8.0) + OPENING(3.5) + narration(~672.4) + ENDCARD. Body/VO starts at
...
sidechain. Never overwrites the input render. Cloned verbatim from the verified
build_cleveland_bgm_real.py (only the IDX/VO paths and work-file name change to EP46).

Usage: python scripts/build_tlo_bgm_real.py <render_in.mp4> <final_out.mp4>
"""
...
IDX = ROOT / "episodes" / "PD-2026-046-tlo" / "06_audio" / "narration_index.v001.json"
VO = Path(r"H:/pd-media/episodes/PD-2026-046-tlo/06_voice/master/vc_master_v001.mp3")
...
def main() -> int:
    ...
    # EP46 tlo is a 3-act film (HOOK/OP/ACT_1/ACT_2/ACT_3/ENDING) -- no ACT_4.
    ...
    work = out.parent / "_tlo_bgm_pre.wav"
