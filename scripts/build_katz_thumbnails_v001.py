#!/usr/bin/env python3
r"""Build EP26 Katz CTR thumbnails (1280x720) with the proven Kyllo visual system.
...
options to episodes/PD-2026-026-katz/10_thumbnail/ and copies the selected CTR option
...
W, H = 1280, 720
EP = ROOT / "episodes" / "PD-2026-026-katz"
OUTDIR = EP / "10_thumbnail"
HERO = ROOT / "remotion" / "public" / "katz" / "img" / "COD-S001-booth-glow-night.png"
...
OPTIONS = [
    ("1967 · NO WARRANT NEEDED?", "THEY RECORDED", "EVERY WORD", None),      # CTR selected
    ("THE FBI WAS LISTENING", "IS YOUR CALL", "PRIVATE?", None),                  # curiosity
    ("KATZ v. UNITED STATES", "THEY BUGGED", "THE BOOTH", None),                  # paradox
]
SELECTED = 0
...
def main():
    ...
    print(f"[katz] {len(made)} options @ 1280x720, selected=YES")
