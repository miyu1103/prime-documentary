#!/usr/bin/env python
"""Create the EP42 BGM base placeholder for dry-run or validate an existing render."""
...
def make_base(path: Path, dur: float = 733.4) -> None:
...
def main() -> int:
    ...
    args = ap.parse_args()
    if args.dryrun:
        out = args.out or EDIT / "_dryrun" / "young_final_bgm.v002.mp4"
        make_base(out)
        print(f"wrote {out}")
        return 0
    render = args.render or EDIT / "young_casefilm.v001.mp4"
    out = args.out or EDIT / "young_final_bgm.v002.mp4"
