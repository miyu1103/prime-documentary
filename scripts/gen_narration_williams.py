#!/usr/bin/env python3
"""Generate PD-2026-036-williams narration with the channel voice (ElevenLabs).
...
Modeled byte-for-byte on scripts/gen_narration_hinders.py; the ONLY differences are
(1) EP = PD-2026-036-williams and (2) DELIVERY_BY_SECTION keyed on THIS episode's
narration_index section names (chapter `function` uppercased). Voice id is read from
.env ELEVENLABS_VOICE_ID at generation time (channel voice = Brian, nPczCjzI2devNBz1zQrb).
"""
...
# section (this episode's narration_index; chapter `function` uppercased) -> delivery arc.
# Arc mirrors the channel default (carsearch/hinders): threat cold-open + measured-pattern
# reveal ride hotter; setup/mechanism/rule build; earned CTA settles calm.
DELIVERY_BY_SECTION = {
    "COLD_OPEN_THREAT": "intense",          # hook
    "OP_TITLE_CARD_AND_THESIS": "building",  # opening thesis
    "ARREST_AND_UMBRELLA_LOOP": "building",  # act 1
    "THE_MATCH_REVEAL": "building",          # act 2
    "THE_PATTERN_AND_THREAT": "intense",     # act 3 (bias reveal / re-hook threat)
    "RULE_AND_SCALE": "building",            # act 4
    "EARNED_CTA_AND_NEXT_ARC": "calm",       # ending / CTA
}
...
def parse_chunks(index: dict) -> list[dict[str, str]]:
    ...
    for c in index["chunks"]:
        section = c.get("section", "ARREST_AND_UMBRELLA_LOOP")
