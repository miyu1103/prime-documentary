#!/usr/bin/env python3
"""Generate SHORT #20 (Gardner Heist / "still missing") vertical-short narration with the channel voice.
...
R2 / sensitive: unsolved crime, third parties. WORDING LOCKS (do not deviate):
  - NEVER name or imply the guilt of any person; the FBI never publicly named the thieves.
  - "$500M / half a billion" is an ESTIMATE, never a sale/exact worth.
  - "biggest/largest" is an ATTRIBUTED characterization ("it's called"), never flat fact.
  - The guard who buzzed them in is NOT portrayed as complicit.
...
# (line_id, delivery, spoken_text) — from SHORTS_EP19-24.md SHORT #20 time table, with R2 wording locks applied.
LINES: list[tuple[str, str, str]] = [
    ("L1", "intense", "It's called the biggest art heist in history — and the empty frames still hang on the wall."),
    ("L2", "building", "March 18, 1990, Boston. Two men dressed as police talked their way into the Isabella Stewart Gardner Museum, handcuffed the guards in the basement, and were inside about eighty-one minutes."),
    ("L3", "building", "They took thirteen works — cutting a Rembrandt and a Vermeer right out of their frames. The haul is estimated around half a billion dollars, though the art is unsellable. Not one of the thirteen has ever been found."),
    ("L4", "intense", "The FBI says it believes it knows who did it, but never named them, and says they are now dead. The time to charge the theft has run out; there's a standing ten-million-dollar reward. And the empty frames still hang exactly where the paintings were."),
...
def main() -> int:
    ...
    master = media_root() / "episodes" / EP / "06_voice" / "master" / "short20_vc_master_en_us_v002.mp3"
    master.parent.mkdir(parents=True, exist_ok=True)
    subprocess.run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(concat),
                    "-c:a", "libmp3lame", "-b:a", "192k", str(master)], check=True, capture_output=True)

    idx = ROOT / "episodes" / EP / "06_audio" / "short20_narration_index.v002.en_us.json"
