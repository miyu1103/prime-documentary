#!/usr/bin/env python3
"""Schedule EP43 tekoh to YouTube (private, auto-publish at publishAt).
...
CONFIRM no existing tekoh video -> resumable PRIVATE upload -> set thumbnail ->
schedule (status.publishAt) -> upload English caption -> re-query + verify.
If a PRIVATE tekoh upload already exists (e.g. an agent died mid-upload, the EP40/EP41
...
run; until then the sha256 guard fails closed. APR ref: APR-0001 (approvals/APR-0001.json).
    ...
    py -3.11 scripts/schedule_tekoh.py --dry-run
    py -3.11 scripts/schedule_tekoh.py
"""
...
VIDEO_FILE = EPDIR / "08_edit" / "tekoh_final_bgm.v003_ae.mp4"
THUMB_FILE = PKG / "thumbnail.selected.v001.png"
CAPTION_FILE = EPDIR / "08_edit" / "captions.final.v001.srt"
EXPECTED_HASH = "e0a66abad99871fa26246671e36c829cd2197bf5a76d23f2059eee5f898f71ce"
PUBLISH_AT = "2026-07-29T03:00:00Z"
SCHEDULED_LOCAL = "2026-07-29T12:00:00+09:00"
...
def upload_caption(token, video_id):
    boundary = f"tekoh_caption_{int(time.time())}"
...
def main(argv):
    ...
    # Pre-flight: enumerate recent uploads, detect an existing tekoh video (double-upload guard)
    print("  scanning recent channel uploads for an existing tekoh video ...")
    recent = list_recent_uploads(token, ch, n=25)
    want_title = meta["title"]
    existing = [(vid, title, priv) for (vid, title, priv) in recent
                if is_tekoh_match(title, want_title)]
    for vid, title, priv in recent[:12]:
        flag = "  <-- MATCH" if (vid, title, priv) in existing else ""
        print(f"    [{priv:8}] {vid}  {title[:60]}{flag}")
    if existing:
        print(f"  NOTE: {len(existing)} possible existing tekoh upload(s) found.")
    ...
    if args.dry_run:
        ...
        if existing:
            print("[dry-run] existing tekoh video(s) present -> real run would FINALIZE the "
                  "private one instead of re-uploading:")
            for vid, title, priv in existing:
                print(f"          {priv} {vid} {title[:50]}")
        return 0

    # Decide: finalize an existing PRIVATE tekoh upload, else fresh upload
    reused = False
    video_id = None
    ...
    if priv_existing:
        ...
        reused = True
        print(f"OK reusing existing PRIVATE tekoh upload video_id={video_id} (no re-upload)")
        caps = existing_captions(token, video_id)
