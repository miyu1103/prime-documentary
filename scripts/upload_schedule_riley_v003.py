#!/usr/bin/env python3
"""Upload EP7 Riley v003 to YouTube and schedule public release.
...
VIDEO = Path(r"H:\pd-media\episodes\PD-2026-007-riley\08_edit\riley_final_v003.mp4")
MANIFEST = EPDIR / "manifest.json"
RESULT = EPDIR / "09_package" / "youtube_schedule_result.v003.json"
EVENTS = EPDIR / "events" / "events.jsonl"

EXPECTED_VIDEO = "f53f6555cd59601231042512dc2490c6600023bdd92eefab73b074208f56590d"
...
def update_local_package_state(result: dict) -> None:
    ...
    manifest.setdefault("active_revisions", {})["youtube_schedule_result"] = "v003"
    approvals = manifest.setdefault("approvals", [])
    if APR_ID not in approvals:
        approvals.append(APR_ID)
    warning = f"YouTube private upload completed for Riley v003 and scheduled under {APR_ID}: video_id {result['video_id']}, public release {SCHEDULED_AT_LOCAL}. Previous v001 video ODCh4-UelI4 and v002 video 6iWtKdktOGY were kept private and unscheduled."
...
def verify_preconditions() -> tuple[dict, dict, Path]:
    if RESULT.exists():
        raise RuntimeError(f"Existing Riley v003 schedule result found; refusing duplicate upload: {RESULT.relative_to(ROOT)}")
    if not APR.exists():
...
def main(argv: list[str]) -> int:
    ...
    append_event({
        ...
        "actor": "codex",
        "approval_id": APR_ID,
        "detail": f"Uploaded EP7 Riley v003 to YouTube as private video_id={video_id} and scheduled public release for {SCHEDULED_AT_LOCAL}. Selected thumbnail set. Previous v001 upload ODCh4-UelI4 and v002 upload 6iWtKdktOGY remain private and unscheduled. No public immediate publish performed.",
        "video_id": video_id,
        "watch": f"https://youtu.be/{video_id}",
        "studio": f"https://studio.youtube.com/video/{video_id}/edit",
        "scheduled_at_local": SCHEDULED_AT_LOCAL,
