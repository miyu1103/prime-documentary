#!/usr/bin/env python3
"""EP49 strieff: composite the rendered After-Effects hero cards onto the finished BGM film
...
overwrites the input. Cloned verbatim from the verified composite_glover_hero.py /
composite_tlo_hero.py (only the BEATS path changes to the EP49 deck).

Usage: python scripts/composite_strieff_hero.py <bgm_in.mp4> <out_v002_ae.mp4>
"""
...
def main() -> int:
    if len(sys.argv) != 3:
        print("Usage: composite_strieff_hero.py <bgm_in.mp4> <out_v002_ae.mp4>", file=sys.stderr); return 2
