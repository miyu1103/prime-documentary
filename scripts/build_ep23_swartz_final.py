#!/usr/bin/env python3
"""Build everything for EP23 that does not require final hero image insertion.
...
The full final render is intentionally blocked until EP23-IMG-001..054 exist.
...
PUBLIC = ROOT / "remotion" / "public" / "swartz"
FACTORY = PUBLIC / "factory"
FACTORY_STILLS = WORK / "factory_stills"
...
MIX_WAV = EP_MEDIA / "08_edit" / f"swartz_final_mix.{AUDIO_REV}.wav"
SILENT_MP4 = WORK / f"swartz_silent_motion.{VIDEO_REV}.mp4"
...
def parse_vo() -> list[dict[str, Any]]:
    ...
    if len(rows) != 49:
        raise RuntimeError(f"expected 49 VO rows, got {len(rows)}")
    return rows
...
def preflight(images_required: bool = False) -> dict[str, Any]:
    ...
    spans = ann.get("spans", [])
    if len(spans) != 49:
        raise RuntimeError(f"expected 49 annotated spans, got {len(spans)}")
    selected = [SELECTED / f"EP23-IMG-{i:03d}.png" for i in range(1, 55)]
...
def synthesize_chunk(row: dict[str, Any], api_key: str) -> dict[str, Any]:
    ...
    idem = f"pd-2026-020-swartz-{VOICE_REV}-{row['span_id']}-{req_hash[:16]}"
    started = {
        "episode_id": EP,
        "span_id": row["span_id"],
        "revision": VOICE_REV,
...
def select_factory_assets(limit: int = 72) -> list[dict[str, Any]]:
    themes = {"surveillance_tech", "legal_court", "crime_police", "documents_paper", "urban_night", "abstract"}
        ...
        staged.append(
            {
                ...
                "remotion_src": f"swartz/factory/{dest.name}",
        ...
        json.dumps(
            {
                "episode_id": EP,
                "revision": "v001",
                "selection_rule": "EP23 non-hero b-roll shelf: tech/legal/crime/documents/urban/abstract; no real-person likeness, real logos, seals, buildings, or self-harm imagery.",
                "count": len(staged),
                "assets": staged,
                "created_at": now(),
            },
...
def write_remotion_roughcut(factory_assets: list[dict[str, Any]]) -> None:
        ...
        shots.append(
            {
                ...
                "searchKeywords": ["tech", "legal", "crime", "documents", "open access"],
    ...
    data = {
        ...
        "openingTitle": "THE INTERNET'S OWN BOY",
        "openingSubtitle": "Aaron Swartz and the open web",
        "fps": FPS,
        "narrationSrc": None,
        "bgmSrc": None,
        "timelineMode": "editorial",
        "captions": [{"id": f"ep23-caption-{cue['index']:04d}", "start": cue["start"], "end": cue["end"], "text": cue["text"]} for cue in captions],
        "shots": shots,
    }
    out = ROOT / "remotion" / "src" / "data" / "swartz_roughcut.ts"
    out.write_text(
        "import type {RoughCutData} from '../compositions/RoughCut';\n\n"
        "// EP23 scaffold. Hero image insertion waits for EP23-IMG-001..054.\n"
        "export const SWARTZ_ROUGHCUT: RoughCutData = "
        + ts_string(data)
        + ";\n",
        encoding="utf-8",
    )
...
def write_package_drafts(preflight_report: dict[str, Any]) -> None:
    pkg = EPDIR / "09_package"
    pkg.mkdir(parents=True, exist_ok=True)
    (pkg / "description.v001.md").write_text(
        "\n".join(
            [
                "The Internet's Own Boy: Aaron Swartz",
                "",
                "A Prime Documentary episode about open knowledge, the law, and the questions Aaron Swartz left behind.",
                "",
                "988 Suicide & Crisis Lifeline -- call or text 988",
                "",
                "Owner review required before publishing: R3 legal and safe-handling review, source hash recheck, title/thumbnail approval, and final publish approval.",
                "",
            ]
        ),
        encoding="utf-8",
        ...
        json.dumps(
            {
                ...
                "title_options": [
                    {"option": "A", "title": "The Internet's Own Boy: Aaron Swartz", "length": 38, "thumbnail_headline": "WHO OWNS KNOWLEDGE?"},
                    {"option": "B", "title": "13 Felonies and the Open Internet", "length": 34, "thumbnail_headline": "13 FELONIES"},
                    {"option": "C", "title": "Aaron Swartz and the Door He Tried to Open", "length": 45, "thumbnail_headline": "THE OPEN DOOR"},
                ],
                "r3_sensitive_locks": [
                    "death by suicide stated only by the locked script; no method, location, note, scene, or reenactment",
                    "no single-cause narrative outside the family's attributed statement",
                    "35 years never appears unqualified; pair with theoretical maximum, six-month plea signal, and Kerr critique",
                    "988 Suicide & Crisis Lifeline -- call or text 988 in the description and on marked lower-thirds",
                    "no real-person likeness, real logo, seal, identifiable building, readable baked text, or self-harm/death imagery",
                ],
                "created_at": now(),
            },
            indent=2,
        ...
        json.dumps(
            {
                ...
                "known_pending": [
                    f"EP23-IMG-{preflight_report['hero_images_present'] + 1:03d}..054 still pending in selected directory",
                    "hero image insertion into Remotion data",
                    "3 thumbnail PNGs plus selected",
                    "final.mp4 render",
                    "independent check_final_acceptance.py 23 --json exit 0",
                ],
                "preflight": preflight_report,
                "created_at": now(),
            },
...
def write_burn_ass(srt_path: Path) -> Path:
    ...
    ass = "\n".join(
        [
            ...
            "Style: Default,Arial,38,&H00F5F7FA,&H000000FF,&H00000000,&HAA000000,0,0,0,0,100,100,0,0,3,1.0,0,2,230,230,34,1",
            "",
            "[Events]",
...
def hero_images_ready() -> bool:
    return all((SELECTED / f"EP23-IMG-{i:03d}.png").exists() for i in range(1, 55))


def stage_visuals() -> list[Path]:
    if not hero_images_ready():
        missing = [f"EP23-IMG-{i:03d}.png" for i in range(1, 55) if not (SELECTED / f"EP23-IMG-{i:03d}.png").exists()]
        raise RuntimeError(f"missing hero images: {missing[:20]}")
    PUBLIC.mkdir(parents=True, exist_ok=True)
    staged: list[Path] = []
    low: list[str] = []
    for i in range(1, 55):
        src = SELECTED / f"EP23-IMG-{i:03d}.png"
        ...
        json.dumps(
            {
                ...
                "r3_sensitive_safety": "symbolic reconstruction; no real-person likeness, real logo/seal/building, readable baked text, self-harm/death imagery, method, location, note, or reenactment",
                "created_at": now(),
            },
            indent=2,
            ensure_ascii=False,
...
def lift_shadow_floor(frame: np.ndarray) -> np.ndarray:
    floor = np.array([38, 42, 52], dtype=np.uint8)
    return np.maximum(frame, floor)
...
def support_resource_windows(total: float) -> list[tuple[float, float]]:
    if not NARRATION_INDEX.exists():
        return [(120.0, 138.0), (max(0.0, total - 240.0), max(0.0, total - 218.0))]
    chunks = load_json(NARRATION_INDEX).get("chunks", [])
    by_span = {chunk.get("span_id"): chunk for chunk in chunks}
    windows: list[tuple[float, float]] = []
    for span_id, hold in [("SPN-0007", 20.0), ("SPN-0039", 18.0)]:
        chunk = by_span.get(span_id)
        if not chunk:
            continue
        start = max(0.0, float(chunk.get("voice_start", 0.0)) - 1.0)
        end = min(total, float(chunk.get("voice_end", start)) + hold)
        windows.append((start, end))
    return windows


def draw_support_lower_third(frame: np.ndarray, t: float, windows: list[tuple[float, float]]) -> np.ndarray:
    active = next(((start, end) for start, end in windows if start <= t <= end), None)
    if not active:
        return frame
    start, end = active
    fade = min(1.0, max(0.0, (t - start) / 0.8), max(0.0, (end - t) / 0.8))
    alpha = int(218 * fade)
    img = Image.fromarray(frame)
    d = ImageDraw.Draw(img, "RGBA")
    x0, y0, x1, y1 = 322, 804, 1598, 884
    d.rounded_rectangle((x0, y0, x1, y1), radius=18, fill=(5, 12, 24, alpha), outline=(229, 181, 58, int(130 * fade)), width=2)
    d.text(
        ((x0 + x1) // 2 + 20, (y0 + y1) // 2),
        "988 Suicide & Crisis Lifeline -- call or text 988",
        anchor="mm",
        font=font(34, True),
        fill=(245, 247, 250, int(245 * fade)),
    )
    return np.asarray(img, dtype=np.uint8)
...
def draw_bookends(frame: np.ndarray, t: float, total: float) -> np.ndarray:
    ...
    glow = int(160 * pulse)
    d.ellipse((520, 340, 1400, 1220), outline=(*GOLD, 58), width=4)
    d.ellipse((690, 238, 1230, 778), outline=(*GOLD, 92), width=8)
    ...
    if opening:
        ...
        draw_center_text(d, (W // 2, 690), "THE INTERNET'S OWN BOY", 94, (*WHITE, int(255 * title_p)), True, 2)
        draw_center_text(d, (W // 2, 776), "Aaron Swartz and the open web", 31, (*GOLD, int(240 * title_p)), False, 1)
    else:
        ...
        draw_center_text(d, (W // 2, 704), "988 SUICIDE & CRISIS LIFELINE - CALL OR TEXT 988", int(35 * cta_scale), (*GOLD, 250), True, 1)
        draw_center_text(d, (W // 2, 762), "In memory of Aaron Swartz, 1986-2013", 34, (*SILVER, 236), False, 1)
    ...
    d.rectangle((0, wave_y, W, wave_y + 4), fill=(*GOLD, 32))
    if opacity < 1:
...
def extract_factory_stills(factory_assets: list[dict[str, Any]], limit: int = 72) -> list[Path]:
    FACTORY_STILLS.mkdir(parents=True, exist_ok=True)
    stills: list[Path] = []
    for idx, item in enumerate(factory_assets[:limit], start=1):
        if item.get("kind") != "video":
            continue
        src_value = item.get("source_path") or item.get("path", "")
        src = Path(src_value) if re.match(r"^[A-Za-z]:[\\/]", str(src_value)) else resolve_factory_path(str(src_value))
        if not src.exists():
            continue
        out = FACTORY_STILLS / f"{idx:03d}_{item.get('asset_id', 'factory')}.jpg"
            ...
            try:
                run(
                    [
                        ...
                        "1",
                        "-vf",
                        "scale=2160:1215:force_original_aspect_ratio=increase,crop=2160:1215",
                        "-update",
                        "1",
                        "-q:v",
                        "2",
                    ...
                    f"extract factory still {idx:03d}",
                    timeout=60,
                )
...
def render_silent_video(paths: list[Path], total: float, factory_paths: list[Path] | None = None) -> None:
    WORK.mkdir(parents=True, exist_ok=True)
    frames = int(math.ceil(total * FPS))
    shot_len = 5.2
    transition = 0.42
    cache = FrameCache(paths)
    factory_cache = FrameCache(factory_paths or [], max_items=10) if factory_paths else None
    cmd = [
        FFMPEG,
        "-y",
        "-f",
    ...
    support_windows = support_resource_windows(total)
    try:
        for n in range(frames):
            ...
            if t < 8.0:
                ...
                base_idx = 4 + shot
                use_factory = bool(factory_cache) and shot % 6 == 2
                if use_factory:
                    cur = crop_motion(factory_cache.get(shot // 6), t, shot, local, shot_len)
                    hero = crop_motion(cache.get(base_idx), t, shot, local, shot_len)
                    cur = (cur.astype(np.float32) * 0.82 + hero.astype(np.float32) * 0.18).astype(np.uint8)
                else:
                ...
                if local >= shot_len - transition:
                    ...
                    next_use_factory = bool(factory_cache) and (shot + 1) % 6 == 2
                    if next_use_factory:
                        nxt = crop_motion(factory_cache.get((shot + 1) // 6), t, shot + 1, 0.0, shot_len)
                        hero_next = crop_motion(cache.get(base_idx + 1), t, shot + 1, 0.0, shot_len)
                        nxt = (nxt.astype(np.float32) * 0.82 + hero_next.astype(np.float32) * 0.18).astype(np.uint8)
                    else:
            ...
            cur = lift_shadow_floor(cur)
            cur = draw_caption_safe_grade(cur)
            cur = draw_support_lower_third(cur, t, support_windows)
...
def make_thumbnail(option: str, headline: str, bg: Path, out: Path) -> None:
    ...
    d.text((232, 67), "AARON SWARTZ", anchor="mm", font=font(30, True), fill=(7, 13, 24, 255))
    ...
    d.text((62, 636), "THE INTERNET'S OWN BOY", font=font(31, True), fill=(200, 205, 214, 245))
...
def build_thumbnails(paths: list[Path]) -> None:
    ...
    pkg = EPDIR / "09_package"
    variants = [
        ("A", "WHO OWNS KNOWLEDGE?", paths[50]),
        ("B", "13 FELONIES", paths[32]),
        ("C", "THE OPEN DOOR", paths[52]),
    ]
    records = []
    for option, headline, bg in variants:
        out = thumb_dir / f"thumbnail.swartz_option_{option}.v001.png"
    ...
    shutil.copy2(thumb_dir / "thumbnail.swartz_option_A.v001.png", selected)
        ...
        json.dumps(
            {
                ...
                "selected": "A",
                "titles": [
                    {"option": "A", "title": "The Internet's Own Boy: Aaron Swartz", "length": 38},
                    {"option": "B", "title": "13 Felonies and the Open Internet", "length": 34},
                    {"option": "C", "title": "Aaron Swartz and the Door He Tried to Open", "length": 45},
                ],
                "thumbnails": records,
                "r3_sensitive_safety": "respectful; no face or real-person likeness; no real logo/seal/building; no self-harm/death imagery; no suicide/death wording; no single-cause claim",
                "created_at": now(),
            },
            indent=2,
            ensure_ascii=False,
...
def update_rights_for_audio_and_factory(factory_assets: list[dict[str, Any]], hero_paths: list[Path] | None = None) -> None:
    ...
    additions = [
        {
            "asset_id": "EP23-VOICE-MASTER-v001",
            "type": "narration",
        ...
        additions.append(
            {
                "asset_id": path.stem,
                "type": "hero_still",
                "origin": "Codex AI-generated symbolic reconstruction selected for EP23",
                "license": "AI-generated under account terms",
                "path": str(path).replace("\\", "/"),
                "remotion_src": f"swartz/{path.name}",
                "ai_disclosure": True,
                "r3_sensitive_safety": "no real-person likeness; no real logo/seal/building; no readable baked text; no self-harm/death imagery; no method/location/note/reenactment",
            }
        )
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Build EP23 non-image-dependent assets.")
    ...
    if args.audio_only or args.all_available:
        rows = parse_vo()
        print(f"== EP23 VO rows: {len(rows)}", flush=True)
    ...
    if args.prep_remotion or args.all_available or args.final:
        ...
        append_event(
            "remotion_scaffold_prepared",
            {
                ...
                "roughcut": "remotion/src/data/swartz_roughcut.ts",
    ...
    if args.final:
        ...
        if not active_captions_srt().exists():
            raise RuntimeError("final captions missing; run force_align_ep20_swartz_captions.py first")
        paths = stage_visuals()
        build_thumbnails(paths)
        total = ffprobe_duration(MIX_WAV)
        factory_stills = extract_factory_stills(factory_assets)
        render_silent_video(paths, total, factory_stills)
        burn_and_mux(total)
