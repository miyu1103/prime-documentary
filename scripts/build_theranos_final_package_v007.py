#!/usr/bin/env python3
"""Build the Theranos final v007 package records.
...
def write_rights_manifest(video_sha: str, captions_sha: str, thumb_sha: str, final_duration: float) -> Path:
    ...
    out = PKG / "rights_manifest.v007.json"
...
def write_youtube_meta(video_sha: str, captions_sha: str, thumb_sha: str) -> Path:
    ...
    out = PKG / "youtube_meta.v007.json"
    write_json(out, data)
    (PKG / "title.v007.txt").write_text(TITLE + "\n", encoding="utf-8")
    write_json(PKG / "chapters.v007.json", {"episode_id": EP, "revision": "v007", "chapters": chapters()})
    write_json(PKG / "tags.v007.json", {"episode_id": EP, "revision": "v007", "tags": tags()})
    return out
...
def write_final_delivery(video_sha: str, captions_sha: str, thumb_sha: str, probe: dict[str, Any], rights: Path, meta: Path) -> Path:
    ...
    out = PKG / "final_delivery.v007.json"
    write_json(out, data)
    return out
...
def write_review_packets(video_sha: str, captions_sha: str, thumb_sha: str, final_delivery: Path, rights: Path, meta: Path) -> tuple[Path, Path, Path]:
    ...
    legal = {
        ...
        "r3_review_points": [
            ...
            "Factory/stock materials are generic illustrative assets with provenance in rights_manifest.v007.",
        ],
    }
    legal_json = PKG / "legal_rights_review_packet.v007.json"
    write_json(legal_json, legal)
    legal_md = PKG / "legal_rights_review_packet.v007.md"
    legal_md.write_text(
        "# Theranos v007 R3 Legal/Rights Review Packet\n\n"
    ...
    owner_json = PKG / "OWNER_REVIEW_REQUEST.v007.json"
    write_json(owner_json, owner)
    owner_md = PKG / "OWNER_REVIEW_REQUEST.v007.md"
    owner_md.write_text(
        "# OWNER_REVIEW_REQUEST v007 - Theranos Final Candidate\n\n"
        f"- Final video: `{str(FINAL_VIDEO).replace(chr(92), '/')}`\n"
        f"- SHA-256: `{video_sha}`\n"
    ...
    approval = {
        ...
        "external_side_effects": {"paid_api": False, "upload": False, "publish": False, "schedule": False},
        "drafts": [
            {"draft_id": "APR-0002-DRAFT", "gate": "final_review_video", "decision": "pending_owner_decision", "target_revision": "v007", "video": str(FINAL_VIDEO).replace("\\", "/"), "video_sha256": video_sha, "owner_review_request": rel(owner_json), "owner_review_request_sha256": sha256(owner_json)},
            {"draft_id": "APR-0003-DRAFT", "gate": "title_thumbnail", "decision": "pending_owner_decision", "target_revision": "v007", "title": TITLE, "thumbnail": rel(THUMB), "thumbnail_sha256": thumb_sha, "youtube_meta": rel(meta), "youtube_meta_sha256": meta_sha},
            {"draft_id": "APR-0004-DRAFT", "gate": "final_narration_release", "decision": "pending_owner_decision", "target_revision": "v007", "narration_index": rel(NARR_INDEX), "audio_mix": rel(AUDIO_MIX), "provider": "ElevenLabs"},
            {"draft_id": "APR-0005-DRAFT", "gate": "legal_rights", "decision": "pending_owner_or_legal_decision", "target_revision": "v007", "legal_rights_review_packet": rel(legal_json), "legal_rights_review_packet_sha256": sha256(legal_json), "rights_manifest": rel(rights), "rights_manifest_sha256": rights_sha},
            {"draft_id": "APR-0006-DRAFT", "gate": "publish_schedule", "decision": "pending_owner_decision", "target_revision": "v007", "video": str(FINAL_VIDEO).replace("\\", "/"), "video_sha256": video_sha, "final_delivery": rel(final_delivery), "final_delivery_sha256": fd_sha, "youtube_meta": rel(meta), "youtube_meta_sha256": meta_sha, "rights_manifest": rel(rights), "rights_manifest_sha256": rights_sha},
        ],
    }
    approval_json = PKG / "approval_drafts.v007.json"
    write_json(approval_json, approval)
    (PKG / "approval_drafts.v007.md").write_text(
        "# Theranos Approval Drafts v007\n\n"
...
def update_manifest(files: dict[str, Path], video_sha: str) -> None:
    ...
    active.update(
        {
            ...
            "approval_drafts": "v007",
            "legal_rights_review_packet": "v007",
            "chapters": "v007",
            "tags": "v007",
            "title": "v007",
        }
    ...
    add_artifact("final_delivery", "v007", files["final_delivery"])
    add_artifact("youtube_metadata", "v007", files["youtube_meta"])
    add_artifact("rights_manifest", "v007", files["rights_manifest"])
    add_artifact("owner_review_request", "v007", files["owner_review_request"])
    add_artifact("approval_drafts", "v007", files["approval_drafts"])
    add_artifact("legal_rights_review_packet", "v007", files["legal_packet"])
    add_artifact("final_acceptance", "v006", ACCEPTANCE)
    manifest["updated_at"] = now()
    manifest["warnings"] = [
        warning
        for warning in manifest.get("warnings", [])
        if "Current review_proxy v007 is 624.96s" not in warning
    ]
    manifest.setdefault("warnings", []).append(
        f"Final candidate v007 rendered and independently accepted: {str(FINAL_VIDEO).replace(chr(92), '/')} sha256={video_sha}; public release remains blocked by APR-0002..APR-0006 including R3 legal review."
    )
...
def append_event(video_sha: str, fd: Path) -> None:
    event = {
        ...
        "episode_id": EP,
        "stage": "final_package",
        "event": "final_candidate_v007_packaged_owner_stop",
        "revision": "v007",
        "actor": "codex",
