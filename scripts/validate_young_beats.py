#!/usr/bin/env python
"""Validate EP42 AE card beats against Remotion figures."""
...
def evaluate(dryrun: bool = False) -> dict:
    ...
    film_path = ROOT / "remotion" / "src" / "data" / "young_film.json"
    if film_path.exists():
    ...
    source = ROOT / "scripts" / "ae" / "build_young_hero_cards.py"
    if source.exists() and DISCLOSURE not in source.read_text(encoding="utf-8", errors="ignore"):
        errors.append("build_young_hero_cards.py lacks AI disclosure layer text")
...
def main() -> int:
    ...
    print(("PASS" if result["ok"] else "FAIL") + f" validate_young_beats: {result['beats']}")
