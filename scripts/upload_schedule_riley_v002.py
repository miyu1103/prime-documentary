#!/usr/bin/env python3
"""Upload EP7 Riley v002 to YouTube and schedule public release.
...
META = EPDIR / "09_package" / "youtube_meta.v002.json"
APR_ID = "APR-0004"
...
VIDEO = Path(r"H:\pd-media\episodes\PD-2026-007-riley\08_edit\riley_final_v002.mp4")
MANIFEST = EPDIR / "manifest.json"
RESULT = EPDIR / "09_package" / "youtube_schedule_result.v002.json"
EVENTS = EPDIR / "events" / "events.jsonl"

EXPECTED_VIDEO = "ad309dd02fb211beb7baea1e52af9d2eb3440908bbaef4bf0b125d5f163e8544"
...
def update_local_package_state(result: dict) -> None:
    ...
    meta.setdefault("pre_publish_checks", {})["supersedes_video_id"] = "ODCh4-UelI4"
    ...
    manifest.setdefault("active_revisions", {})["edit"] = "v002"
    manifest.setdefault("active_revisions", {})["captions_final"] = "v002"
    manifest.setdefault("active_revisions", {})["youtube_meta"] = "v002"
    manifest.setdefault("active_revisions", {})["youtube_schedule_result"] = "v002"
    approvals = manifest.setdefault("approvals", [])
    if APR_ID not in approvals:
        approvals.append(APR_ID)
    warning = f"YouTube private upload completed for Riley v002 and scheduled under {APR_ID}: video_id {result['video_id']}, public release {SCHEDULED_AT_LOCAL}. Previous v001 video ODCh4-UelI4 was kept private and unscheduled."
...
def verify_preconditions() -> tuple[dict, dict, Path]:
    if RESULT.exists():
        raise RuntimeError(f"Existing Riley v002 schedule result found; refusing duplicate upload: {RESULT.relative_to(ROOT)}")
    if not APR.exists():
...
def main(argv: list[str]) -> int:
    ...
    append_event({
        ...
        "actor": "codex",
        "approval_id": APR_ID,
        "detail": f"Uploaded EP7 Riley v002 to YouTube as private video_id={video_id} and scheduled public release for {SCHEDULED_AT_LOCAL}. Selected thumbnail set. Previous v001 upload ODCh4-UelI4 remains private and unscheduled. No public immediate publish performed.",
        "video_id": video_id,
        "watch": f"https://youtu.be/{video_id}",
        "studio": f"https://studio.youtube.com/video/{video_id}/edit",
        "scheduled_at_local": SCHEDULED_AT_LOCAL,
