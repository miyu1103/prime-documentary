import React from 'react';
import {Composition, registerRoot} from 'remotion';
import {BRAND} from './brand';
import {CarpenterPremium, carpenterPremiumDurationInFrames} from './compositions/CarpenterPremium';
import {CarpenterThumbnail, thumbnailVariants} from './compositions/CarpenterThumbnails';

const CarpenterRoot: React.FC = () => (
  <>
    <Composition
      id="CarpenterPremium"
      component={CarpenterPremium}
      durationInFrames={carpenterPremiumDurationInFrames(BRAND.video.fps)}
      fps={BRAND.video.fps}
      width={BRAND.video.width}
      height={BRAND.video.height}
    />
    {thumbnailVariants.map((variant, index) => (
      <Composition
        key={variant.id}
        id={`Carpenter-${variant.id}`}
        component={CarpenterThumbnail}
        durationInFrames={1}
        fps={BRAND.video.fps}
        width={BRAND.thumb.width}
        height={BRAND.thumb.height}
        defaultProps={{variantIndex: index}}
      />
    ))}
  </>
);

registerRoot(CarpenterRoot);
