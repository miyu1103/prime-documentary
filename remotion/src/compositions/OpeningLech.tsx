...
export const openingLechDurationInFrames = (fps: number) => Math.round(fps * 3.0);

export const OpeningLech: React.FC<OpeningLechProps> = ({title, subtitle, accent, hasLogo}) => {
  const frame = useCurrentFrame();
  const {fps, width, height} = useVideoConfig();
  const bgOpacity = interpolate(frame, [sec(fps, T.bgIn), sec(fps, 0.4)], [0, 1], {extrapolateRight: 'clamp'});
  const bgScale = interpolate(frame, [0, openingLechDurationInFrames(fps)], [1.08, 1.0], {
    easing: Easing.out(Easing.cubic),
    extrapolateRight: 'clamp',
  });
  const gridReveal = spring({frame: frame - sec(fps, T.gridIn), fps, config: {damping: 200, mass: 1}, durationInFrames: sec(fps, 0.8)});
  const gridY = interpolate(frame, [0, openingLechDurationInFrames(fps)], [0, 48], {
    easing: Easing.inOut(Easing.sin),
    extrapolateRight: 'clamp',
  });
  const glow = spring({frame: frame - sec(fps, T.glowIn), fps, config: {damping: 18, mass: 1.2}});
  const fracture = spring({frame: frame - sec(fps, T.fractureIn), fps, config: {damping: 22, mass: 1.1}});
  ...
  const stagger = Math.max(1, sec(fps, T.charStagger));
    ...
    <AbsoluteFill style={{backgroundColor: '#0d0b08', overflow: 'hidden'}}>
      <AbsoluteFill
        style={{
          ...
          background: 'radial-gradient(120% 120% at 50% 35%, #3a2f1c 0%, #1c1710 45%, #0d0b08 100%)',
        }}
      />
      <AbsoluteFill
        style={{
      ...
      <AbsoluteFill style={{justifyContent: 'center', alignItems: 'center'}}>
        <div
          style={{
            width: width * 0.78,
            height: 3,
            transform: `translateY(-6px) scaleX(${fracture})`,
            opacity: interpolate(fracture, [0, 1], [0, 0.55]),
            transformOrigin: 'center',
            background: `linear-gradient(90deg, transparent 0%, ${accent}00 8%, ${accent}cc 34%, ${accent}55 52%, ${accent}cc 71%, ${accent}00 92%, transparent 100%)`,
          }}
        />
      </AbsoluteFill>
      <AbsoluteFill style={{justifyContent: 'center', alignItems: 'center'}}>
      ...
      <AbsoluteFill style={{justifyContent: 'center', alignItems: 'center', transform: 'translateY(-70px)'}}>
        <Trail layers={6} lagInFrames={1.2} trailOpacity={0.45}>
          <AbsoluteFill style={{justifyContent: 'center', alignItems: 'center'}}>
            <div style={{display: 'flex', fontFamily: '"Oswald", "Archivo", Impact, sans-serif', fontWeight: 800, fontSize: 150, letterSpacing: 0, color: '#fff', lineHeight: 1.05}}>
              {title.split('').map((ch, i) => {
                const s = spring({frame: frame - sec(fps, T.titleIn) - i * stagger, fps, config: {damping: 16, mass: 1}});
                ...
                return (
                  <span key={`${ch}-${i}`} style={{display: 'inline-block', overflow: 'hidden', paddingBottom: '0.12em'}}>
                    <span style={{display: 'inline-block', transform: `translateY(${y}%)`, opacity: op, whiteSpace: 'pre'}}>{ch}</span>
                  </span>
                );
              })}
            </div>
      ...
      <AbsoluteFill style={{justifyContent: 'center', alignItems: 'center', transform: 'translateY(55px)'}}>
        <div style={{display: 'flex', flexDirection: 'column', gap: 18, alignItems: 'center'}}>
          <div style={{width: 240, height: 6, borderRadius: 3, background: accent, boxShadow: `0 0 24px ${accent}aa`, transform: `scaleX(${accentSpring})`, transformOrigin: 'left center'}} />
