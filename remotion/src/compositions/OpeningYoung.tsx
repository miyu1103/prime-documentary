...
export const openingYoungDurationInFrames = (fps: number) => Math.round(fps * 3.0);

export const OpeningYoung: React.FC<OpeningYoungProps> = ({title, subtitle, accent, hasLogo}) => {
  const frame = useCurrentFrame();
  const {fps, width, height} = useVideoConfig();
  const bgOpacity = interpolate(frame, [0, sec(fps, 0.4)], [0, 1], {extrapolateRight: 'clamp'});
  const bgScale = interpolate(frame, [0, openingYoungDurationInFrames(fps)], [1.08, 1.0], {
    ...
    <AbsoluteFill style={{backgroundColor: '#0A0A0C', overflow: 'hidden'}}>
      <AbsoluteFill
        style={{
          ...
          background: 'radial-gradient(110% 100% at 50% 34%, #10213d 0%, #0d1421 44%, #0A0A0C 100%)',
        }}
      />
      <AbsoluteFill
        style={{
          opacity: grid * 0.18,
          transform: `translateY(${interpolate(frame, [0, openingYoungDurationInFrames(fps)], [0, 48], {
