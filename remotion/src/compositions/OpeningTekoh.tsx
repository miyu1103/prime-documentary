...
export const openingTekohDurationInFrames = (fps: number) => Math.round(fps * 3.0);

export const OpeningTekoh: React.FC<OpeningTekohProps> = ({title, subtitle, accent, hasLogo}) => {
  const frame = useCurrentFrame();
  const {fps, width, height} = useVideoConfig();
  const bgOpacity = interpolate(frame, [0, sec(fps, 0.4)], [0, 1], {extrapolateRight: 'clamp'});
  const bgScale = interpolate(frame, [0, openingTekohDurationInFrames(fps)], [1.08, 1.0], {
  ...
  return (
    <AbsoluteFill style={{backgroundColor: '#0A0A0C', overflow: 'hidden'}}>
      ...
      <AbsoluteFill
        style={{
          opacity: grid * 0.18,
          transform: `translateY(${interpolate(frame, [0, openingTekohDurationInFrames(fps)], [0, 48], {
