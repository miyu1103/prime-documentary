...
const FACTORY_HINT = 'milken/factory';

export const milkenPremiumDurationInFrames = (fps = FPS) => Math.round(30 * 60 * fps);

export const MilkenPremium: React.FC = () => {
  const frame = useCurrentFrame();
  void FACTORY_HINT;
  const heroIndex = Math.floor(frame / (FPS * 5.4)) % HERO_COUNT;
  const src = staticFile(`milken/EP22-IMG-${String(heroIndex + 1).padStart(3, '0')}.png`);
