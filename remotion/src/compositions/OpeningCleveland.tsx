...
export const openingClevelandDurationInFrames = (fps: number) => Math.round(fps * 3.0);

export const OpeningCleveland: React.FC<OpeningClevelandProps> = ({title, subtitle, accent, hasLogo}) => {
  const frame = useCurrentFrame();
  const {fps, width, height} = useVideoConfig();
  const bgOpacity = interpolate(frame, [0, sec(fps, 0.4)], [0, 1], {extrapolateRight: 'clamp'});
  const bgScale = interpolate(frame, [0, openingClevelandDurationInFrames(fps)], [1.08, 1.0], {
  ...
  return (
    <AbsoluteFill style={{backgroundColor: '#0A0A0C', overflow: 'hidden'}}>
      ...
      <AbsoluteFill
        style={{
          opacity: grid * 0.18,
          transform: `translateY(${interpolate(frame, [0, openingClevelandDurationInFrames(fps)], [0, 48], {
