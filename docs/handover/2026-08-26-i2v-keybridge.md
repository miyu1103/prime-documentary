...
**追納完了(2026-08-27 03:55)**: H132〜H147 の16本を納品、H030 を明るく作り直し。
**motion = 128本**（112 + 新規16。H030 は差し替えなので本数は増えない）。master と pub はバイト全数一致、
depth 0、全 frame dir 81枚、check_motion_saturation exit 0、追納17本は元プレートと目視突き合わせ済み。

- **H030**: 「暗すぎる」の真因は i2v ではなく**元プレート**。実測 YAVG 10.9（他プレートは31〜64、
  中央値47付近）。旧クリップは既に 23〜25 で、i2v は倍以上明るくしていた。→ プロンプトでは解決しない。
  `img/` は触らず gamma=1.5 の補正版（YAVG 42.7）を `runs/qc/keybridge_h030_bright/` に作り
  `I2V_SRC` で入力。新クリップ **YAVG 50.8〜52.1**。元プレートとハッシュは不変。
- **H146**: 人物禁止ネガが**元プレートにあった作業員の腕まで消した**（H135/H137/H145 は保持されたので
  H146 のみ）。ネガから人物語を外し「すでに写っている人はそのまま、新しい人だけ入れない」に変えて
  作り直し、腕の保持を確認。**プレートに人が写っている絵では人物ネガを使わないこと。**
- **H147**: 待機ループのタイムアウトが16本目の完成より先に来て一度取りこぼした。追加で回して完成。
  ComfyUI 600秒タイムアウトを3回起こしている（concordia と GPU を奪い合ったため）。
